...
create index if not exists cpo_outages_current_sort_idx
  on public.complete_power_outages (starts_at, id)
  include (ends_at, source, source_status)
  where source_status in ('scheduled', 'active');

create index if not exists cpo_companies_visible_page_idx
  on public.complete_power_outage_companies
  (business_relevance_status, candidate_status, outage_address_id, id);

create or replace function public.get_complete_power_outage_company_page_v2(
...
create or replace function public.count_complete_power_outage_companies(
  p_mode text default 'current',
  p_query text default '',
  p_owner_filter text default 'all',
  p_source text default 'all',
...
revoke all on function public.get_complete_power_outage_company_page_v2(text,integer,timestamptz,uuid,text,text,text,text,text) from public, anon;
grant execute on function public.get_complete_power_outage_company_page_v2(text,integer,timestamptz,uuid,text,text,text,text,text) to authenticated;
revoke all on function public.count_complete_power_outage_companies(text,text,text,text,text,text) from public, anon;
grant execute on function public.count_complete_power_outage_companies(text,text,text,text,text,text) to authenticated;
