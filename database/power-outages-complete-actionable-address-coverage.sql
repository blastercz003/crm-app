...
-- Technicka normalizace a pouzitelnost pro vyhledavani firem jsou dve
-- rozdilne metriky. Presne adresy a ulice jsou akcni; cil obsahujici pouze
-- obec zustava auditne evidovany, ale nesmi zvysovat ani blokovat progress.
...
with sources(source) as (
  ...
  select
    ...
    count(*) filter (where address.normalization_version >= 2 and address.address_scope = 'street')::bigint as street_count,
    count(*) filter (where address.normalization_version >= 2 and address.address_scope = 'municipality')::bigint as municipality_count
...
select
  ...
  coalesce(address_counts.street_count, 0)::bigint as street_count,
  coalesce(address_counts.municipality_count, 0)::bigint as municipality_count
from sources
...
alter table public.complete_power_outage_address_coverage_snapshot
  add column if not exists street_count bigint not null default 0,
  add column if not exists municipality_count bigint not null default 0;
...
begin
  ...
  insert into public.complete_power_outage_address_coverage_snapshot (
    ...
    municipality_target_count, refreshed_at, street_count, municipality_count
  )
  select
    ...
    live.municipality_target_count, now(), live.street_count,
    live.municipality_count
...
-- Nove sloupce se nejprve naplni z live pohledu; teprve pote lze bezpecne
-- zpřísnit kontrolu již existujících snapshotových řádků.
alter table public.complete_power_outage_address_coverage_snapshot
  add constraint cpo_address_coverage_snapshot_counts_check check (
    ...
    and municipality_target_count >= 0 and street_count >= 0
    and municipality_count >= 0
    and normalized_count + pending_count = total_count
    and exact_count + street_count + municipality_count + unresolved_count = normalized_count
    and street_count + municipality_count = broad_count
...
select 'COLUMN' as check_type, 'actionable address coverage fields' as object_name,
  (select count(*) = 2
   from information_schema.columns
   where table_schema = 'public'
     and table_name = 'complete_power_outage_address_coverage_snapshot'
     and column_name in ('street_count', 'municipality_count')) as is_correct
union all
select 'DATA', 'address scope counts are internally consistent',
  not exists (
    select 1 from public.complete_power_outage_address_coverage_snapshot
    where exact_count + street_count + municipality_count + unresolved_count <> normalized_count
       or street_count + municipality_count <> broad_count
  )
union all
select 'LOGIC', 'municipality only addresses are separated from searchable coverage',
  position('municipality_count' in pg_get_viewdef(
    'public.complete_power_outage_address_coverage_live'::regclass, true)) > 0
union all
select 'GRANT', 'authenticated reads actionable coverage snapshot only',
  has_table_privilege('authenticated', 'public.complete_power_outage_address_coverage', 'SELECT')
  and not has_table_privilege('authenticated', 'public.complete_power_outage_address_coverage_live', 'SELECT')
union all
select 'ISOLATION', 'actionable coverage does not reference MARKET outage tables',
