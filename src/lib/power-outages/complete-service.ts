...
type OverviewRow = {
  ...
  communication_workflow_status?: CompleteCommunicationWorkflowStatus | null
...
function mapOverview(row: OverviewRow, assignment?: AssignmentRow): CompletePowerOutageListItem {
  return {
    ...
    communicationWorkflowStatus: row.communication_workflow_status
      && WORKFLOW_STATUSES.has(row.communication_workflow_status)
      ? row.communication_workflow_status
      : null,
...
export async function getCompletePowerOutageCommercialSelectionCounts(
  ...
  return {
    ...
    operationallySensitive: Number(payload.operationallySensitive) || 0,
...
export async function getCompletePowerOutageSidebarWorkspace(): Promise<CompletePowerOutageSidebarWorkspace> {
  ...
  const taskLoadError = taskResult.error
    ...
    : null
  const sourceLoadError = sourceResult.error
    ? `Stav distributorů se nepodařilo načíst: ${sourceResult.error.message}`
    : sourceDiscoveryResult.error
      ? `Průběh distributorů se nepodařilo načíst: ${sourceDiscoveryResult.error.message}`
      : cezNewResult.error && !cezNewMonitoringSchemaMissing(cezNewResult.error)
        ? `Stav nového sběru ČEZ se nepodařilo načíst: ${cezNewResult.error.message}`
        : taskLoadError
