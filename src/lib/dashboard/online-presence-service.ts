...
export type PresencePeriod = 'today' | '7d' | '30d'
...
function isMissingTableError(error: { code?: string; message?: string } | null, table: string) {
  ...
  return String(error.message ?? '').toLowerCase().includes(`relation "${table}" does not exist`)
}
...
export async function trackUserPresence(input?: TrackPresenceInput) {
  try {
    const supabase = await createClient()
    const {
      data: { user },
...
export async function getPresenceOverviewForAdmin(period: PresencePeriod = 'today') {
  try {
    const { supabase, user, error: accessError } = await requireAdmin()
    if (!user) {
      return { success: false, error: accessError, onlineUsers: [], usersInPeriod: [] }
    }

    const { data: presenceRows, error: presenceError } = await supabase
      .from('user_presence')
      .select('user_id, last_seen_at, last_route, last_section, last_action, last_action_at')
      .gte('last_seen_at', getPeriodStartIso(period))
      .neq('user_id', user.id)
      .order('last_seen_at', { ascending: false })

    if (presenceError) {
      const error = isMissingTableError(presenceError, 'user_presence')
        ? 'Chybí tabulka user_presence.'
        : 'Nepodařilo se načíst online uživatele.'
      return { success: false, error, onlineUsers: [], usersInPeriod: [] }
    }
    ...
    if (uniqueIds.length === 0) {
      return { success: true, error: null, onlineUsers: [], usersInPeriod: [] }
    }
    ...
    const onlineThresholdMs = Date.now() - ONLINE_WINDOW_MINUTES * 60_000
    ...
    return {
      success: true,
      error: null,
      onlineUsers: usersInPeriod.filter((item) => item.isOnline),
      usersInPeriod,
    }
  } catch {
    return {
...
export async function getUserActivityForAdmin(
  ...
  try {
    ...
    if (!normalizedUserId) return { success: false, error: 'Chybí ID uživatele.', items: [] }
    ...
    const { data, error } = await supabase
      ...
      .gte('created_at', getPeriodStartIso(period))
      .order('created_at', { ascending: false })
      .limit(safeLimit)

    if (error) {
      const message = isMissingTableError(error, 'user_activity_log')
        ? 'Chybí tabulka user_activity_log.'
        : 'Nepodařilo se načíst historii aktivit.'
      return { success: false, error: message, items: [] }
    }

    return { success: true, error: null, items: (data ?? []) as ActivityRow[] }
  } catch {
