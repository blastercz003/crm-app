...
export default function PowerOutagesLoading() {
  return (
    <main className="activities-page power-outages-page relative min-h-screen overflow-hidden bg-[linear-gradient(160deg,#f8fafc_0%,#eef3f8_50%,#e9f0f7_100%)] text-[var(--foreground)]">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-[1920px] flex-col gap-4 px-4 py-6 sm:px-6 lg:gap-5 lg:px-8" aria-busy="true" aria-label="Načítám plánované odstávky">
        ...
        <div className="grid items-start gap-5 xl:grid-cols-4 xl:gap-3">
          <section className="activities-page__panel min-w-0 rounded-[28px] border border-white/70 bg-[linear-gradient(155deg,rgba(255,255,255,0.96)_0%,rgba(248,250,252,0.92)_48%,rgba(241,245,249,0.88)_100%)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_20px_44px_rgba(15,23,42,0.12)] sm:p-5 xl:col-span-3">
            <div className="flex items-center justify-between gap-3"><div><Skeleton className="h-2.5 w-40" /><Skeleton className="mt-2 h-6 w-44" /></div><Skeleton className="h-12 w-72" /></div>
            <Skeleton className="mt-5 h-[420px] w-full rounded-[22px]" />
          </section>
          <aside className="space-y-4">
            <Skeleton className="h-[238px] w-full rounded-[24px]" />
