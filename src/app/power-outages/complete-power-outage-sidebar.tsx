      ...
      {sourceCards.map((card) => {
        if (card.kind === 'cez-new') {
          ...
          return <div key="cez-new" className={`weather-alerts__record-surface relative h-full rounded-xl border px-2.5 py-1 lg:px-3 lg:py-2 ${compact ? 'lg:h-[90px]' : 'lg:h-[94px]'}`}>
            <div className="flex items-center justify-between gap-2"><strong className="text-[13px] text-[var(--text-primary)]">{shadowActive ? 'ČEZ' : 'ČEZ – NEW'}</strong><CompletePanelStatusBadge label={cezNewStatusLabel(state.status)} badgeClassName={presentation.badge} dotClassName={presentation.dot} attention={presentation.attention || hasWarning} title={state.statusMessage} ariaLabel={`${shadowActive ? 'ČEZ' : 'ČEZ – NEW'}: ${cezNewStatusLabel(state.status)}. ${state.statusMessage} Zobrazit detail.`} onClick={() => setShowCezNew(true)} /></div>
