  ...
  return <PowerOutagePopupShell titleId="complete-cez-new-status" eyebrow={`KOMPLETNÍ SBĚR · ${shadowActive ? 'CELOPLOŠNÝ ZDROJ' : 'NOVÝ CELOPLOŠNÝ ZDROJ'}`} title={shadowActive ? <CezAllV1Label /> : 'ČEZ – NEW'} icon={<DatabaseZap aria-hidden size={21} />} onClose={onClose}>
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
      ...
      <section className="mt-4"><h3 className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]"><Route aria-hidden size={14} /> Navazující tok dat</h3><div className="mt-2 grid gap-2 sm:grid-cols-2">
        <div className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3"><div className="flex items-start justify-between gap-2"><span><strong className="block text-[10px] text-[var(--text-primary)]">1 · Publikace katalogu</strong><small className="mt-0.5 block text-[7px] text-[var(--text-secondary)]">Pouze úplný a bezpečný cyklus</small></span><strong className={`text-[8px] ${publicationIntegrityOk ? 'text-emerald-600 [html[data-theme=dark]_&]:text-emerald-300' : state.projectionStatus === 'failed' ? 'text-red-600 [html[data-theme=dark]_&]:text-red-300' : 'text-sky-600 [html[data-theme=dark]_&]:text-sky-300'}`}>{publicationStatus}</strong></div><p className="mt-2 text-[8px] text-[var(--text-secondary)]">{(source?.publishedOutageCount ?? 0).toLocaleString('cs-CZ')} publikovaných záznamů · {(source?.publishedAddressCount ?? 0).toLocaleString('cs-CZ')} adres</p>{publicationIntegrityAvailable && !publicationIntegrityOk ? <RecoveryControl isAdmin={isAdmin} blocked={false} label="Obnovit publikaci katalogu" request={{ target: 'cez_new_pipeline', stage: 'projection' }} onRecovered={onReload} /> : null}</div>
    ...
    <div className={`${refreshWarning ? 'mt-2 lg:mt-2' : 'mt-3 lg:mt-4'} grid min-h-0 flex-1 gap-2 ${compact ? 'grid-rows-4' : 'grid-rows-3'} lg:block lg:space-y-2`}>
      {sourceCards.map((card) => {
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <CompletePanelStatusBadge label={discoveryStatusLabel(discovery.status)} badgeClassName={presentation.badge} dotClassName={presentation.dot} attention={presentation.attention || discovery.errorTargetCount > 0} title={discovery.statusMessage} ariaLabel={`${sourceLabel(source.source)}: ${discoveryStatusLabel(discovery.status)}. ${discovery.statusMessage} Zobrazit provozní detail.`} onClick={() => openSource(source.source)} />
        </div>
...
function CoveragePanel({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  ...
  return <PanelShell>
    ...
    <button type="button" onClick={openDetail} className="mt-4 rounded-2xl border border-sky-400/25 bg-sky-500/8 p-3.5 text-left transition hover:-translate-y-px hover:border-sky-400/45 hover:bg-sky-500/10" aria-label="Otevřít detail pokrytí adres">
    ...
    {showDetail && typeof document !== 'undefined' ? createPortal(<AddressCoverageDiagnosticPopup diagnostic={diagnostic} loading={loading} error={error} isAdmin={workspace.currentUser.isAdmin} onReload={() => loadDiagnostic(false)} onClose={() => { setShowDetail(false); setDiagnostic(null); setError(null) }} />, document.body) : null}
  </PanelShell>
}

const COMMERCIAL_SELECTION_OPTIONS: Array<{ value: CompleteCommercialSelectionFilter; label: string; description: string }> = [
  { value: 'top', label: 'TOP KANDIDÁTI', description: 'Výběr doporučený současnými pravidly' },
...
function CommercialSelectionPanel({ workspace, value, sort, counts, onChange, onSortChange, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; value: CompleteCommercialSelectionFilter; sort: CompleteCommercialSort; counts: CompleteCommercialSelectionCounts | null; onChange: (value: CompleteCommercialSelectionFilter) => void; onSortChange: (value: CompleteCommercialSort) => void; onRetry?: () => void }) {
  ...
  return <PanelShell>
    ...
    <div className="mt-3 grid grid-cols-2 gap-2" role="group" aria-label="Obchodní výběr firem">{COMMERCIAL_SELECTION_OPTIONS.map((option) => {
      ...
      return <button key={option.value} type="button" onClick={() => onChange(option.value)} aria-pressed={value === option.value} title={option.description} className={`flex h-[58px] flex-col items-center justify-center rounded-xl border px-2 text-center transition hover:-translate-y-px ${value === option.value ? 'border-violet-400/45 bg-violet-500/12 text-violet-700 shadow-sm [html[data-theme=dark]_&]:text-violet-200' : 'border-[var(--surface-border)] bg-[var(--surface-muted)] text-[var(--text-secondary)] hover:border-violet-400/30 hover:text-[var(--text-primary)]'}`}><strong className="text-[8px] font-extrabold uppercase tracking-[0.055em]">{option.label}</strong><small className="mt-0.5 line-clamp-1 text-[6px] font-medium normal-case tracking-normal opacity-80">{option.description}</small>{workspace.commercialSelection.countsAndSortingEnabled ? <small className="mt-0.5 text-[7px] font-extrabold tabular-nums text-[var(--text-primary)]">{count == null ? 'Načítám…' : `${count.toLocaleString('cs-CZ')} záznamů`}</small> : null}</button>
    })}</div>
