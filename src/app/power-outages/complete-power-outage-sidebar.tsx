...
function CommercialSelectionPanel({ workspace, value, sort, counts, onChange, onSortChange, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; value: CompleteCommercialSelectionFilter; sort: CompleteCommercialSort; counts: CompleteCommercialSelectionCounts | null; onChange: (value: CompleteCommercialSelectionFilter) => void; onSortChange: (value: CompleteCommercialSort) => void; onRetry?: () => void }) {
  ...
  return <PanelShell>
    <div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-violet-500/10 text-violet-600 [html[data-theme=dark]_&]:text-violet-300"><Sparkles aria-hidden size={18} /></span><span className="min-w-0"><small className="block whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Obchodní výběr</small><h3 className="whitespace-nowrap text-base font-semibold text-[var(--text-primary)]">AI SELECT</h3></span></div>
