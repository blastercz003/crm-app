...
function cezNewStatusLabel(status: CompleteCezNewState['status'], active = false) {
  if (active && status === 'ready') return 'AKTUÁLNÍ'
...
function cezAllPipelineDelayed(state: CompleteCezNewState) {
  const reference = state.stage === 'ready'
    ? state.scanFinishedAt ?? state.lastProjectionAt
    : state.scanStartedAt
  if (!reference) return false
  const referenceMs = new Date(reference).getTime()
  if (!Number.isFinite(referenceMs)) return false
  const thresholdMs = state.stage === 'ready' ? 30 * 60 * 60_000 : 24 * 60 * 60_000
...
function cezAllStatus(state: CompleteCezNewState, source: CompleteSourceState | null) {
  ...
  const productionNormalizationError = state.productionAddressError > 0
    || (state.productionAddressPending > 0 && state.productionNormalizationTaskStatus === 'failed')
  const productionNormalizationPartial = state.productionAddressReview > 0
  const productionNormalizationProcessing = state.productionAddressPending > 0
  const status: CompleteSourceState['discovery']['status'] = state.status === 'error' || productionNormalizationError || discovery?.status === 'error'
    ? 'error'
    : pipelineDelayed || discovery?.status === 'delayed'
      ? 'delayed'
      : state.status === 'partial' || productionNormalizationPartial || discovery?.status === 'partial'
        ? 'partial'
        : state.status === 'processing' || productionNormalizationProcessing || discovery?.status === 'processing'
          ? 'processing'
          : state.status === 'waiting' || discovery?.status === 'waiting'
            ? 'waiting'
            : 'current'
  const message = status === 'error'
    ? productionNormalizationError
      ? state.productionNormalizationErrorMessage ?? 'Produkční normalizace adres ČEZ skončila chybou.'
      : state.status === 'error'
      ? state.statusMessage
    ...
    : status === 'delayed'
      ...
      : status === 'partial'
        ? productionNormalizationPartial
          ? `${state.productionAddressReview} produkčních adres ČEZ vyžaduje kontrolu.`
          : state.status === 'partial' ? state.statusMessage : discovery?.statusMessage ?? state.statusMessage
        : status === 'processing'
          ? state.status === 'processing'
            ? state.statusMessage
            : productionNormalizationProcessing
              ? `${state.productionAddressPending} produkčních adres ČEZ čeká na normalizaci.`
              : discovery?.statusMessage ?? state.statusMessage
          : status === 'current'
...
function cezNewStageLabel(stage: CompleteCezNewState['stage'], active = false) {
  if (active) return ({ ruian: 'Aktualizace katalogu RÚIAN', mapping: 'Aktualizace území ČEZ', scan: 'Celoplošný sběr odstávek', normalization: 'Příprava zdrojových adres', projection: 'Produkční publikace', ready: 'Katalog je aktuální' } as const)[stage]
  return ({ ruian: 'Katalog RÚIAN', mapping: 'Mapování území ČEZ', scan: 'Sběr odstávek', normalization: 'Ověření adres', projection: 'Příprava projekce', ready: 'Připraveno k přepnutí' } as const)[stage]
}

function CezAllV1Label({ compact = false }: { compact?: boolean }) {
  return <>
    <span>ČEZ</span>{' '}
    <span className={`${compact ? 'text-[9px]' : 'text-[0.62em]'} font-medium text-[var(--text-secondary)]`}>ALL v1</span>
  </>
}
...
function addressCoverageStatusPresentation(status: CompleteAddressCoverage['status']) {
  if (status === 'current') return { badge: 'border-emerald-400/35 bg-emerald-400/10 text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300', dot: 'bg-emerald-500', attention: false }
  if (status === 'processing') return { badge: 'border-sky-400/35 bg-sky-400/10 text-sky-700 [html[data-theme=dark]_&]:text-sky-300', dot: 'animate-pulse bg-sky-500 motion-reduce:animate-none', attention: false }
  if (status === 'partial') return { badge: 'border-amber-400/35 bg-amber-400/10 text-amber-700 [html[data-theme=dark]_&]:text-amber-300', dot: 'bg-amber-500', attention: true }
  if (status === 'error') return { badge: 'border-red-400/35 bg-red-400/10 text-red-700 [html[data-theme=dark]_&]:text-red-300', dot: 'bg-red-500', attention: true }
  return { badge: 'border-slate-400/35 bg-slate-400/10 text-slate-600 [html[data-theme=dark]_&]:text-slate-300', dot: 'bg-slate-400', attention: false }
}

function addressCoveragePercent(coverage: Pick<CompleteAddressCoverage, 'searchableCount' | 'progressTotalCount'>) {
  return coverage.progressTotalCount > 0
    ? Math.round((coverage.searchableCount / coverage.progressTotalCount) * 1000) / 10
    : 100
}
...
function PanelLoadError({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return <div className="flex min-h-0 flex-1 flex-col items-center justify-center px-4 text-center"><CircleAlert aria-hidden size={25} className="text-red-500" /><strong className="mt-3 text-xs text-[var(--text-primary)]">Data tohoto panelu se nepodařilo načíst</strong><p className="mt-1 max-w-sm text-[9px] leading-4 text-[var(--text-secondary)]">{message}</p>{onRetry ? <button type="button" onClick={onRetry} className="mt-3 inline-flex h-8 items-center justify-center gap-1.5 rounded-xl bg-[var(--text-primary)] px-3 text-[8px] font-bold uppercase text-[var(--surface-page)] transition hover:-translate-y-px"><RefreshCw aria-hidden size={12} /> Zkusit znovu</button> : null}</div>
}

function globalProgressPresentation(status: CompleteGlobalProgress['status']) {
  if (status === 'current') return { label: 'AKTUÁLNÍ', badge: 'border-emerald-400/35 bg-emerald-400/10 text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300', dot: 'bg-emerald-500', bar: 'bg-emerald-500' }
  if (status === 'processing') return { label: 'ZPRACOVÁNÍ', badge: 'border-sky-400/35 bg-sky-400/10 text-sky-700 [html[data-theme=dark]_&]:text-sky-300', dot: 'animate-pulse bg-sky-500 motion-reduce:animate-none', bar: 'bg-sky-500' }
  if (status === 'delayed') return { label: 'ZPOŽDĚNÍ', badge: 'border-amber-400/35 bg-amber-400/10 text-amber-700 [html[data-theme=dark]_&]:text-amber-300', dot: 'bg-amber-500', bar: 'bg-amber-500' }
  if (status === 'error') return { label: 'CHYBA', badge: 'border-red-400/35 bg-red-400/10 text-red-700 [html[data-theme=dark]_&]:text-red-300', dot: 'bg-red-500', bar: 'bg-red-500' }
  return { label: 'ČEKÁ', badge: 'border-slate-400/35 bg-slate-400/10 text-slate-600 [html[data-theme=dark]_&]:text-slate-300', dot: 'bg-slate-400', bar: 'bg-slate-400' }
}

function formatRemainingTime(seconds: number | null) {
  if (seconds == null || !Number.isFinite(seconds)) return 'Odhad se připravuje'
  if (seconds <= 0) return 'Aktuální práce je dokončena'
  const minutes = Math.max(1, Math.ceil(seconds / 60))
  const days = Math.floor(minutes / 1440)
  const hours = Math.floor((minutes % 1440) / 60)
  const rest = minutes % 60
  const parts = [days > 0 ? `${days} d` : '', hours > 0 ? `${hours} h` : '', rest > 0 ? `${rest} min` : ''].filter(Boolean)
  return `Zbývá přibližně ${parts.join(' ')}`
}

function GlobalProgressPanel({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  const progress = workspace.globalProgress
  if (!progress) return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-500/10 text-[var(--accent)]"><Activity aria-hidden size={19} /></span><span><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">KOMPLETNÍ ANALÝZA</small><h3 className="text-base font-semibold text-[var(--text-primary)]">Celkové zpracování</h3></span></div><PanelLoadError message={workspace.loadErrors.globalProgress || 'Souhrnný průběh zatím není dostupný.'} onRetry={onRetry} /></PanelShell>
  const presentation = globalProgressPresentation(progress.status)
  const percent = Math.min(100, Math.max(0, progress.progressPercent))
  return <PanelShell>
    <div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-500/10 text-[var(--accent)]"><Activity aria-hidden size={19} /></span><span className="min-w-0"><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">KOMPLETNÍ ANALÝZA</small><h3 className="truncate text-base font-semibold text-[var(--text-primary)]">Celkové zpracování</h3></span></div><CompletePanelStatusBadge label={presentation.label} badgeClassName={presentation.badge} dotClassName={presentation.dot} /></div>
    <div className="mt-4 rounded-2xl border border-sky-400/25 bg-sky-500/8 p-3.5">
      <div className="flex items-end justify-between gap-3"><span><small className="block text-[7px] font-bold uppercase tracking-[0.08em] text-[var(--text-secondary)]">Všechny aktivní fronty</small><strong className="mt-1 block text-[11px] leading-4 text-[var(--text-primary)]">{formatRemainingTime(progress.remainingSeconds)}</strong></span><strong className="shrink-0 text-3xl leading-none tabular-nums text-[var(--accent)]">{percent.toLocaleString('cs-CZ')} %</strong></div>
      <div className="mt-3 h-2 overflow-hidden rounded-full bg-[var(--surface-border)]"><div className={`h-full rounded-full transition-[width] duration-500 ${presentation.bar}`} style={{ width: `${percent}%` }} /></div>
    </div>
    <p className="mt-3 text-center text-[8px] leading-4 text-[var(--text-secondary)]">{progress.statusMessage}</p>
    <div className="mt-auto flex items-center justify-between gap-3 pt-3 text-[7px] text-[var(--text-secondary)]"><span>{progress.activeQueueCount.toLocaleString('cs-CZ')} {progress.activeQueueCount === 1 ? 'aktivní fronta' : progress.activeQueueCount >= 2 && progress.activeQueueCount <= 4 ? 'aktivní fronty' : 'aktivních front'}</span><span>Výpočet: {formatRelativeDate(progress.refreshedAt)}</span></div>
  </PanelShell>
}
...
function SkipProviderReviewErrorsButton({ provider, count, onRecovered }: {
  provider: 'ares' | 'mapy'
  count: number
  onRecovered: () => void | Promise<void>
}) {
  ...
  const skip = async () => {
    ...
    const confirmed = window.confirm(
      `Opravdu přeskočit ${count.toLocaleString('cs-CZ')} ${count === 1 ? 'chybný dotaz' : count < 5 ? 'chybné dotazy' : 'chybných dotazů'}? Původní chyba zůstane uložená pro audit, ale tyto dotazy už nebudou blokovat aktuální frontu.`,
    )
    if (!confirmed) return
    setState('running')
    setMessage(null)
    try {
      const response = await fetch('/api/power-outages/complete/recover', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ target: 'provider_review_skip', provider } satisfies RecoveryRequest),
      })
      const payload = await response.json() as { ok?: boolean; message?: string; error?: string }
      if (!response.ok || !payload.ok) throw new Error(payload.error ?? 'Chybné dotazy se nepodařilo přeskočit.')
      setState('success')
      setMessage(payload.message ?? 'Chybné dotazy byly přeskočeny.')
      await onRecovered()
    } catch (skipError) {
      setState('error')
      setMessage(skipError instanceof Error ? skipError.message : 'Chybné dotazy se nepodařilo přeskočit.')
    }
  }

  return <div className="mt-2 flex flex-col items-start gap-2 sm:flex-row sm:items-center"><button type="button" onClick={() => void skip()} disabled={state === 'running'} className="inline-flex h-9 items-center justify-center gap-2 rounded-xl border border-amber-400/40 bg-amber-500/10 px-4 text-[9px] font-bold uppercase tracking-[0.07em] text-amber-800 transition hover:-translate-y-px hover:border-amber-400/60 hover:bg-amber-500/15 disabled:cursor-wait disabled:opacity-60 [html[data-theme=dark]_&]:text-amber-300"><CircleAlert aria-hidden size={13} />{state === 'running' ? 'PŘESKAKUJI…' : state === 'success' ? 'PŘESKOČENO' : 'PŘESKOČIT CHYBNÉ DOTAZY'}</button>{message ? <p role="status" className={`text-[9px] leading-4 ${state === 'error' ? 'text-red-600 [html[data-theme=dark]_&]:text-red-300' : 'text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300'}`}>{message}</p> : null}</div>
}
      ...
      {loading ? <div className="flex min-h-[320px] flex-1 flex-col items-center justify-center p-6 text-center"><span className="h-8 w-8 animate-spin rounded-full border-2 border-[var(--surface-border)] border-t-[var(--accent)]" /><strong className="mt-4 text-sm text-[var(--text-primary)]">Načítám provozní stav…</strong></div> : error || !diagnostic ? <div className="flex min-h-[300px] flex-1 flex-col items-center justify-center p-6 text-center"><CircleAlert aria-hidden size={28} className="text-red-500" /><strong className="mt-3 text-sm text-[var(--text-primary)]">Provozní detail se nepodařilo načíst</strong><p className="mt-1 max-w-md text-xs leading-5 text-[var(--text-secondary)]">{error ?? 'Data nejsou dostupná.'}</p></div> : (
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
          <section className="rounded-2xl border border-sky-400/25 bg-sky-500/8 p-3.5">
            ...
            <p className="mt-2 text-[9px] text-[var(--text-secondary)]">Zbývá {diagnostic.state.discovery.remainingTargetCount} dotazů · k vyhodnocení {diagnostic.state.discovery.pendingCandidateCount} kandidátů · přesná čísla {diagnostic.state.discovery.exactTargetCount} · ulice {diagnostic.state.discovery.streetTargetCount}</p>
          </section>
...
function CompleteCezNewDiagnosticPopup({ state, source, isAdmin, onReload, onClose }: { state: CompleteCezNewState; source: CompleteSourceState | null; isAdmin: boolean; onReload: () => void | Promise<void>; onClose: () => void }) {
  ...
  const presentation = discoveryStatusPresentation(aggregate.status)
  const discoveryPresentation = discoveryStatusPresentation(discovery?.status ?? 'waiting')
  const shadowActive = state.activeSource === 'shadow'
  const scanPercent = boundedProgressPercent(state.scanProcessed, state.scanTotal)
  const productionNormalizationFailed = state.productionAddressError > 0
    || (state.productionAddressPending > 0
      && (state.productionNormalizationTaskStatus === 'failed'
        || state.productionNormalizationTaskStatus === 'partial'))
  const productionNormalizationProcessing = state.productionNormalizationTaskStatus === 'running'
    || state.productionAddressPending > 0
  const productionNormalizationReview = state.productionAddressReview > 0
  const productionNormalizationStatus = productionNormalizationFailed
    ? 'CHYBA'
    : productionNormalizationReview
      ? 'KONTROLA'
      : productionNormalizationProcessing
        ? 'ZPRACOVÁNÍ'
        : state.productionNormalizationLastSuccessAt
          ? 'AKTUÁLNÍ'
          : 'ČEKÁ'
  const productionNormalizationPercent = boundedProgressPercent(
    state.productionAddressNormalized,
    state.productionAddressTotal,
    100,
  )
  const evaluationPercent = discovery
    ? boundedProgressPercent(discovery.evaluatedCandidateCount, discovery.candidateCount, 100)
    : 0
  const publicationCycleMatches = Boolean(
    source?.publishedCycleId
      && state.latestCompleteCycleId
      && source.publishedCycleId === state.latestCompleteCycleId,
  )
  const publicationOutageCountMatches = Boolean(
    source && source.publishedOutageCount === state.projectedOutageCount,
  )
  const publicationAddressCountMatches = Boolean(
    source && source.publishedAddressCount === state.projectedAddressCount,
  )
  const publicationIntegrityAvailable = Boolean(source && state.latestCompleteCycleId)
  const publicationIntegrityOk = publicationIntegrityAvailable
    && publicationCycleMatches
    && publicationOutageCountMatches
    && publicationAddressCountMatches
    && state.projectionStatus === 'ready'
  const publicationTransitionActive = state.status === 'processing'
    || ['building', 'partial_ready', 'processing'].includes(state.projectionStatus)
  const publicationFailed = state.projectionStatus === 'failed'
    || state.errorStage === 'projection'
  const publicationRecoveryNeeded = publicationIntegrityAvailable
    && !publicationIntegrityOk
    && !publicationTransitionActive
    && (publicationFailed || state.stage === 'ready')
  const publicationStatus = publicationIntegrityOk
    ? 'AKTUÁLNÍ'
    : publicationFailed
      ? 'CHYBA'
      : 'ZPRACOVÁNÍ'
  const publicationChanges = source?.lastPublicationChanges ?? null
  const pipelineRecoveryStage = state.stage === 'ready' ? 'scan' : state.stage
  const pipelineRecoveryBlocked = recoveryNeedsManualRepair(state.errorCode, state.lastErrorMessage)
  const normalizationRecoveryBlocked = recoveryNeedsManualRepair(
    state.productionNormalizationErrorCode,
    state.productionNormalizationErrorMessage,
  )
  return <PowerOutagePopupShell titleId="complete-cez-new-status" eyebrow={`KOMPLETNÍ SBĚR · ${shadowActive ? 'CELOPLOŠNÝ ZDROJ' : 'NOVÝ CELOPLOŠNÝ ZDROJ'}`} title={shadowActive ? <CezAllV1Label /> : 'ČEZ – NEW'} icon={<DatabaseZap aria-hidden size={21} />} onClose={onClose}>
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
      <section className="rounded-2xl border border-sky-400/25 bg-sky-500/8 p-4">
        <div className="flex items-start justify-between gap-3"><span><small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--text-secondary)]">Celkový provozní stav</small><strong className="mt-1 block text-base text-[var(--text-primary)]">{cezNewStageLabel(state.stage, shadowActive)}</strong><p className="mt-1 max-w-lg text-[9px] leading-4 text-[var(--text-secondary)]">{aggregate.message}</p></span><span className={`relative inline-flex h-8 w-[108px] shrink-0 items-center justify-center gap-1.5 rounded-xl border px-2 text-[8px] font-bold uppercase leading-none tracking-[0.06em] ${presentation.badge}`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${presentation.dot}`} />{discoveryStatusLabel(aggregate.status)}{presentation.attention || hasWarning ? <i className="absolute -right-1.5 -top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-amber-400 text-[9px] font-black not-italic text-amber-950">!</i> : null}</span></div>
        <div className="mt-3 grid gap-2 sm:grid-cols-3"><PowerOutageDetailRow label="Poslední dokončený cyklus" value={formatDate(state.lastCompletedScanAt)} /><PowerOutageDetailRow label="Poslední publikace" value={formatDate(source?.lastSuccessAt ?? null)} /><PowerOutageDetailRow label="Aktivní zdroj" value={shadowActive ? 'ČEZ ALL v1' : 'Původní ČEZ'} /></div>
      </section>

      <section className="mt-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-4"><div className="flex items-end justify-between gap-3"><span><small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--text-secondary)]">Celoplošný cyklus ČEZ</small><strong className="mt-1 block text-sm text-[var(--text-primary)]">{state.status === 'processing' && state.stage === 'scan' ? 'Právě probíhá kontrola obcí' : 'Poslední cyklus'}</strong></span><strong className="text-xl tabular-nums text-[var(--accent)]">{scanPercent.toLocaleString('cs-CZ')} %</strong></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[var(--surface-strong)]"><div className="h-full rounded-full bg-sky-500 transition-[width] duration-700" style={{ width: `${scanPercent}%` }} /></div><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><PowerOutageDetailRow label="Prověřené obce" value={`${state.scanProcessed.toLocaleString('cs-CZ')} / ${state.scanTotal.toLocaleString('cs-CZ')}`} /><PowerOutageDetailRow label="Nalezené odstávky" value={state.scanOutageCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Zdrojové adresy" value={state.scanAddressCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Automatické opakování" value={state.scanRetryableErrorCount.toLocaleString('cs-CZ')} /></div>{state.scanReviewErrorCount > 0 ? <p className="mt-2 text-[9px] font-semibold text-red-600 [html[data-theme=dark]_&]:text-red-300">{state.scanReviewErrorCount.toLocaleString('cs-CZ')} obcí vyžaduje kontrolu.</p> : null}</section>

      <section className="mt-4"><h3 className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]"><Route aria-hidden size={14} /> Navazující tok dat</h3><div className="mt-2 grid gap-2 sm:grid-cols-2">
        <div className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3"><div className="flex items-start justify-between gap-2"><span><strong className="block text-[10px] text-[var(--text-primary)]">1 · Publikace katalogu</strong><small className="mt-0.5 block text-[7px] text-[var(--text-secondary)]">Pouze úplný a bezpečný cyklus</small></span><strong className={`text-[8px] ${publicationIntegrityOk ? 'text-emerald-600 [html[data-theme=dark]_&]:text-emerald-300' : publicationFailed ? 'text-red-600 [html[data-theme=dark]_&]:text-red-300' : 'text-sky-600 [html[data-theme=dark]_&]:text-sky-300'}`}>{publicationStatus}</strong></div><p className="mt-2 text-[8px] text-[var(--text-secondary)]">{(source?.publishedOutageCount ?? 0).toLocaleString('cs-CZ')} publikovaných záznamů · {(source?.publishedAddressCount ?? 0).toLocaleString('cs-CZ')} adres</p>{publicationRecoveryNeeded ? <RecoveryControl isAdmin={isAdmin} blocked={false} label="Obnovit publikaci katalogu" request={{ target: 'cez_new_pipeline', stage: 'projection' }} onRecovered={onReload} /> : null}</div>
        <div className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3"><div className="flex items-start justify-between gap-2"><span><strong className="block text-[10px] text-[var(--text-primary)]">2 · Normalizace v KOMPLETNÍ</strong><small className="mt-0.5 block text-[7px] text-[var(--text-secondary)]">Příprava produkčních adres pro providery</small></span><strong className={`text-[8px] ${productionNormalizationFailed ? 'text-red-600 [html[data-theme=dark]_&]:text-red-300' : productionNormalizationReview ? 'text-amber-700 [html[data-theme=dark]_&]:text-amber-300' : productionNormalizationProcessing ? 'text-sky-600 [html[data-theme=dark]_&]:text-sky-300' : 'text-emerald-600 [html[data-theme=dark]_&]:text-emerald-300'}`}>{productionNormalizationStatus}</strong></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--surface-strong)]"><span className="block h-full rounded-full bg-sky-500" style={{ width: `${productionNormalizationPercent}%` }} /></div><p className="mt-2 text-[8px] text-[var(--text-secondary)]">{state.productionAddressNormalized.toLocaleString('cs-CZ')} / {state.productionAddressTotal.toLocaleString('cs-CZ')} · zbývá {state.productionAddressPending.toLocaleString('cs-CZ')} · problémy {(state.productionAddressError + state.productionAddressReview).toLocaleString('cs-CZ')}</p>{productionNormalizationFailed ? <RecoveryControl isAdmin={isAdmin} blocked={normalizationRecoveryBlocked} label="Obnovit celkovou normalizaci" request={{ target: 'address_normalization' }} onRecovered={onReload} /> : null}</div>
      </div></section>

      <div aria-hidden className="mt-2 flex h-6 items-center justify-center text-base font-bold leading-none text-[var(--text-secondary)]">↓</div>

      <section className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-4">
        <div className="flex items-start justify-between gap-3">
          <span>
            <small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--text-secondary)]">3 · Vyhledávání firem · 30 dnů</small>
            <strong className="mt-1 block text-sm text-[var(--text-primary)]">{discovery ? `${discovery.completedTargetCount.toLocaleString('cs-CZ')} / ${discovery.totalTargetCount.toLocaleString('cs-CZ')} adresních dotazů` : 'Průběh zatím není dostupný'}</strong>
            <p className="mt-1 max-w-lg text-[9px] leading-4 text-[var(--text-secondary)]">ARES a Mapy.com zpracovávají své cíle paralelně.</p>
          </span>
          <span className={`relative inline-flex h-8 w-[108px] shrink-0 items-center justify-center gap-1.5 rounded-xl border px-2 text-[8px] font-bold uppercase leading-none tracking-[0.06em] ${discoveryPresentation.badge}`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${discoveryPresentation.dot}`} />{discoveryStatusLabel(discovery?.status ?? 'waiting')}</span>
        </div>
        {discovery ? <>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] p-3"><strong className="block text-[10px] text-[var(--text-primary)]">ARES</strong><small className="text-[7px] font-semibold uppercase tracking-[0.06em] text-[var(--text-secondary)]">Přesná čísla</small><p className="mt-2 text-[8px] text-[var(--text-secondary)]">Hotovo {Math.max(0, discovery.exactTargetCount - discovery.exactPendingTargetCount - discovery.exactErrorTargetCount)} / {discovery.exactTargetCount} · problémy {discovery.exactErrorTargetCount}</p>{discovery.exactErrorTargetCount > 0 || (discovery.status === 'delayed' && discovery.exactPendingTargetCount > 0) ? <RecoveryControl isAdmin={isAdmin} blocked={false} label="Spustit bezpečnou dávku ARES" request={{ target: 'provider_discovery', provider: 'ares' }} onRecovered={onReload} /> : null}</div>
            <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] p-3"><strong className="block text-[10px] text-[var(--text-primary)]">Mapy.com</strong><small className="text-[7px] font-semibold uppercase tracking-[0.06em] text-[var(--text-secondary)]">Uliční cíle</small><p className="mt-2 text-[8px] text-[var(--text-secondary)]">Hotovo {Math.max(0, discovery.streetTargetCount - discovery.streetPendingTargetCount - discovery.streetErrorTargetCount)} / {discovery.streetTargetCount} · problémy {discovery.streetErrorTargetCount}</p>{discovery.streetErrorTargetCount > 0 || (discovery.status === 'delayed' && discovery.streetPendingTargetCount > 0) ? <RecoveryControl isAdmin={isAdmin} blocked={false} label="Spustit bezpečnou dávku Mapy.com" request={{ target: 'provider_discovery', provider: 'mapy' }} onRecovered={onReload} /> : null}</div>
          </div>
          <div aria-hidden className="flex h-7 items-center justify-center text-base font-bold leading-none text-[var(--text-secondary)]">↓</div>
          <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] p-3">
            <div className="flex items-start justify-between gap-2"><span><strong className="block text-[10px] text-[var(--text-primary)]">4 · Vyhodnocení firem</strong><small className="text-[7px] font-semibold uppercase tracking-[0.06em] text-[var(--text-secondary)]">Kandidáti ČEZ ze všech providerů</small></span><strong className="text-xs tabular-nums text-[var(--accent)]">{evaluationPercent.toLocaleString('cs-CZ')} %</strong></div>
            <p className="mt-2 text-[8px] text-[var(--text-secondary)]">{discovery.evaluatedCandidateCount.toLocaleString('cs-CZ')} / {discovery.candidateCount.toLocaleString('cs-CZ')} · zbývá {discovery.pendingCandidateCount.toLocaleString('cs-CZ')}</p>
            {discovery.status === 'error' && discovery.exactErrorTargetCount === 0 && discovery.streetErrorTargetCount === 0 ? <RecoveryControl isAdmin={isAdmin} blocked={false} label="Obnovit celkové vyhodnocení" request={{ target: 'company_reconciliation' }} onRecovered={onReload} /> : null}
          </div>
        </> : null}
        <p className="mt-2 text-[8px] leading-4 text-[var(--text-secondary)]">{discovery?.statusMessage ?? 'Čekám na provozní snapshot navazujících firemních front.'}</p>
        <p className="mt-1 text-[8px] leading-4 text-[var(--text-secondary)]">Ruční spuštění ARES, Mapy.com nebo vyhodnocení používá společnou bezpečnou frontu všech distributorů; existující zámek zabrání duplicitnímu běhu.</p>
      </section>

      <section className={`mt-4 rounded-2xl border p-4 ${publicationIntegrityOk ? 'border-emerald-400/30 bg-emerald-500/8' : publicationIntegrityAvailable ? 'border-amber-400/30 bg-amber-500/8' : 'border-[var(--surface-border)] bg-[var(--surface-muted)]'}`}>
        <div className="flex items-start justify-between gap-3"><span><small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--text-secondary)]">Integrita publikace</small><strong className="mt-1 block text-sm text-[var(--text-primary)]">{publicationIntegrityOk ? 'Projekce a produkční katalog se shodují' : publicationIntegrityAvailable ? 'Probíhá předání nebo kontrola katalogu' : 'Kontrola zatím není dostupná'}</strong><p className="mt-1 text-[8px] leading-4 text-[var(--text-secondary)]">Kontroluje se totožnost bezpečného cyklu a počty odstávek i adres mezi projekcí a publikovaným katalogem.</p></span><span className={`inline-flex h-8 shrink-0 items-center gap-1.5 rounded-xl border px-3 text-[8px] font-bold uppercase tracking-[0.06em] ${publicationIntegrityOk ? 'border-emerald-400/35 bg-emerald-400/10 text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300' : publicationIntegrityAvailable ? 'border-amber-400/35 bg-amber-400/10 text-amber-700 [html[data-theme=dark]_&]:text-amber-300' : 'border-slate-400/35 bg-slate-400/10 text-slate-600 [html[data-theme=dark]_&]:text-slate-300'}`}><ShieldCheck aria-hidden size={13} />{publicationIntegrityOk ? 'V POŘÁDKU' : publicationIntegrityAvailable ? 'KONTROLA' : 'ČEKÁ'}</span></div>
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3"><PowerOutageDetailRow label="Publikovaný cyklus" value={publicationCycleMatches ? 'Shodný' : 'Čeká na shodu'} /><PowerOutageDetailRow label="Odstávky · projekce / katalog" value={`${state.projectedOutageCount.toLocaleString('cs-CZ')} / ${(source?.publishedOutageCount ?? 0).toLocaleString('cs-CZ')}`} /><PowerOutageDetailRow label="Adresy · projekce / katalog" value={`${state.projectedAddressCount.toLocaleString('cs-CZ')} / ${(source?.publishedAddressCount ?? 0).toLocaleString('cs-CZ')}`} /></div>
      </section>

      <section className="mt-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-4">
        <div className="flex items-start justify-between gap-3"><span><small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--text-secondary)]">Změny při poslední publikaci</small><strong className="mt-1 block text-sm text-[var(--text-primary)]">{publicationChanges ? 'Přesný rozdíl proti předchozímu katalogu' : 'Podrobná evidence se připravuje'}</strong></span>{publicationChanges?.completedAt ? <time className="shrink-0 text-[8px] font-semibold tabular-nums text-[var(--text-secondary)]">{formatDate(publicationChanges.completedAt)}</time> : null}</div>
        {publicationChanges ? <><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3"><PowerOutageDetailRow label="Nové odstávky" value={publicationChanges.outageAddedCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Aktualizované odstávky" value={publicationChanges.outageUpdatedCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Zrušené nebo zmizelé" value={publicationChanges.outageCancelledOrMissingCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Nové adresy" value={publicationChanges.addressAddedCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Změněné adresy" value={publicationChanges.addressUpdatedCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Odstraněné adresy" value={publicationChanges.addressRemovedCount.toLocaleString('cs-CZ')} /></div><p className="mt-2 text-[8px] leading-4 text-[var(--text-secondary)]">Kategorie jsou vzájemně oddělené a vztahují se pouze k uvedené publikaci.</p></> : <p className="mt-3 rounded-xl border border-dashed border-[var(--surface-border)] bg-[var(--surface-strong)] p-3 text-[9px] leading-4 text-[var(--text-secondary)]">Přesné rozdělení bude dostupné od první úspěšné publikace po nasazení této verze. Starší souhrnné počty nelze zpětně bezpečně rozdělit.</p>}
      </section>

      <details className="mt-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3.5"><summary className="flex cursor-pointer list-none items-center gap-2 text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]"><Info aria-hidden size={14} /> Technický základ a rollback</summary><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><PowerOutageDetailRow label="Katalog RÚIAN" value={`${state.representativeDone.toLocaleString('cs-CZ')} / ${state.catalogTotal.toLocaleString('cs-CZ')}`} /><PowerOutageDetailRow label="Mapování obcí" value={`${state.mappingDone.toLocaleString('cs-CZ')} / ${state.catalogTotal.toLocaleString('cs-CZ')}`} /><PowerOutageDetailRow label="Obce v území ČEZ" value={state.cezMapped.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Zdrojová normalizace" value={`${state.normalizationDone.toLocaleString('cs-CZ')} / ${state.normalizationTotal.toLocaleString('cs-CZ')}`} /><PowerOutageDetailRow label="Odstávky v projekci" value={state.projectedOutageCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Adresy v projekci" value={state.projectedAddressCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Bezpečný cyklus" value={`${Math.min(1, state.safeRecentCycleCount)} / 1`} /><PowerOutageDetailRow label="Projekce posledního cyklu" value={state.latestProjectionMatches ? 'Shodná' : 'Čeká na shodu'} /><PowerOutageDetailRow label="Publikovaný cyklus" value={publicationCycleMatches ? 'Shodný' : 'Čeká na shodu'} /><PowerOutageDetailRow label="Rollback zdroj" value="Původní ČEZ je zachovaný" /></div><p className="mt-2 text-[8px] leading-4 text-[var(--text-secondary)]">Interní databázové označení aktivního celoplošného zdroje zůstává kvůli bezpečnému rollbacku beze změny. Neovlivňuje označení ČEZ ALL v1 v aplikaci.</p></details>

      {aggregate.pipelineDelayed ? <section className="mt-4"><h3 className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.12em] text-amber-700 [html[data-theme=dark]_&]:text-amber-300"><CircleAlert aria-hidden size={14} /> Zpoždění celoplošného sběru</h3><p className="mt-2 rounded-2xl border border-amber-400/30 bg-amber-500/8 p-3.5 text-xs leading-5 text-[var(--text-primary)]">{aggregate.message}</p><RecoveryControl isAdmin={isAdmin} blocked={pipelineRecoveryBlocked} label={`Obnovit · ${cezNewStageLabel(pipelineRecoveryStage, shadowActive)}`} request={{ target: 'cez_new_pipeline', stage: pipelineRecoveryStage }} onRecovered={onReload} /></section> : null}

      {state.lastErrorMessage || state.errorStage || hasWarning ? <section className="mt-4"><h3 className={`flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.12em] ${state.errorStage ? 'text-red-700 [html[data-theme=dark]_&]:text-red-300' : 'text-amber-700 [html[data-theme=dark]_&]:text-amber-300'}`}><CircleAlert aria-hidden size={14} /> Co vyžaduje pozornost</h3><p className={`mt-2 rounded-2xl border p-3.5 text-xs leading-5 text-[var(--text-primary)] ${state.errorStage ? 'border-red-400/30 bg-red-500/8' : 'border-amber-400/30 bg-amber-500/8'}`}>{state.errorStage ? (state.errorCode ? `${state.errorCode}: ` : '') : (state.warningCode ? `${state.warningCode}: ` : '')}{state.errorStage ? (state.lastErrorMessage ?? 'Tato fáze vyžaduje opravu.') : (state.warningMessage ?? `${state.scanRetryableErrorCount} obcí se automaticky zopakuje po dokončení aktuálního snapshotu.`)}</p>{state.scanNextRetryAt ? <p className="mt-2 text-[9px] text-[var(--text-secondary)]">Nejbližší naplánované opakování: {formatDate(state.scanNextRetryAt)}</p> : null}{state.errorStage || state.warningStage ? <RecoveryControl isAdmin={isAdmin} blocked={state.errorStage ? pipelineRecoveryBlocked : false} label={state.warningStage === 'scan' && !state.errorStage ? 'Zařadit chybné obce k opakování' : `Obnovit · ${cezNewStageLabel(state.errorStage ?? state.warningStage!, shadowActive)}`} request={{ target: 'cez_new_pipeline', stage: state.errorStage ?? state.warningStage! }} onRecovered={onReload} /> : null}</section> : null}
    </div>
  </PowerOutagePopupShell>
}
...
function SourcesPanel({ workspace, refreshWarning, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; refreshWarning?: string | null; onRetry?: () => void }) {
  ...
  if (workspace.loadErrors.sources) return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10 text-red-500"><CircleAlert aria-hidden size={18} /></span><span><small className="block text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Kompletní sběr</small><h3 className="text-base font-semibold text-[var(--text-primary)]">Stav distributorů</h3></span></div><PanelLoadError message={workspace.loadErrors.sources} onRetry={onRetry} /></PanelShell>
  ...
  return <PanelShell>
      ...
      {sourceCards.map((card) => {
        if (card.kind === 'cez-new') {
          ...
          return <div key="cez-new" className={`weather-alerts__record-surface relative h-full rounded-xl border px-2.5 py-1 lg:px-3 lg:py-2 ${compact ? 'lg:h-[90px]' : 'lg:h-[94px]'}`}>
            <div className="flex items-center justify-between gap-2"><strong className="text-[13px] text-[var(--text-primary)]">{shadowActive ? <CezAllV1Label compact /> : 'ČEZ – NEW'}</strong><CompletePanelStatusBadge label={badgeLabel} badgeClassName={presentation.badge} dotClassName={presentation.dot} attention={presentation.attention || hasWarning} title={workspace.currentUser.isAdmin ? badgeMessage : undefined} ariaLabel={`${shadowActive ? 'ČEZ ALL v1' : 'ČEZ – NEW'}: ${badgeLabel}. ${badgeMessage}`} onClick={workspace.currentUser.isAdmin ? () => setShowCezNew(true) : undefined} /></div>
        ...
        return <div key={source.source} className={`weather-alerts__record-surface relative h-full rounded-xl border px-2.5 py-1 lg:px-3 lg:py-2 ${compact ? 'lg:h-[90px]' : 'lg:h-[94px]'}`}>
        <div className="flex items-center justify-between gap-2">
          <strong className="text-[13px] text-[var(--text-primary)]">{sourceLabel(source.source)}</strong>
          <CompletePanelStatusBadge label={discoveryStatusLabel(discovery.status)} badgeClassName={presentation.badge} dotClassName={presentation.dot} attention={presentation.attention || discovery.errorTargetCount > 0} title={workspace.currentUser.isAdmin ? discovery.statusMessage : undefined} ariaLabel={`${sourceLabel(source.source)}: ${discoveryStatusLabel(discovery.status)}. ${discovery.statusMessage}`} onClick={workspace.currentUser.isAdmin ? () => openSource(source.source) : undefined} />
        </div>
        <div className={`${compact ? 'mt-0.5' : 'mt-1'} grid grid-cols-2 gap-2`}>
          <span className="weather-alerts__record-surface flex h-6 items-center justify-between rounded-lg border px-2.5"><small className="text-[7px] font-bold uppercase leading-none tracking-[0.035em] text-[var(--text-secondary)]">Prověřeno</small><strong className="text-[11px] leading-none tabular-nums text-[var(--text-primary)]">{discovery.completedTargetCount}/{discovery.totalTargetCount}</strong></span>
          <span className="weather-alerts__record-surface flex h-6 items-center justify-between rounded-lg border px-2.5"><small className="text-[7px] font-bold uppercase leading-none tracking-[0.035em] text-[var(--text-secondary)]">Zbývá</small><strong className="text-[11px] leading-none tabular-nums text-[var(--text-primary)]">{discovery.remainingTargetCount}</strong></span>
        </div>
    ...
    {workspace.currentUser.isAdmin && selectedSource && typeof document !== 'undefined' ? createPortal(<CompleteSourceDiagnosticPopup source={selectedSource} diagnostic={diagnostic} loading={loading} error={error} isAdmin onReload={() => loadDiagnostic(selectedSource, false)} onClose={() => { setSelectedSource(null); setDiagnostic(null); setError(null) }} />, document.body) : null}
    {workspace.currentUser.isAdmin && showCezNew && cezNew && typeof document !== 'undefined' ? createPortal(<CompleteCezNewDiagnosticPopup state={cezNew} source={activeCezSource} isAdmin onReload={() => onRetry?.()} onClose={() => setShowCezNew(false)} />, document.body) : null}
  </PanelShell>
}
...
function CompleteProviderDiagnosticPopup({ provider, diagnostic, loading, error, refreshWarning, isAdmin, onReload, onClose }: {
  ...
  const targetProgressPercent = diagnostic
    ? boundedProgressPercent(diagnostic.state.processedTargetCount, diagnostic.state.totalTargetCount, 100)
    : 0
  const evaluationProgressPercent = diagnostic
    ? boundedProgressPercent(diagnostic.state.evaluatedCandidateCount, diagnostic.state.candidateCount, 100)
    : 0
  ...
  const diagnosticWarnings = diagnostic ? Object.values(diagnostic.loadWarnings).filter(Boolean) as string[] : []
  ...
  return <PowerOutagePopupShell titleId={`complete-provider-${provider}`} eyebrow="KOMPLETNÍ SBĚR · DOHLEDÁVÁNÍ FIREM" title={providerLabel(provider)} icon={<SearchCheck aria-hidden size={21} />} onClose={onClose}>
    {loading ? <div className="flex min-h-[320px] flex-1 flex-col items-center justify-center p-6 text-center"><span className="h-8 w-8 animate-spin rounded-full border-2 border-[var(--surface-border)] border-t-[var(--accent)]" /><strong className="mt-4 text-sm text-[var(--text-primary)]">Načítám provozní stav…</strong></div> : error || !diagnostic ? <div className="flex min-h-[300px] flex-1 flex-col items-center justify-center p-6 text-center"><CircleAlert aria-hidden size={28} className="text-red-500" /><strong className="mt-3 text-sm text-[var(--text-primary)]">Detail poskytovatele se nepodařilo načíst</strong><p className="mt-1 max-w-md text-xs leading-5 text-[var(--text-secondary)]">{error ?? 'Data nejsou dostupná.'}</p></div> : <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
      {refreshWarning || diagnosticWarnings.length > 0 ? <div className="mb-3 flex items-start gap-2 rounded-2xl border border-amber-400/30 bg-amber-500/8 px-3.5 py-3 text-[9px] leading-4 text-amber-800 [html[data-theme=dark]_&]:text-amber-200"><CircleAlert aria-hidden size={14} className="mt-0.5 shrink-0" /><span><strong className="block text-[10px]">Část detailu se nyní nepodařilo obnovit</strong><span className="mt-0.5 block">Zobrazujeme poslední dostupný provozní stav. {refreshWarning ?? diagnosticWarnings[0]}</span></span></div> : null}
      <div className="flex items-center justify-between gap-3 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3.5"><span><small className="block text-[8px] font-bold uppercase tracking-[0.09em] text-[var(--text-secondary)]">Aktuální stav poskytovatele</small><strong className="mt-1 block text-sm text-[var(--text-primary)]">{diagnostic.state.pendingCandidateCount > 0 ? 'VYHODNOCENÍ' : providerStatusLabel(diagnostic.state.status)}</strong><span className="mt-1 block max-w-lg text-[9px] leading-4 text-[var(--text-secondary)]">{diagnostic.state.statusMessage}</span></span><span className={`relative inline-flex h-8 w-[104px] shrink-0 items-center justify-center gap-1 rounded-xl border px-2 text-center text-[8px] font-bold leading-none tracking-[0.07em] ${presentation.badge}`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${presentation.dot}`} /><span className="translate-y-px">{diagnostic.state.pendingCandidateCount > 0 ? 'VYHODNOCENÍ' : providerStatusLabel(diagnostic.state.status)}</span></span></div>

      <section className={`mt-3 rounded-2xl border p-3.5 ${diagnostic.state.status === 'processing' ? 'border-sky-400/25 bg-sky-500/8' : 'border-[var(--surface-border)] bg-[var(--surface-muted)]'}`}>
        <div className="flex items-center gap-2"><Route aria-hidden size={14} className="text-[var(--text-secondary)]" /><h3 className="text-[9px] font-bold uppercase tracking-[0.11em] text-[var(--text-secondary)]">Aktuální fronty</h3></div>
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] p-3">
            <div className="flex items-start justify-between gap-3"><span className="flex min-w-0 items-center gap-2">{diagnostic.state.remainingTargetCount > 0 || providerBatchRunning ? <LoaderCircle aria-hidden size={16} className="shrink-0 animate-spin text-sky-500" /> : <CircleCheck aria-hidden size={16} className="shrink-0 text-emerald-500" />}<span><small className="block text-[8px] font-bold uppercase tracking-[0.07em] text-[var(--text-secondary)]">Adresní dotazy {providerLabel(provider)}</small><strong className="mt-1 block text-[10px] text-[var(--text-primary)]">{diagnostic.state.remainingTargetCount > 0 ? `${diagnostic.state.remainingTargetCount.toLocaleString('cs-CZ')} dotazů čeká na poskytovatele` : 'Všechny aktuální adresní dotazy jsou hotové'}</strong></span></span><strong className="shrink-0 text-base tabular-nums text-[var(--accent)]">{targetProgressPercent.toLocaleString('cs-CZ')} %</strong></div>
            <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-sky-950/10 [html[data-theme=dark]_&]:bg-white/10"><div className="h-full rounded-full bg-sky-500 transition-[width] duration-700" style={{ width: `${targetProgressPercent}%` }} /></div>
            <p className="mt-2 text-[8px] text-[var(--text-secondary)]">{diagnostic.state.processedTargetCount.toLocaleString('cs-CZ')} / {diagnostic.state.totalTargetCount.toLocaleString('cs-CZ')} adresních cílů</p>
          </div>
          <div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] p-3">
            <div className="flex items-start justify-between gap-3"><span className="flex min-w-0 items-center gap-2">{diagnostic.state.pendingCandidateCount > 0 ? <LoaderCircle aria-hidden size={16} className="shrink-0 animate-spin text-violet-500" /> : <CircleCheck aria-hidden size={16} className="shrink-0 text-emerald-500" />}<span><small className="block text-[8px] font-bold uppercase tracking-[0.07em] text-[var(--text-secondary)]">Interní vyhodnocení kandidátů</small><strong className="mt-1 block text-[10px] text-[var(--text-primary)]">{diagnostic.state.pendingCandidateCount > 0 ? `${diagnostic.state.pendingCandidateCount.toLocaleString('cs-CZ')} kandidátů čeká na lokální vyhodnocení` : 'Všichni aktuální kandidáti jsou vyhodnoceni'}</strong></span></span><strong className="shrink-0 text-base tabular-nums text-violet-600 [html[data-theme=dark]_&]:text-violet-300">{evaluationProgressPercent.toLocaleString('cs-CZ')} %</strong></div>
            <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-violet-950/10 [html[data-theme=dark]_&]:bg-white/10"><div className="h-full rounded-full bg-violet-500 transition-[width] duration-700" style={{ width: `${evaluationProgressPercent}%` }} /></div>
            <p className="mt-2 text-[8px] text-[var(--text-secondary)]">{diagnostic.state.evaluatedCandidateCount.toLocaleString('cs-CZ')} / {diagnostic.state.candidateCount.toLocaleString('cs-CZ')} kandidátů · bez externích požadavků</p>
            {provider === 'mapy' && diagnostic.evaluationBySource.length > 0 ? <div className="mt-2 grid grid-cols-3 gap-1">{diagnostic.evaluationBySource.map((item) => <span key={item.source} className="rounded-lg border border-[var(--surface-border)] bg-[var(--surface-muted)] px-1.5 py-1 text-center"><small className="block text-[7px] font-bold uppercase text-[var(--text-secondary)]">{sourceLabel(item.source)}</small><strong className="block text-[8px] tabular-nums text-[var(--text-primary)]">{item.evaluatedCandidateCount.toLocaleString('cs-CZ')}/{item.candidateCount.toLocaleString('cs-CZ')}</strong></span>)}</div> : null}
          </div>
        </div>
        <p className="mt-2 text-[8px] leading-4 text-[var(--text-secondary)]">Nové cíle z ČEZ, EG.D a PRE mohou oba rozsahy průběžně zvýšit. Základní stav se při zpracování obnovuje každých 20 sekund; seznam chyb pouze při otevření nebo ruční obnově.</p>
      </section>
      ...
      <details className="mt-5 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3.5"><summary className="flex cursor-pointer list-none items-center gap-2 text-[10px] font-bold uppercase tracking-[0.13em] text-[var(--text-secondary)]"><Info aria-hidden size={14} /> Jak číst statistiky</summary><div className="mt-3 space-y-2 text-[10px] leading-5 text-[var(--text-primary)]"><p><strong>S výsledkem</strong> jsou adresní dotazy s použitelnou odpovědí poskytovatele, nikoli přímo počet jedinečných firem.</p><p><strong>Bez výsledku</strong> znamená úspěšnou odpověď bez použitelného subjektu.</p><p><strong>Opakuje se</strong> jsou dočasné chyby čekající na automatický retry. <strong>Ke kontrole</strong> vyžaduje rozhodnutí administrátora.</p><p><strong>Prověřeno</strong> zahrnuje každý cíl, u kterého už proběhl pokus, včetně chyb a auditně přeskočených dotazů. Nové cíle ČEZ, EG.D a PRE průběžně mění celkový rozsah fronty.</p></div></details>
    </div>}
  </PowerOutagePopupShell>
}

function ProviderCardProgress({ provider, capacityText }: {
  provider: CompleteProviderState
  capacityText: string
}) {
  const targetPercent = boundedProgressPercent(provider.processedTargetCount, provider.totalTargetCount, 100)
  const evaluationPercent = boundedProgressPercent(provider.evaluatedCandidateCount, provider.candidateCount, 100)
  if (provider.provider === 'ares' || provider.provider === 'mapy') {
    const providerProgressLabel = provider.provider === 'ares' ? 'ARES DOTAZY' : 'MAPY.COM DOTAZY'
    return <div className="mt-1 space-y-1">
      <div><div className="flex items-center justify-between gap-2 text-[6px] font-semibold text-[var(--text-secondary)]"><span>{providerProgressLabel} <strong className="tabular-nums text-[var(--text-primary)]">{provider.processedTargetCount.toLocaleString('cs-CZ')}/{provider.totalTargetCount.toLocaleString('cs-CZ')}</strong></span><strong className="tabular-nums text-[var(--accent)]">{targetPercent.toLocaleString('cs-CZ')} %</strong></div><div className="mt-0.5 h-1 overflow-hidden rounded-full bg-[var(--surface-border)]"><i className="block h-full rounded-full bg-sky-500 transition-[width] duration-700" style={{ width: `${targetPercent}%` }} /></div></div>
      <div><div className="flex items-center justify-between gap-2 text-[6px] font-semibold text-[var(--text-secondary)]"><span>INTERNÍ VYHODNOCENÍ <strong className="tabular-nums text-[var(--text-primary)]">{provider.evaluatedCandidateCount.toLocaleString('cs-CZ')}/{provider.candidateCount.toLocaleString('cs-CZ')}</strong></span><strong className="tabular-nums text-violet-600 [html[data-theme=dark]_&]:text-violet-300">{evaluationPercent.toLocaleString('cs-CZ')} %</strong></div><div className="mt-0.5 h-1 overflow-hidden rounded-full bg-[var(--surface-border)]"><i className="block h-full rounded-full bg-violet-500 transition-[width] duration-700" style={{ width: `${evaluationPercent}%` }} /></div></div>
    </div>
  }
  ...
  const progressPercent = totalWorkCount > 0 ? boundedProgressPercent(completedWorkCount, totalWorkCount) : null
  const progressCountText = provider.pendingCandidateCount > 0
    ? `${completedWorkCount.toLocaleString('cs-CZ')}/${totalWorkCount.toLocaleString('cs-CZ')}`
    : `${provider.processedTargetCount.toLocaleString('cs-CZ')}/${provider.totalTargetCount.toLocaleString('cs-CZ')}`
  return <><div className="mt-1"><div className="flex items-center justify-between gap-2 text-[6px] font-semibold text-[var(--text-secondary)]"><span>{provider.pendingCandidateCount > 0 ? 'VYHODNOCENÍ' : 'AKTUÁLNÍ FRONTA'} <strong className="tabular-nums text-[var(--text-primary)]">{progressCountText}</strong></span><strong className="tabular-nums text-[var(--accent)]">{progressPercent == null ? '—' : `${progressPercent.toLocaleString('cs-CZ')} %`}</strong></div><div className="mt-0.5 h-1.5 overflow-hidden rounded-full bg-[var(--surface-border)]"><i className="block h-full rounded-full bg-sky-500 transition-[width] duration-700" style={{ width: `${progressPercent ?? 0}%` }} /></div></div><p className="mt-1 hidden truncate text-center text-[7px] leading-3 text-[var(--text-secondary)] lg:block">{provider.pendingCandidateCount > 0 ? `${provider.pendingCandidateCount.toLocaleString('cs-CZ')} kandidátů čeká na interní vyhodnocení` : capacityText}</p></>
}

function DiscoveryPanel({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  ...
  const diagnosticRef = useRef<CompleteProviderDiagnostic | null>(null)
  ...
  const loadDiagnostic = async (
    ...
    if (showLoading) setLoading(true)
    if (!diagnosticRef.current) setError(null)
    try {
      const result = await getCompletePowerOutageProviderDiagnosticAction(provider, undefined, includeErrorDetails)
      if (result.success) {
        const nextDiagnostic = includeErrorDetails || !diagnosticRef.current
          ? result.diagnostic
          : {
              ...result.diagnostic,
              loadWarnings: {
                ...result.diagnostic.loadWarnings,
                recentErrors: diagnosticRef.current.loadWarnings.recentErrors,
              },
              recentErrors: diagnosticRef.current.recentErrors,
              enrichment: result.diagnostic.enrichment && diagnosticRef.current.enrichment
                ? { ...result.diagnostic.enrichment, recentErrors: diagnosticRef.current.enrichment.recentErrors }
                : result.diagnostic.enrichment,
            }
        diagnosticRef.current = nextDiagnostic
        setDiagnostic(nextDiagnostic)
        setError(null)
        setRefreshWarning(null)
      } else if (diagnosticRef.current) {
        setRefreshWarning(result.error)
      } else {
        setError(result.error)
      }
    } catch (loadError) {
      const message = loadError instanceof Error ? loadError.message : 'Obnovení detailu se nezdařilo.'
      if (diagnosticRef.current) setRefreshWarning(message)
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => { void loadDiagnostic(selectedProvider, false, false) }, 20_000)
    return () => window.clearInterval(interval)
  ...
  const attentionSource = workspace.sources.find((source) => (
    ['error', 'delayed'].includes(source.discovery.status)
    && !['error', 'delayed', 'partial'].includes(source.upstreamStatus)
    && (source.discovery.remainingTargetCount > 0 || source.discovery.pendingCandidateCount > 0)
  ))
  const searchProcessing = workspace.providers.some((provider) => provider.status === 'processing')
    || workspace.sources.some((source) => (
      source.discovery.status === 'processing'
      && !['error', 'delayed', 'partial'].includes(source.upstreamStatus)
    ))
  ...
  const searchStatus = attentionProvider || attentionSource?.discovery.status === 'error'
    ...
    : searchProcessing
      ...
      : searchWaiting
        ? 'waiting' as const
        : 'healthy' as const
  const RuntimeIcon = searchStatus === 'healthy'
    ? CircleCheck
    : searchStatus === 'processing'
      ? LoaderCircle
      : CircleAlert
  const conciseAttention = attentionProvider
    ? `${providerLabel(attentionProvider.provider)}: ${attentionProvider.statusMessage}`
    : attentionSource
      ? `${sourceLabel(attentionSource.source)}: ${attentionSource.discovery.statusMessage}`
      : 'provozní chyba vyhledávání'
  const runtimeText = searchStatus === 'healthy'
  ...
  return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-500/10 text-violet-600 [html[data-theme=dark]_&]:text-violet-300"><SearchCheck aria-hidden size={18} /></span><span><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">ARES · Mapy.com · Google</small><h3 className="text-base font-semibold text-[var(--text-primary)]">Vyhledávání firem</h3></span></div><div className="mt-3 grid min-h-0 flex-1 grid-rows-3 gap-2 lg:mt-4 lg:block lg:space-y-2.5">{workspace.providers.map((provider) => {
    ...
    const capacityText = provider.configured
      ? provider.provider === 'mapy'
        ? `Zbývá ${provider.monthlyCreditRemaining.toLocaleString('cs-CZ')} z měsíčního stropu ${provider.monthlyCreditSafetyCap.toLocaleString('cs-CZ')} kreditů`
    ...
    return <div key={provider.provider} className={`weather-alerts__record-surface h-full rounded-xl border px-2.5 py-1 lg:h-[126px] lg:px-3 lg:py-2 ${provider.configured ? '' : 'opacity-65'}`}>
      <div className="flex items-center justify-between gap-2"><strong className="text-[12px] uppercase text-[var(--text-primary)]">{providerLabel(provider.provider)}</strong><CompletePanelStatusBadge label={provider.pendingCandidateCount > 0 && provider.remainingTargetCount === 0 ? 'VYHODNOCENÍ' : providerStatusLabel(provider.status)} badgeClassName={presentation.badge} dotClassName={presentation.dot} attention={presentation.attention} ariaLabel={`${providerLabel(provider.provider)}: ${provider.pendingCandidateCount > 0 ? 'VYHODNOCENÍ' : providerStatusLabel(provider.status)}.`} title={workspace.currentUser.isAdmin ? 'Zobrazit provozní detail' : undefined} onClick={workspace.currentUser.isAdmin ? () => openProvider(provider.provider) : undefined} /></div>
      <div className="mt-0.5 grid grid-cols-4 gap-1 text-center lg:mt-1 lg:gap-1.5">
        <span className="weather-alerts__record-surface flex h-8 flex-col items-center justify-center rounded-lg border text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300"><small className="whitespace-nowrap text-[6px] font-bold uppercase leading-[8px]">S výsledkem</small><strong className="text-sm leading-4 tabular-nums text-[var(--text-primary)]">{provider.readyCount.toLocaleString('cs-CZ')}</strong></span>
        <span className="weather-alerts__record-surface flex h-8 flex-col items-center justify-center rounded-lg border text-amber-700 [html[data-theme=dark]_&]:text-amber-300"><small className="whitespace-nowrap text-[6px] font-bold uppercase leading-[8px]">Bez výsledku</small><strong className="text-sm leading-4 tabular-nums text-[var(--text-primary)]">{provider.notFoundCount.toLocaleString('cs-CZ')}</strong></span>
        <span className="weather-alerts__record-surface flex h-8 flex-col items-center justify-center rounded-lg border text-sky-700 [html[data-theme=dark]_&]:text-sky-300"><small className="whitespace-nowrap text-[6px] font-bold uppercase leading-[8px]">Opakuje se</small><strong className="text-sm leading-4 tabular-nums text-[var(--text-primary)]">{provider.retryableErrorCount.toLocaleString('cs-CZ')}</strong></span>
        <span className="weather-alerts__record-surface flex h-8 flex-col items-center justify-center rounded-lg border text-red-700 [html[data-theme=dark]_&]:text-red-300"><small className="whitespace-nowrap text-[6px] font-bold uppercase leading-[8px]">Ke kontrole</small><strong className="text-sm leading-4 tabular-nums text-[var(--text-primary)]">{provider.reviewErrorCount.toLocaleString('cs-CZ')}</strong></span>
      </div>
      <ProviderCardProgress provider={provider} capacityText={capacityText} />
    </div>
  })}</div><div className={`mt-auto flex items-center justify-center gap-1.5 pt-2 text-center text-[9px] lg:pt-3 ${runtimeTone}`} title={runtimeHasAttention ? conciseAttention : runtimeText}><RuntimeIcon aria-hidden size={12} className={searchStatus === 'processing' ? 'animate-spin' : ''} /><span className="truncate">{runtimeText}</span></div><p className="mt-0.5 flex items-center justify-center gap-1.5 text-center text-[8px] text-[var(--text-secondary)]"><TimerReset aria-hidden size={11} /><span>Poslední aktivita: {formatDate(runtime.lastActivityAt)}</span></p>{workspace.currentUser.isAdmin && selectedProvider && typeof document !== 'undefined' ? createPortal(<CompleteProviderDiagnosticPopup provider={selectedProvider} diagnostic={diagnostic} loading={loading} error={error} refreshWarning={refreshWarning} isAdmin onReload={() => loadDiagnostic(selectedProvider, false, true)} onClose={() => { setSelectedProvider(null); setDiagnostic(null); diagnosticRef.current = null; setError(null); setRefreshWarning(null) }} />, document.body) : null}</PanelShell>
}
...
function AddressCoverageDiagnosticPopup({ diagnostic, loading, error, isAdmin, onReload, onClose }: {
  ...
  const percent = coverage ? addressCoveragePercent(coverage) : 0
  ...
  const normalizationStalled = diagnostic?.task?.status === 'running'
    && Boolean(diagnostic.task.lockExpiresAt)
    && new Date(diagnostic.task.lockExpiresAt!).getTime() <= new Date(diagnostic.observedAt).getTime()
  const normalizationRecoverable = diagnostic?.task?.status === 'failed'
    || diagnostic?.task?.status === 'partial'
    || normalizationStalled
  return <PowerOutagePopupShell titleId="complete-address-coverage" eyebrow="KOMPLETNÍ SBĚR · DATABÁZE ADRES" title="Detail pokrytí adres" icon={<MapPinned aria-hidden size={21} />} onClose={onClose}>
    {loading ? <div className="flex min-h-[320px] flex-1 flex-col items-center justify-center p-6 text-center"><span className="h-8 w-8 animate-spin rounded-full border-2 border-[var(--surface-border)] border-t-[var(--accent)]" /><strong className="mt-4 text-sm text-[var(--text-primary)]">Načítám pokrytí adres…</strong></div> : error || !diagnostic || !coverage ? <div className="flex min-h-[300px] flex-1 flex-col items-center justify-center p-6 text-center"><CircleAlert aria-hidden size={28} className="text-red-500" /><strong className="mt-3 text-sm text-[var(--text-primary)]">Detail pokrytí se nepodařilo načíst</strong><p className="mt-1 max-w-md text-xs leading-5 text-[var(--text-secondary)]">{error ?? 'Data nejsou dostupná.'}</p></div> : <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
      <section className="rounded-2xl border border-sky-500/25 bg-sky-500/8 p-4"><div className="flex items-end justify-between gap-3"><span><small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-sky-700 [html[data-theme=dark]_&]:text-sky-300">Připraveno pro vyhledávání firem</small><strong className="mt-1 block text-base text-[var(--text-primary)]">{coverage.searchableCount} z {coverage.progressTotalCount} relevantních adres</strong></span><span className="flex flex-col items-end gap-2"><strong className="text-3xl font-semibold tabular-nums text-[var(--accent)]">{percent.toLocaleString('cs-CZ')} %</strong><span className={`relative inline-flex h-7 w-[92px] items-center justify-center gap-1 rounded-xl border px-2 text-[7px] font-bold leading-none tracking-[0.06em] ${presentation.badge}`}><i className={`h-1.5 w-1.5 rounded-full ${presentation.dot}`} /><span>{addressCoverageStatusLabel(coverage.status)}</span>{presentation.attention ? <i className="absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-amber-400 text-[8px] font-black not-italic text-amber-950">!</i> : null}</span></span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-sky-950/10 [html[data-theme=dark]_&]:bg-white/10"><div className="h-full rounded-full bg-sky-500 transition-[width] duration-700" style={{ width: `${Math.min(100, percent)}%` }} /></div><p className="mt-2 text-[9px] leading-4 text-[var(--text-secondary)]">{coverage.statusMessage}</p><p className="mt-1 text-[8px] text-[var(--text-secondary)]">Technicky normalizováno {coverage.normalizedCount} z {coverage.totalCount} adres · pouze obec mimo progress {coverage.municipalityCount} · aktualizováno {formatDate(coverage.refreshedAt)}</p></section>

      {coverage.status === 'processing' ? <section className="mt-3 rounded-2xl border border-sky-400/25 bg-sky-500/8 p-3.5"><div className="flex items-center gap-3"><LoaderCircle aria-hidden size={18} className="animate-spin text-sky-500" /><span><small className="block text-[8px] font-bold uppercase tracking-[0.08em] text-[var(--text-secondary)]">Interní normalizační fronta</small><strong className="mt-1 block text-[10px] text-[var(--text-primary)]">Čeká {coverage.pendingCount.toLocaleString('cs-CZ')} adres · dávka {diagnostic.normalizerBatchSize.toLocaleString('cs-CZ')} každou {diagnostic.normalizerCadenceMinutes === 1 ? 'minutu' : `${diagnostic.normalizerCadenceMinutes} minuty`}</strong></span></div><p className="mt-2 text-[9px] leading-4 text-[var(--text-secondary)]">Odhad dokončení při současné rychlosti: přibližně {diagnostic.estimatedRemainingMinutes.toLocaleString('cs-CZ')} min. Jde pouze o interní přípravu adres; žádný externí server tato fáze nevolá.</p><p className="mt-1 text-[8px] leading-4 text-[var(--text-secondary)]">Údaj se obnovuje každých 20 sekund. Výsledek právě běžící dávky se projeví po jejím dokončení.</p></section> : null}

      <section className="mt-4"><h3 className="text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Kvalita zpracovaných adres</h3><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-5">{[
        ['Přesné', coverage.exactCount, 'Ulice a konkrétní číslo domu.', 'border-emerald-400/30 bg-emerald-400/8'],
        ['Pouze ulice', coverage.streetCount, 'Použitelné pro Mapy.com.', 'border-amber-400/30 bg-amber-400/8'],
        ['Mimo hledání', coverage.municipalityCount, 'Pouze obec; neovlivňuje progress.', 'border-slate-400/30 bg-slate-400/8'],
      ...
      {coverage.errorCount + coverage.reviewCount + coverage.unresolvedCount > 0 ? <section className="mt-4 rounded-2xl border border-amber-400/30 bg-amber-500/8 p-3.5"><h3 className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.12em] text-amber-700 [html[data-theme=dark]_&]:text-amber-300"><CircleAlert aria-hidden size={14} /> Co vyžaduje pozornost</h3><p className="mt-2 text-[9px] leading-4 text-[var(--text-primary)]">Nerozpoznané adresy {coverage.unresolvedCount} · ke kontrole {coverage.reviewCount} · technické chyby {coverage.errorCount}.</p>{!normalizationRecoverable ? <p className="mt-2 text-[8px] leading-4 text-[var(--text-secondary)]">Nerozpoznané adresy vyžadují opravu zdrojových údajů; samotné opakování by vytvořilo stejný výsledek.</p> : null}</section> : null}
      ...
      <section className="mt-4"><h3 className="text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Pokrytí podle distributora</h3><div className="mt-2 grid gap-2 sm:grid-cols-3">{diagnostic.sources.map((source) => { const sourcePercent = source.progressTotalCount > 0 ? Math.round((source.searchableCount / source.progressTotalCount) * 1000) / 10 : 100; return <div key={source.source} className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3"><div className="flex items-center justify-between"><strong className="text-xs text-[var(--text-primary)]">{sourceLabel(source.source)}</strong><strong className="text-sm tabular-nums text-[var(--accent)]">{sourcePercent.toLocaleString('cs-CZ')} %</strong></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--surface-strong)]"><span className="block h-full rounded-full bg-sky-500" style={{ width: `${Math.min(100, sourcePercent)}%` }} /></div><div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-[8px] text-[var(--text-secondary)]"><span>Připraveno <strong className="text-[var(--text-primary)]">{source.searchableCount}/{source.progressTotalCount}</strong></span><span>Přesné <strong className="text-[var(--text-primary)]">{source.exactCount}</strong></span><span>Ulice <strong className="text-[var(--text-primary)]">{source.streetCount}</strong></span><span>Mimo hledání <strong className="text-[var(--text-primary)]">{source.municipalityCount}</strong></span><span>Čeká <strong className="text-[var(--text-primary)]">{source.pendingCount}</strong></span><span className={source.attentionCount > 0 ? 'text-amber-700 [html[data-theme=dark]_&]:text-amber-300' : ''}>Pozornost <strong className="text-[var(--text-primary)]">{source.attentionCount}</strong></span></div></div> })}</div></section>

      <section className="mt-4"><h3 className="text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Provoz normalizátoru</h3><div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><PowerOutageDetailRow label="Verze normalizátoru" value={String(diagnostic.normalizerVersion)} /><PowerOutageDetailRow label="Stav úlohy" value={normalizationStalled ? 'Zaseknutý běh' : providerTaskStatusLabel(diagnostic.task?.status ?? null)} /><PowerOutageDetailRow label="Velikost dávky" value={`${diagnostic.normalizerBatchSize.toLocaleString('cs-CZ')} adres`} /><PowerOutageDetailRow label="Interval" value={diagnostic.normalizerCadenceMinutes === 1 ? 'Každou minutu' : `Každé ${diagnostic.normalizerCadenceMinutes} minuty`} /><PowerOutageDetailRow label="Poslední start" value={formatDate(diagnostic.task?.lastStartedAt ?? null)} /><PowerOutageDetailRow label="Poslední dokončení" value={formatDate(diagnostic.task?.lastFinishedAt ?? null)} /><PowerOutageDetailRow label="Poslední úspěch" value={formatDate(diagnostic.task?.lastSuccessAt ?? null)} /><PowerOutageDetailRow label="Selhání v řadě" value={String(diagnostic.task?.consecutiveFailureCount ?? 0)} /></div>{diagnostic.task?.lastErrorMessage ? <p className="mt-2 rounded-2xl border border-red-400/30 bg-red-500/8 p-3.5 text-xs leading-5 text-[var(--text-primary)]">{diagnostic.task.lastErrorCode ? `${diagnostic.task.lastErrorCode}: ` : ''}{diagnostic.task.lastErrorMessage}</p> : null}{normalizationRecoverable ? <RecoveryControl isAdmin={isAdmin} blocked={normalizationRecoveryBlocked} label="Obnovit normalizaci" request={{ target: 'address_normalization' }} onRecovered={onReload} /> : null}</section>
...
function CoveragePanel({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  ...
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const percent = addressCoveragePercent(coverage)
  const presentation = addressCoverageStatusPresentation(coverage.status)
  const attentionCount = coverage.attentionCount
  const isAdmin = workspace.currentUser.isAdmin
  ...
  if (workspace.loadErrors.addressCoverage) return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10 text-red-500"><CircleAlert aria-hidden size={18} /></span><span><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Databáze adres</small><h3 className="text-base font-semibold text-[var(--text-primary)]">Pokrytí adres</h3></span></div><PanelLoadError message={workspace.loadErrors.addressCoverage} onRetry={onRetry} /></PanelShell>

  return <PanelShell>
    <div className="flex items-start justify-between gap-3">
      <div className="flex min-w-0 items-center gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-600 [html[data-theme=dark]_&]:text-emerald-300"><MapPinned aria-hidden size={18} /></span>
        <span className="min-w-0"><small className="block whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Aktuální odstávky</small><h3 className="whitespace-nowrap text-base font-semibold text-[var(--text-primary)]">Pokrytí adres</h3></span>
      </div>
      <CompletePanelStatusBadge label={addressCoverageStatusLabel(coverage.status)} badgeClassName={presentation.badge} dotClassName={presentation.dot} attention={presentation.attention} />
    </div>
    {isAdmin ? <button type="button" onClick={openDetail} className="mt-4 rounded-2xl border border-sky-400/25 bg-sky-500/8 p-3.5 text-left transition hover:-translate-y-px hover:border-sky-400/45 hover:bg-sky-500/10" aria-label="Otevřít detail pokrytí adres">
      <div className="flex items-end justify-between gap-3">
        <span className="min-w-0">
          <small className="block whitespace-nowrap text-[7px] font-bold uppercase tracking-[0.04em] text-[var(--text-secondary)]">Připraveno pro hledání</small>
          <strong className="mt-1 block whitespace-nowrap text-[10px] tabular-nums text-[var(--text-primary)]">{coverage.searchableCount} z {coverage.progressTotalCount}</strong>
        </span>
        <strong className="shrink-0 whitespace-nowrap text-2xl leading-none tabular-nums text-[var(--accent)]">{percent.toLocaleString('cs-CZ')} %</strong>
      </div>
      <div className="mt-3 h-2 overflow-hidden rounded-full bg-[var(--surface-border)]"><div className="h-full rounded-full bg-sky-500" style={{ width: `${Math.min(100, percent)}%` }} /></div>
    </button> : <div className="mt-4 rounded-2xl border border-sky-400/25 bg-sky-500/8 p-3.5 text-left">
      <div className="flex items-end justify-between gap-3">
        <span className="min-w-0">
          <small className="block whitespace-nowrap text-[7px] font-bold uppercase tracking-[0.04em] text-[var(--text-secondary)]">Připraveno pro hledání</small>
          <strong className="mt-1 block whitespace-nowrap text-[10px] tabular-nums text-[var(--text-primary)]">{coverage.searchableCount} z {coverage.progressTotalCount}</strong>
    ...
    <p className={`mt-2 flex items-center justify-center gap-1.5 text-center text-[8px] ${coverage.pendingCount > 0 ? 'text-sky-700 [html[data-theme=dark]_&]:text-sky-300' : 'text-[var(--text-secondary)]'}`}><TimerReset aria-hidden size={11} className={coverage.pendingCount > 0 ? 'animate-pulse' : ''} /><span>Normalizační fronta: <strong className="tabular-nums">{coverage.pendingCount.toLocaleString('cs-CZ')}</strong>{coverage.pendingCount > 0 ? ' · interní zpracování pokračuje' : ' · vše připraveno'}</span></p>
    <div className="mt-3 grid grid-cols-2 gap-1.5 text-center"><span className="rounded-xl border border-emerald-400/25 bg-emerald-400/8 p-2 text-[6px] font-bold uppercase text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300">Přesné<strong className="mt-1 block text-sm text-[var(--text-primary)]">{coverage.exactCount}</strong></span><span className="rounded-xl border border-amber-400/25 bg-amber-400/8 p-2 text-[6px] font-bold uppercase text-amber-700 [html[data-theme=dark]_&]:text-amber-300">Pouze ulice<strong className="mt-1 block text-sm text-[var(--text-primary)]">{coverage.streetCount}</strong></span><span className="rounded-xl border border-orange-400/25 bg-orange-400/8 p-2 text-[6px] font-bold uppercase text-orange-700 [html[data-theme=dark]_&]:text-orange-300">K pozornosti<strong className="mt-1 block text-sm text-[var(--text-primary)]">{attentionCount}</strong></span><span className="rounded-xl border border-slate-400/25 bg-slate-400/8 p-2 text-[6px] font-bold uppercase text-slate-600 [html[data-theme=dark]_&]:text-slate-300">Čeká<strong className="mt-1 block text-sm text-[var(--text-primary)]">{coverage.pendingCount}</strong></span></div>
    <p className="mt-2 text-center text-[7px] leading-3 text-[var(--text-secondary)]">Pouze obec mimo vyhledávání: {coverage.municipalityCount}</p>
    {isAdmin && showDetail && typeof document !== 'undefined' ? createPortal(<AddressCoverageDiagnosticPopup diagnostic={diagnostic} loading={loading} error={error} isAdmin onReload={() => loadDiagnostic(false)} onClose={() => { setShowDetail(false); setDiagnostic(null); setError(null) }} />, document.body) : null}
  </PanelShell>
}
...
function contactManagementPresentation(summary: CompleteContactManagementSummary) {
  ...
  if (!summary.runtimeEnabled) return {
    label: 'POZASTAVENO',
    badge: 'border-slate-400/35 bg-slate-400/10 text-slate-700 [html[data-theme=dark]_&]:text-slate-300',
    dot: 'bg-slate-400',
  }
...
function ContactManagementPopup({ initialSummary, onSummaryChange, onClose }: {
  ...
  const progressPercent = summary.targetCompanyCount > 0
    ? Math.min(100, Math.round((completedCompanyCount / summary.targetCompanyCount) * 1000) / 10)
    : 100
  ...
  return <PowerOutagePopupShell titleId="complete-contact-management" eyebrow="DOHLEDÁVÁNÍ A OVĚŘENÍ" title="Kontakty firem" icon={<AtSign aria-hidden size={21} />} onClose={onClose}>
    {loading ? <div className="flex min-h-[360px] items-center justify-center"><LoaderCircle aria-hidden size={28} className="animate-spin text-[var(--accent)]" /></div> : error && !workspace ? <div className="m-5"><PanelLoadError message={error} onRetry={() => void load()} /></div> : workspace ? <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
      ...
      <section className={`rounded-2xl border p-4 ${summary.runtimeEnabled ? 'border-sky-400/25 bg-sky-500/8' : 'border-[var(--surface-border)] bg-[var(--surface-muted)]'}`}>
        <div className="flex flex-wrap items-start justify-between gap-3"><div><small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--text-secondary)]">Celkový provozní stav</small><h3 className="mt-0.5 text-lg font-semibold text-[var(--text-primary)]">{summary.runtimeEnabled ? 'Dohledávání kontaktů je zapnuté' : 'Dohledávání kontaktů je pozastavené'}</h3><p className="mt-1 text-[9px] text-[var(--text-secondary)]">Poslední aktivita {formatRelativeDate(summary.lastActivityAt)}</p></div><CompletePanelStatusBadge label={contactManagementPresentation(summary).label} badgeClassName={contactManagementPresentation(summary).badge} dotClassName={contactManagementPresentation(summary).dot} /></div>

        <div className={`mt-4 grid gap-3 lg:grid-cols-2 ${pendingKey === 'runtime' ? 'pointer-events-none opacity-60' : ''}`}>
          <div><small className="block text-[8px] font-bold uppercase tracking-[0.08em] text-[var(--text-secondary)]">Dohledávání</small><SlidingTwoTabSwitch value={summary.runtimeEnabled ? 'enabled' : 'paused'} options={[{ value: 'paused', label: 'Pozastaveno' }, { value: 'enabled', label: 'Zapnuto' }]} onValueChange={(value) => void setRuntime(value === 'enabled', value === 'enabled' && summary.braveFallbackEnabled)} ariaLabel="Stav dohledávání kontaktů" className="mt-2 rounded-xl bg-[var(--surface-strong)] p-1" compact /></div>
      ...
      {reviewTab === 'emails' ? <section className="mt-4">
      ...
      {reviewTab === 'domains' ? <section className="mt-4">
      ...
      <details className="mt-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)]"><summary className="cursor-pointer list-none px-4 py-3 text-[9px] font-bold uppercase tracking-[0.08em] text-[var(--text-secondary)]">Historie zpracování · {workspace.recentBatches.length}</summary><div className="space-y-2 border-t border-[var(--surface-border)] p-3">{workspace.recentBatches.map((batch) => <div key={batch.id} className="grid grid-cols-[1fr_auto] gap-2 rounded-xl bg-[var(--surface-strong)] px-3 py-2"><span><strong className="block text-[9px] text-[var(--text-primary)]">{workspace.selectors.find((selector) => selector.key === batch.selectorKey)?.name ?? batch.selectorKey}</strong><small className="text-[7px] text-[var(--text-secondary)]">{formatDate(batch.capturedAt)}</small></span><span className="text-right"><strong className="block text-[8px] uppercase text-[var(--accent)]">{batch.status}</strong><small className="text-[7px] tabular-nums text-[var(--text-secondary)]">{batch.targetCount.toLocaleString('cs-CZ')} IČO</small></span></div>)}</div></details>
    </div> : null}
  </PowerOutagePopupShell>
}
...
function ContactManagementPanel({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  ...
  if (!summary) return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10 text-red-500"><AtSign aria-hidden size={18} /></span><span><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Dohledávání a ověření</small><h3 className="text-base font-semibold text-[var(--text-primary)]">Kontakty firem</h3></span></div><PanelLoadError message={workspace.loadErrors.contactManagement || 'Souhrn kontaktů zatím není dostupný.'} onRetry={onRetry} /></PanelShell>
...
function emailManagementPresentation(workspace: CompleteNotificationEmailManagementWorkspace) {
  if (workspace.productionSafety.isPaused) return {
    label: 'CHYBA',
    badge: 'border-red-400/35 bg-red-400/10 text-red-700 [html[data-theme=dark]_&]:text-red-300',
    dot: 'bg-red-500',
  }
  if (workspace.productionConfiguration.configurationStatus === 'live' && workspace.productionConfiguration.continuousDispatchEnabled && workspace.operations.dispatchEnabled) return {
    label: 'AKTIVNÍ',
    badge: 'border-emerald-400/35 bg-emerald-400/10 text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300',
    dot: 'bg-emerald-500',
  }
  return {
    label: workspace.operations.runtimeMode === 'paused' ? 'POZASTAVENO' : 'PŘIPRAVENO',
    badge: workspace.operations.runtimeMode === 'paused'
    ...
    dot: workspace.operations.runtimeMode === 'paused' ? 'bg-amber-500' : 'bg-sky-500',
  }
}
...
function deliveryPresentation(status: CompleteNotificationEmailManagementWorkspace['operations']['recentDeliveries'][number]['deliveryStatus']) {
  if (status === 'delivered') return { label: 'DORUČENO', className: 'text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300' }
  if (status === 'bounced') return { label: 'NEDORUČENO', className: 'text-red-700 [html[data-theme=dark]_&]:text-red-300' }
  if (status === 'complaint') return { label: 'STÍŽNOST', className: 'text-red-700 [html[data-theme=dark]_&]:text-red-300' }
  if (status === 'error') return { label: 'CHYBA', className: 'text-red-700 [html[data-theme=dark]_&]:text-red-300' }
  return { label: 'ODESLÁNO', className: 'text-sky-700 [html[data-theme=dark]_&]:text-sky-300' }
}

function pragueDateDaysAgo(daysAgo: number) {
...
function EmailManagementPopup({ initialWorkspace, onWorkspaceChange, onClose }: {
  ...
  const [pauseReason, setPauseReason] = useState('Ruční pozastavení administrátorem.')
  const [historyOpen, setHistoryOpen] = useState(false)
  const [history, setHistory] = useState<CompleteNotificationEmailDeliveryHistory | null>(null)
  const [historyLoading, setHistoryLoading] = useState(false)
  const [historyError, setHistoryError] = useState<string | null>(null)
  const [historyStatus, setHistoryStatus] = useState<'all' | CompleteNotificationEmailDeliveryStatus>('all')
  ...
  const acknowledgePause = async () => {
    ...
    setError(null)
    const result = workspace.productionSafety.isPaused
      ? await acknowledgeCompleteNotificationEmailProductionPauseAction(incidentNote)
      : await acknowledgeCompleteNotificationEmailPilotPauseAction(incidentNote)
    if (result.success) applyWorkspace(result.workspace)
    else setError(result.error)
    setPendingKey(null)
  }
  const pauseProduction = async () => {
    setPendingKey('pause-live')
    setError(null)
    const result = await pauseCompleteNotificationEmailProductionAction(pauseReason)
    if (result.success) applyWorkspace(result.workspace)
    else setError(result.error)
    setPendingKey(null)
  }
  const prepareProductionActivation = async () => {
    setPendingKey('prepare-production-activation')
    setError(null)
    const result = await prepareCompleteNotificationEmailProductionActivationAction()
    if (result.success) setActivationConfirmation(result.confirmation)
    else setError(result.error)
    setPendingKey(null)
  }
  const activateProduction = async () => {
    if (!activationConfirmation) return
    setPendingKey('activate-production')
    setError(null)
    const result = await activateCompleteNotificationEmailProductionAction(activationConfirmation.confirmationToken)
    if (result.success) {
  ...
  const loadHistory = async (options?: { append?: boolean; search?: string }) => {
    ...
    const offset = append ? history?.items.length ?? 0 : 0
    setHistoryLoading(true)
    setHistoryError(null)
    const result = await getCompleteNotificationEmailDeliveryHistoryAction({
      offset,
      status: historyStatus,
      search,
      dateFrom: historyPeriod === 'all' ? null : pragueDateDaysAgo(Number(historyPeriod) - 1),
      dateTo: historyPeriod === 'all' ? null : pragueDateDaysAgo(0),
    })
    if (result.success) {
      setHistory(append && history
        ? { ...result.history, items: [...history.items, ...result.history.items] }
        : result.history)
      setHistoryAppliedSearch(search)
  ...
  const pendingItems = workspace.review.items.filter((item) => item.reviewStatus === 'pending').slice(0, 10)
  ...
  const liveDispatchEnabled = productionConfig.configurationStatus === 'live'
    && productionConfig.productionActivationEnabled
    && productionConfig.continuousDispatchEnabled
    && workspace.operations.dispatchEnabled
  const operationalProblemCount = workspace.productionSafety.isPaused ? 1 : 0
  const attentionCount = workspace.review.pendingCount + operationalProblemCount
  const operationalTitle = workspace.productionSafety.isPaused
    ? 'Odesílání bylo bezpečnostně zastaveno'
    : liveDispatchEnabled
  ...
  const renderNotice = (item: CompleteNotificationEmailPlanItem, mode: 'pending' | 'approved' | 'inactive') => {
    ...
    return <article key={`${mode}-${item.planId}`} className={`rounded-2xl border p-3 ${inPilot ? 'border-emerald-400/35 bg-emerald-400/8' : mode === 'pending' ? 'border-amber-400/30 bg-amber-400/6' : 'border-[var(--surface-border)] bg-[var(--surface-muted)]'}`}>
      ...
      {mode === 'pending' ? <div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={pendingKey !== null || liveDispatchEnabled} onClick={() => void decide(item.planId, 'approved')} className={`${actionButton} border-emerald-400/35 bg-emerald-400/10 text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300`}>{pendingKey === reviewKey ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <Check aria-hidden size={11} />}Schválit příjemce</button><button type="button" disabled={pendingKey !== null || liveDispatchEnabled} onClick={() => void decide(item.planId, 'rejected')} className={`${actionButton} border-red-400/30 bg-red-400/8 text-red-700 [html[data-theme=dark]_&]:text-red-300`}><X aria-hidden size={11} />Zamítnout</button></div> : null}
      {mode === 'approved' ? <div className="mt-3 flex flex-wrap gap-2">{inPilot ? <button type="button" disabled={pendingKey !== null || liveDispatchEnabled} onClick={() => void changeAllowlist(item.planId, false)} className={`${actionButton} border-red-400/30 bg-red-400/8 text-red-700 [html[data-theme=dark]_&]:text-red-300`}>{pendingKey === allowlistKey ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <X aria-hidden size={11} />}Odebrat z pilotu</button> : <button type="button" disabled={pendingKey !== null || liveDispatchEnabled || allowlistFull} title={allowlistFull ? 'Pilotní seznam dosáhl nastaveného maxima.' : undefined} onClick={() => void changeAllowlist(item.planId, true)} className={`${actionButton} border-sky-400/35 bg-sky-400/10 text-sky-700 [html[data-theme=dark]_&]:text-sky-300`}>{pendingKey === allowlistKey ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <Check aria-hidden size={11} />}Zařadit do pilotu</button>}<button type="button" disabled={pendingKey !== null || liveDispatchEnabled} onClick={() => void decide(item.planId, 'revoked')} className={`${actionButton} border-[var(--surface-border)] bg-[var(--surface-strong)] text-[var(--text-secondary)]`}>Zrušit schválení</button></div> : null}
    </article>
  }

  const renderPreparedNotice = (item: CompleteNotificationEmailManagementWorkspace['operations']['preparedItems'][number]) => <article key={item.planId} className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3">
    <div className="flex items-start justify-between gap-3"><span className="min-w-0"><strong className="block truncate text-[11px] text-[var(--text-primary)]">{item.companyName}</strong><small className="block truncate text-[8px] text-[var(--text-secondary)]">{item.recipientEmail}</small></span><span className="shrink-0 rounded-full border border-sky-400/30 bg-sky-400/10 px-2 py-1 text-[7px] font-bold uppercase text-sky-700 [html[data-theme=dark]_&]:text-sky-300">Připraveno</span></div>
    <div className="mt-2 grid gap-1 text-[8px] leading-4 text-[var(--text-secondary)] sm:grid-cols-2"><span><strong className="text-[var(--text-primary)]">{sourceLabel(item.source)}</strong> · {formatDate(item.startsAt)}–{new Intl.DateTimeFormat('cs-CZ', { timeZone: 'Europe/Prague', hour: '2-digit', minute: '2-digit' }).format(new Date(item.endsAt))}</span><span className="truncate sm:text-right">{formatPilotAddress({ ...item, reviewStatus: 'pending', approvedAndEligibleNow: false, ico: '', reason: null, decidedAt: null })}</span></div>
  </article>

  const renderDelivery = (delivery: CompleteNotificationEmailManagementWorkspace['operations']['recentDeliveries'][number], index: number, expandable = false) => {
    const status = deliveryPresentation(delivery.deliveryStatus)
    const content = <><span className="min-w-0"><strong className="block truncate text-[10px] text-[var(--text-primary)]">{delivery.companyName}</strong><small className="block truncate text-[8px] text-[var(--text-secondary)]">{delivery.recipientEmail} · {formatDate(delivery.sentAt)}</small></span><strong className={`shrink-0 text-[8px] ${status.className}`}>{status.label}</strong></>
    if (!expandable) return <div key={`${delivery.companyName}-${delivery.sentAt}`} className={`flex items-center justify-between gap-3 px-3 py-2.5 ${index ? 'border-t border-[var(--surface-border)]' : ''}`}>{content}</div>
    return <details key={`${delivery.companyName}-${delivery.sentAt}`} className={`group ${index ? 'border-t border-[var(--surface-border)]' : ''}`}><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2.5">{content}</summary><div className="grid gap-1 border-t border-[var(--surface-border)] bg-[var(--surface-strong)] px-3 py-2 text-[8px] leading-4 text-[var(--text-secondary)] sm:grid-cols-2"><span>Odstávka: <strong className="text-[var(--text-primary)]">{formatDate(delivery.startsAt)}{delivery.endsAt ? `–${new Intl.DateTimeFormat('cs-CZ', { timeZone: 'Europe/Prague', hour: '2-digit', minute: '2-digit' }).format(new Date(delivery.endsAt))}` : ''}</strong></span><span className="sm:text-right">{delivery.municipality || sourceLabel(delivery.source)}</span>{delivery.errorCode ? <span className="text-red-600 sm:col-span-2">Chyba: {delivery.errorCode}</span> : null}</div></details>
  }

  return <PowerOutagePopupShell titleId="complete-email-management" eyebrow="AUTOMATICKÁ OZNÁMENÍ" title="Upozornění firmám" icon={<Mail aria-hidden size={21} />} onClose={onClose}>
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
      {error ? <p className="mb-3 rounded-xl border border-red-400/30 bg-red-400/10 px-3 py-2 text-[10px] text-red-700 [html[data-theme=dark]_&]:text-red-300">{error}</p> : null}
      {historyOpen ? <section>
        <div className="flex flex-wrap items-center justify-between gap-3"><button type="button" onClick={() => setHistoryOpen(false)} className={`${actionButton} border-[var(--surface-border)] bg-[var(--surface-muted)] text-[var(--text-primary)]`}><ArrowLeft aria-hidden size={12} />Zpět na přehled</button><span className="text-[9px] font-semibold text-[var(--text-secondary)]">{history?.totalCount.toLocaleString('cs-CZ') ?? '—'} záznamů</span></div>
        <div className="mt-4"><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Historie odesílání</small><h3 className="mt-1 text-lg font-semibold text-[var(--text-primary)]">Odeslaná upozornění firmám</h3><p className="mt-1 text-[9px] text-[var(--text-secondary)]">Výsledky se načítají ze serveru po 20 záznamech.</p></div>
        <form onSubmit={(event) => { event.preventDefault(); void loadHistory({ search: historySearch.trim() }) }} className="mt-4 grid gap-2 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3 sm:grid-cols-[1fr_130px_130px_auto]">
          <input value={historySearch} onChange={(event) => setHistorySearch(event.target.value)} maxLength={120} placeholder="Firma nebo e-mail" className="h-9 min-w-0 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] px-3 text-[9px] text-[var(--text-primary)] outline-none focus:border-sky-400" />
          <select value={historyStatus} onChange={(event) => { setHistoryStatus(event.target.value as typeof historyStatus); setHistory(null) }} className="h-9 appearance-none rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] px-3 text-[9px] font-semibold text-[var(--text-primary)] outline-none [-webkit-appearance:none] focus:border-sky-400"><option value="all">Všechny stavy</option><option value="delivered">Doručeno</option><option value="sent">Odesláno</option><option value="bounced">Nedoručeno</option><option value="complaint">Stížnost</option><option value="error">Chyba</option></select>
          <select value={historyPeriod} onChange={(event) => { setHistoryPeriod(event.target.value as typeof historyPeriod); setHistory(null) }} className="h-9 appearance-none rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] px-3 text-[9px] font-semibold text-[var(--text-primary)] outline-none [-webkit-appearance:none] focus:border-sky-400"><option value="all">Celé období</option><option value="7">Posledních 7 dní</option><option value="30">Posledních 30 dní</option><option value="90">Posledních 90 dní</option></select>
          <button type="submit" disabled={historyLoading} className={`${actionButton} h-9 border-sky-400/30 bg-sky-500/10 text-[var(--accent)]`}>{historyLoading ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <SearchCheck aria-hidden size={11} />}Filtrovat</button>
        </form>
        {historyError ? <p className="mt-3 rounded-xl border border-red-400/30 bg-red-400/10 px-3 py-2 text-[9px] text-red-700 [html[data-theme=dark]_&]:text-red-300">{historyError}</p> : null}
        <div className="mt-3 overflow-hidden rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)]">{history?.items.length ? history.items.map((delivery, index) => renderDelivery(delivery, index, true)) : historyLoading ? <div className="flex min-h-28 items-center justify-center"><LoaderCircle aria-label="Načítání historie" size={20} className="animate-spin text-[var(--accent)]" /></div> : <p className="px-3 py-5 text-center text-[9px] text-[var(--text-secondary)]">Pro zvolené filtry nebyl nalezen žádný e-mail.</p>}</div>
        {history?.hasMore ? <button type="button" disabled={historyLoading} onClick={() => void loadHistory({ append: true })} className={`${actionButton} mt-3 w-full border-sky-400/30 bg-sky-500/10 text-[var(--accent)]`}>{historyLoading ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <ChevronDown aria-hidden size={12} />}Načíst dalších 20</button> : null}
      </section> : <>
      <section className={`rounded-2xl border p-4 ${workspace.productionSafety.isPaused ? 'border-red-400/30 bg-red-400/8' : liveDispatchEnabled ? 'border-emerald-400/30 bg-emerald-400/8' : 'border-sky-400/25 bg-sky-500/8'}`}>
        <div className="flex flex-wrap items-start justify-between gap-3"><div><small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--text-secondary)]">Celkový provozní stav</small><h3 className="mt-0.5 text-lg font-semibold text-[var(--text-primary)]">{operationalTitle}</h3><p className="mt-1 text-[9px] text-[var(--text-secondary)]">Zdroj firem: <strong className="text-[var(--text-primary)]">{contactSelectorLabel(workspace.operations.selectedSelectorKey)}</strong></p></div><div className="flex items-center gap-2"><CompletePanelStatusBadge label={emailManagementPresentation(workspace).label} badgeClassName={emailManagementPresentation(workspace).badge} dotClassName={emailManagementPresentation(workspace).dot} /><button type="button" onClick={() => void reload()} disabled={loading || pendingKey !== null} className={`${actionButton} border-sky-400/30 bg-[var(--surface-strong)] text-[var(--accent)]`}>{loading ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <RefreshCw aria-hidden size={11} />}Obnovit</button></div></div>
        <div className="mt-3 grid grid-cols-2 gap-2 lg:grid-cols-4"><PowerOutageDetailRow label="Připraveno" value={workspace.operations.preparedCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Dnes odesláno" value={workspace.operations.sentTodayCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Dnes doručeno" value={workspace.operations.deliveredTodayCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Vyžaduje pozornost" value={attentionCount.toLocaleString('cs-CZ')} /></div>
        <p className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[8px] text-[var(--text-secondary)]"><span>Poslední odeslání: <strong className="text-[var(--text-primary)]">{formatDate(workspace.operations.lastSentAt)}</strong></span><span>Poslední doručení: <strong className="text-[var(--text-primary)]">{formatDate(workspace.operations.lastDeliveredAt)}</strong></span></p>
      </section>

      {!liveDispatchEnabled && !workspace.productionSafety.isPaused && workspace.liveActivationAvailable ? <section className={`mt-4 rounded-2xl border p-4 ${activationConfirmation ? 'border-amber-400/35 bg-amber-400/8' : 'border-sky-400/30 bg-sky-500/8'}`}>
        {!activationConfirmation ? <>
          <small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-sky-700 [html[data-theme=dark]_&]:text-sky-300">Ruční aktivace ostrého provozu</small>
          <h3 className="mt-1 text-base font-semibold text-[var(--text-primary)]">První potvrzení</h3>
          <p className="mt-2 text-[9px] leading-4 text-[var(--text-secondary)]">První kliknutí znovu provede bezpečnostní SHADOW audit a připraví krátkodobé potvrzení. Samo o sobě žádný e-mail neodešle ani odesílání nezapne.</p>
          <button type="button" disabled={pendingKey !== null} onClick={() => void prepareProductionActivation()} className={`${actionButton} mt-3 w-full border-sky-400/40 bg-sky-500/12 text-sky-700 [html[data-theme=dark]_&]:text-sky-300`}>{pendingKey === 'prepare-production-activation' ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <Check aria-hidden size={11} />}Aktivovat odesílání</button>
        </> : <>
          <small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-amber-700 [html[data-theme=dark]_&]:text-amber-300">Druhé a finální potvrzení</small>
          <h3 className="mt-1 text-base font-semibold text-[var(--text-primary)]">Potvrďte spuštění ostrého provozu</h3>
          <p className="mt-2 text-[9px] leading-4 text-[var(--text-secondary)]">Následující kliknutí odesílání skutečně zapne. První zpráva může odejít při nejbližším běhu workeru podle nastaveného času, rozestupu a limitů.</p>
          <div className="mt-3 grid grid-cols-2 gap-2 lg:grid-cols-4"><PowerOutageDetailRow label="Zdroj firem" value={contactSelectorLabel(activationConfirmation.selectorKey)} /><PowerOutageDetailRow label="Připraveno k odeslání" value={activationConfirmation.dispatchablePlanCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Denní / měsíční limit" value={`${activationConfirmation.dailySendLimit} / ${activationConfirmation.monthlySendLimit}`} /><PowerOutageDetailRow label="Minimální rozestup" value={`${Math.ceil(activationConfirmation.minimumIntervalSeconds / 60)} min`} /></div>
          <p className="mt-2 text-[8px] text-[var(--text-secondary)]">Odesílací okno {activationConfirmation.sendWindowStart}–{activationConfirmation.sendWindowEnd} · potvrzení platí do {formatDate(activationConfirmation.expiresAt)}.</p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2"><button type="button" disabled={pendingKey !== null} onClick={() => setActivationConfirmation(null)} className={`${actionButton} border-[var(--surface-border)] bg-[var(--surface-strong)] text-[var(--text-secondary)]`}><X aria-hidden size={11} />Zrušit</button><button type="button" disabled={pendingKey !== null} onClick={() => void activateProduction()} className={`${actionButton} border-amber-400/45 bg-amber-500/12 text-amber-800 [html[data-theme=dark]_&]:text-amber-200`}>{pendingKey === 'activate-production' ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <Check aria-hidden size={11} />}Potvrdit a aktivovat</button></div>
        </>}
      </section> : null}

      <section className="mt-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-4"><h3 className="text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Tok e-mailových upozornění</h3><div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto_1fr_auto_1fr]"><PowerOutageDetailRow label="Připraveno" value={workspace.operations.preparedCount.toLocaleString('cs-CZ')} /><span className="hidden self-center text-[var(--text-secondary)] sm:block">→</span><PowerOutageDetailRow label="Odesláno dnes" value={workspace.operations.sentTodayCount.toLocaleString('cs-CZ')} /><span className="hidden self-center text-[var(--text-secondary)] sm:block">→</span><PowerOutageDetailRow label="Doručeno dnes" value={workspace.operations.deliveredTodayCount.toLocaleString('cs-CZ')} /></div></section>

      <section className="mt-4"><div className="flex items-center justify-between gap-2"><h3 className="text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Připravená oznámení</h3><span className="text-[9px] font-bold text-sky-600">{workspace.operations.preparedCount}</span></div><p className="mt-1 text-[9px] text-[var(--text-secondary)]">Budoucí platná oznámení s použitelným firemním kontaktem. V režimu PŘIPRAVENO se neodesílají.</p><div className="mt-2 space-y-2">{workspace.operations.preparedItems.length ? workspace.operations.preparedItems.map(renderPreparedNotice) : <p className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-3 py-2 text-[9px] text-[var(--text-secondary)]">Žádné další oznámení nyní nečeká.</p>}</div></section>

      <section className="mt-4"><div className="flex items-center justify-between gap-2"><h3 className="text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Poslední odeslané e-maily</h3><span className="text-[9px] font-bold text-emerald-600">{workspace.operations.sentTotalCount.toLocaleString('cs-CZ')}</span></div><div className="mt-2 overflow-hidden rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)]">{workspace.operations.recentDeliveries.length ? workspace.operations.recentDeliveries.slice(0, 5).map((delivery, index) => renderDelivery(delivery, index)) : <p className="px-3 py-3 text-[9px] text-[var(--text-secondary)]">Zatím nebyl odeslán žádný e-mail.</p>}</div>{workspace.operations.sentTotalCount > 0 ? <button type="button" onClick={openHistory} className={`${actionButton} mt-2 w-full border-[var(--surface-border)] bg-[var(--surface-muted)] text-[var(--text-primary)]`}><History aria-hidden size={12} />Zobrazit celou historii · celkem {workspace.operations.sentTotalCount.toLocaleString('cs-CZ')}</button> : null}</section>

      {workspace.productionSafety.isPaused ? <section className="mt-4 rounded-2xl border border-red-400/30 bg-red-400/8 p-4"><h3 className="text-[10px] font-bold uppercase tracking-[0.08em] text-red-700 [html[data-theme=dark]_&]:text-red-300">Bezpečnostní incident vyžaduje kontrolu</h3><p className="mt-2 text-[9px] leading-4 text-[var(--text-secondary)]">{workspace.productionSafety.lastErrorMessage || workspace.productionSafety.pauseReasonCode || 'Odesílání bylo automaticky pozastaveno.'}</p><textarea value={incidentNote} onChange={(event) => setIncidentNote(event.target.value)} rows={2} className="mt-3 w-full resize-none rounded-xl border border-red-400/25 bg-[var(--surface-strong)] px-3 py-2 text-[9px] text-[var(--text-primary)] outline-none focus:border-red-400" /><button type="button" disabled={pendingKey !== null || incidentNote.trim().length < 3} onClick={() => void acknowledgePause()} className={`${actionButton} mt-2 border-red-400/35 bg-red-400/10 text-red-700 [html[data-theme=dark]_&]:text-red-300`}>{pendingKey === 'acknowledge' ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <Check aria-hidden size={11} />}Potvrdit kontrolu incidentu</button><p className="mt-2 text-[8px] text-[var(--text-secondary)]">Potvrzení incidentu samo o sobě odesílání znovu nezapne.</p></section> : null}
      ...
      <details className="mt-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)]">
        <summary className="cursor-pointer list-none px-4 py-3 text-[9px] font-bold uppercase tracking-[0.08em] text-[var(--text-secondary)]">Bezpečnost a pozastavení</summary>
        <div className="border-t border-[var(--surface-border)] p-4">
          <div className="grid grid-cols-2 gap-2 lg:grid-cols-4"><PowerOutageDetailRow label="Série chyb" value={`${workspace.productionSafety.consecutiveTransientFailureCount} / ${workspace.productionSafety.transientFailureThreshold}`} /><PowerOutageDetailRow label="Aktivní rezervace" value={workspace.rateLimit.activeReservationCount.toLocaleString('cs-CZ')} /><PowerOutageDetailRow label="Webhook" value={workspace.productionSafety.liveSignalIngestionEnabled ? 'AKTIVNÍ' : 'PŘIPRAVEN'} /><PowerOutageDetailRow label="Režim" value={workspace.operations.runtimeMode.toUpperCase()} /></div>
          <p className="mt-3 text-[8px] leading-4 text-[var(--text-secondary)]">Complaint, hard bounce nebo chyba konfigurace zastaví odesílání okamžitě. Tři navazující přechodné chyby jej zastaví také.</p>
          {liveDispatchEnabled ? <div className="mt-4 border-t border-red-400/20 pt-4"><p className="text-[9px] leading-4 text-[var(--text-secondary)]">Nouzové pozastavení vypne pouze rozesílání KOMPLETNÍ. Panel MARKETY zůstane beze změny.</p><textarea value={pauseReason} onChange={(event) => setPauseReason(event.target.value)} rows={2} className="mt-3 w-full resize-none rounded-xl border border-red-400/25 bg-[var(--surface-strong)] px-3 py-2 text-[9px] text-[var(--text-primary)] outline-none focus:border-red-400" /><button type="button" disabled={pendingKey !== null || pauseReason.trim().length < 3} onClick={() => void pauseProduction()} className={`${actionButton} mt-2 border-red-400/35 bg-red-400/10 text-red-700 [html[data-theme=dark]_&]:text-red-300`}>{pendingKey === 'pause-live' ? <LoaderCircle aria-hidden size={11} className="animate-spin" /> : <X aria-hidden size={11} />}Pozastavit odesílání</button></div> : null}
        </div>
      </details>

      {approvedItems.length ? <details className="mt-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)]"><summary className="cursor-pointer list-none px-4 py-3 text-[9px] font-bold uppercase tracking-[0.08em] text-[var(--text-secondary)]">Historie pilotního výběru · {approvedItems.length}</summary><div className="space-y-2 border-t border-[var(--surface-border)] p-3">{approvedItems.map((item) => renderNotice(item, 'approved'))}</div></details> : null}
...
function EmailManagementPanel({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  ...
  if (!workspace.currentUser.isAdmin) return null
  if (!emailWorkspace) return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10 text-red-500"><Mail aria-hidden size={18} /></span><span><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Automatická oznámení</small><h3 className="text-base font-semibold text-[var(--text-primary)]">Upozornění firmám</h3></span></div><PanelLoadError message={workspace.loadErrors.emailManagement || 'Správa e-mailových upozornění zatím není dostupná.'} onRetry={onRetry} /></PanelShell>
  const presentation = emailManagementPresentation(emailWorkspace)
  return <PanelShell>
    <div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-500/10 text-sky-600 [html[data-theme=dark]_&]:text-sky-300"><Mail aria-hidden size={19} /></span><span className="min-w-0"><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Automatická oznámení</small><h3 className="truncate text-base font-semibold text-[var(--text-primary)]">Upozornění firmám</h3></span></div><CompletePanelStatusBadge label={presentation.label} badgeClassName={presentation.badge} dotClassName={presentation.dot} /></div>
    <div className="mt-3 grid grid-cols-3 gap-2"><div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-2 py-2 text-center"><small className="block text-[6.5px] font-bold uppercase text-[var(--text-secondary)]">Připraveno</small><strong className="mt-0.5 block text-[12px] tabular-nums text-sky-600">{emailWorkspace.operations.preparedCount.toLocaleString('cs-CZ')}</strong></div><div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-2 py-2 text-center"><small className="block text-[6.5px] font-bold uppercase text-[var(--text-secondary)]">Dnes odesláno</small><strong className="mt-0.5 block text-[12px] tabular-nums text-[var(--text-primary)]">{emailWorkspace.operations.sentTodayCount.toLocaleString('cs-CZ')}</strong></div><div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] px-2 py-2 text-center"><small className="block text-[6.5px] font-bold uppercase text-[var(--text-secondary)]">Doručeno</small><strong className="mt-0.5 block text-[12px] tabular-nums text-emerald-600 [html[data-theme=dark]_&]:text-emerald-300">{emailWorkspace.operations.deliveredTodayCount.toLocaleString('cs-CZ')}</strong></div></div>
    <p className="mt-2 truncate text-center text-[8px] text-[var(--text-secondary)]">Poslední doručení: {formatDate(emailWorkspace.operations.lastDeliveredAt)}</p>
    <button type="button" onClick={() => setShowDetail(true)} className="mt-3 inline-flex h-9 w-full items-center justify-center gap-2 rounded-xl border border-sky-400/30 bg-sky-500/10 text-[8px] font-bold uppercase tracking-[0.06em] text-sky-700 transition hover:-translate-y-px hover:border-sky-400/50 [html[data-theme=dark]_&]:text-sky-300"><Mail aria-hidden size={13} />Spravovat e-maily</button>
...
const COMMERCIAL_CLIENT_SCOPE_OPTIONS = [
  ...
  { value: 'clients', label: 'Pouze odstávky klientů' },
] as const

const COMMERCIAL_SORT_OPTIONS = [
  { value: 'date', label: 'Nejbližší termín' },
...
export function CompletePowerOutageSidebar({ workspace, sourceRefreshWarning, commercialSelection, commercialSort, clientsOnly, commercialSelectionCounts, onCommercialSelectionChange, onCommercialSortChange, onClientsOnlyChange, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; sourceRefreshWarning?: string | null; commercialSelection: CompleteCommercialSelectionFilter; commercialSort: CompleteCommercialSort; clientsOnly: boolean; commercialSelectionCounts: CompleteCommercialSelectionCounts | null; onCommercialSelectionChange: (value: CompleteCommercialSelectionFilter) => void; onCommercialSortChange: (value: CompleteCommercialSort) => void; onClientsOnlyChange: (value: boolean) => void; onRetry?: () => void }) {
  ...
  return <div className="-mt-2 min-w-0 lg:mt-0 xl:h-full">
    ...
    <aside className="power-outages-mobile-aside-carousel grid min-w-0 auto-cols-[100%] grid-flow-col items-stretch gap-3 snap-x snap-mandatory overflow-x-auto overflow-y-hidden overscroll-x-contain rounded-[24px] lg:block lg:space-y-4 lg:overflow-visible lg:rounded-none" aria-label="Stav kompletního sběru a vyhledávání firem">
      ...
      <div className="min-w-0 snap-start snap-always lg:snap-none"><CoveragePanel workspace={workspace} onRetry={operationalRetry} /></div>
    </aside>
  </div>
}
