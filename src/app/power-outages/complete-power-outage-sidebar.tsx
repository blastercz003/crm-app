    ...
    <div className="mt-3 space-y-1">
      {workspace.sources.map((source) => {
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={() => openSource(source.source)} className={`relative inline-flex h-6 w-[98px] items-center justify-center gap-1 rounded-lg border px-1.5 text-center text-[7px] font-bold uppercase leading-none tracking-[0.055em] transition hover:-translate-y-px ${presentation.badge}`} title={discovery.statusMessage} aria-label={`${sourceLabel(source.source)}: ${discoveryStatusLabel(discovery.status)}. ${discovery.statusMessage} Zobrazit provozní detail.`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${presentation.dot}`} /><span className="translate-y-px">{discoveryStatusLabel(discovery.status)}</span>{presentation.attention ? <span className="absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full border border-white bg-amber-400 text-[8px] font-black leading-none text-amber-950 shadow-sm [html[data-theme=dark]_&]:border-slate-900">!</span> : null}</button>
        </div>
