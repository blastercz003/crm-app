    ...
    <div className="mt-3 flex min-h-0 flex-1 flex-col justify-between lg:mt-4 lg:block lg:space-y-2">
      {sourceCards.map((card) => {
        if (card.kind === 'cez-new') {
          ...
          return <div key="cez-new" className={`weather-alerts__record-surface relative rounded-xl border px-2.5 py-1 lg:px-3 lg:py-2 ${compact ? 'h-[62px] lg:h-[90px]' : 'h-[70px] lg:h-[94px]'}`}>
            <div className="flex items-center justify-between gap-2"><strong className="text-[13px] text-[var(--text-primary)]">{shadowActive ? 'ČEZ' : 'ČEZ – NEW'}</strong><button type="button" onClick={() => setShowCezNew(true)} className={`relative inline-flex h-6 w-[98px] items-center justify-center gap-1 rounded-lg border px-1.5 text-center text-[7px] font-bold uppercase leading-none tracking-[0.055em] transition hover:-translate-y-px lg:h-8 lg:w-[104px] lg:gap-1.5 lg:rounded-xl lg:px-2 lg:text-[8px] lg:tracking-[0.065em] ${presentation.badge}`} title={state.statusMessage} aria-label={`${shadowActive ? 'ČEZ' : 'ČEZ – NEW'}: ${cezNewStatusLabel(state.status)}. ${state.statusMessage} Zobrazit detail.`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${presentation.dot}`} /><span className="translate-y-px">{cezNewStatusLabel(state.status)}</span>{presentation.attention || hasWarning ? <span className="absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-amber-400 text-[8px] font-black leading-none text-amber-950 shadow-sm lg:-right-1.5 lg:-top-1.5 lg:h-4 lg:w-4 lg:text-[9px]">!</span> : null}</button></div>
        ...
        return <div key={source.source} className={`weather-alerts__record-surface relative rounded-xl border px-2.5 py-1 lg:px-3 lg:py-2 ${compact ? 'h-[62px] lg:h-[90px]' : 'h-[70px] lg:h-[94px]'}`}>
...
function DiscoveryPanel({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  ...
  const loadDiagnostic = async (provider: CompleteProviderState['provider'], showLoading = true) => {
    if (showLoading) setLoading(true)
    setError(null)
    const result = await getCompletePowerOutageProviderDiagnosticAction(
      provider,
      workspace.providers.find((item) => item.provider === provider),
    )
    if (result.success) setDiagnostic(result.diagnostic)
    else setError(result.error)
    setLoading(false)
