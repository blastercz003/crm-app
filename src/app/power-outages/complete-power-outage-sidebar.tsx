...
import { Activity, CircleAlert, CircleCheck, DatabaseZap, History, Info, LoaderCircle, MapPinned, RefreshCw, Route, SearchCheck, TimerReset } from 'lucide-react'
import { useEffect, useState } from 'react'
import { createPortal } from 'react-dom'
import type { CompleteAddressCoverage, CompleteAddressCoverageDiagnostic, CompleteCezNewState, CompleteGlobalProgress, CompletePowerOutageSidebarWorkspace, CompleteProviderDiagnostic, CompleteProviderState, CompleteSourceDiagnostic, CompleteSourceState } from '@/lib/power-outages/complete-types'
...
export function CompletePowerOutageSidebar({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  return <div className="-mt-2 min-w-0 lg:mt-0 xl:h-full"><p className="mb-2 flex items-center justify-center gap-1.5 text-center text-[10px] text-[var(--text-secondary)] lg:hidden"><Info aria-hidden size={12} /><span>Přejetím do strany zobrazíte další část.</span></p><aside className="power-outages-mobile-aside-carousel grid min-w-0 auto-cols-[100%] grid-flow-col items-stretch gap-3 snap-x snap-mandatory overflow-x-auto overflow-y-hidden overscroll-x-contain rounded-[24px] lg:block lg:space-y-4 lg:overflow-visible lg:rounded-none" aria-label="Stav kompletního sběru a vyhledávání firem"><div className="min-w-0 snap-start snap-always lg:snap-none"><GlobalProgressPanel workspace={workspace} onRetry={onRetry} /></div><div className="min-w-0 snap-start snap-always lg:snap-none"><SourcesPanel workspace={workspace} onRetry={onRetry} /></div><div className="min-w-0 snap-start snap-always lg:snap-none"><DiscoveryPanel workspace={workspace} onRetry={onRetry} /></div><div className="min-w-0 snap-start snap-always lg:snap-none"><CoveragePanel workspace={workspace} onRetry={onRetry} /></div></aside></div>
}
