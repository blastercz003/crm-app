...
function CompleteCezNewDiagnosticPopup({ state, source, isAdmin, onReload, onClose }: { state: CompleteCezNewState; source: CompleteSourceState | null; isAdmin: boolean; onReload: () => void | Promise<void>; onClose: () => void }) {
  ...
  const phases = [
    ...
    { label: 'Celoplošný sken', done: state.scanProcessed, total: state.scanTotal, error: state.scanReviewErrorCount, warning: state.scanRetryableErrorCount, detail: `${state.scanOutageCount.toLocaleString('cs-CZ')} odstávek · ${state.scanAddressCount.toLocaleString('cs-CZ')} adres` },
