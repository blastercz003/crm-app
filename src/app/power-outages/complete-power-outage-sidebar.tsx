...
function CompleteCezNewDiagnosticPopup({ state, source, isAdmin, onReload, onClose }: { state: CompleteCezNewState; source: CompleteSourceState | null; isAdmin: boolean; onReload: () => void | Promise<void>; onClose: () => void }) {
  ...
  return <PowerOutagePopupShell titleId="complete-cez-new-status" eyebrow={`KOMPLETNÍ SBĚR · ${shadowActive ? 'CELOPLOŠNÝ ZDROJ' : 'NOVÝ CELOPLOŠNÝ ZDROJ'}`} title={shadowActive ? <CezAllV1Label /> : 'ČEZ – NEW'} icon={<DatabaseZap aria-hidden size={21} />} onClose={onClose}>
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
      ...
      <section className="mt-4"><h3 className="text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Produkční zpracování dat</h3><div className="mt-2 grid gap-2 sm:grid-cols-2">
      ...
      <section className="mt-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-4">
        <div className="flex items-start justify-between gap-3"><span><small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--text-secondary)]">3–5 · Vyhledávání a vyhodnocení firem · 30 dnů</small><strong className="mt-1 block text-sm text-[var(--text-primary)]">{discovery ? `${discovery.completedTargetCount.toLocaleString('cs-CZ')} / ${discovery.totalTargetCount.toLocaleString('cs-CZ')} adresních dotazů` : 'Průběh zatím není dostupný'}</strong><p className="mt-1 max-w-lg text-[9px] leading-4 text-[var(--text-secondary)]">{discovery?.statusMessage ?? 'Čekám na provozní snapshot navazujících firemních front.'}</p></span><span className={`relative inline-flex h-8 w-[108px] shrink-0 items-center justify-center gap-1.5 rounded-xl border px-2 text-[8px] font-bold uppercase leading-none tracking-[0.06em] ${discoveryPresentation.badge}`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${discoveryPresentation.dot}`} />{discoveryStatusLabel(discovery?.status ?? 'waiting')}</span></div>
        {discovery ? <div className="mt-3 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] p-3"><strong className="block text-[10px] text-[var(--text-primary)]">3 · ARES</strong><small className="text-[7px] font-semibold uppercase tracking-[0.06em] text-[var(--text-secondary)]">Přesná čísla</small><p className="mt-2 text-[8px] text-[var(--text-secondary)]">Hotovo {Math.max(0, discovery.exactTargetCount - discovery.exactPendingTargetCount - discovery.exactErrorTargetCount)} / {discovery.exactTargetCount} · problémy {discovery.exactErrorTargetCount}</p>{discovery.exactErrorTargetCount > 0 || (discovery.status === 'delayed' && discovery.exactPendingTargetCount > 0) ? <RecoveryControl isAdmin={isAdmin} blocked={false} label="Spustit bezpečnou dávku ARES" request={{ target: 'provider_discovery', provider: 'ares' }} onRecovered={onReload} /> : null}</div><div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] p-3"><strong className="block text-[10px] text-[var(--text-primary)]">4 · Mapy.com</strong><small className="text-[7px] font-semibold uppercase tracking-[0.06em] text-[var(--text-secondary)]">Uliční cíle</small><p className="mt-2 text-[8px] text-[var(--text-secondary)]">Hotovo {Math.max(0, discovery.streetTargetCount - discovery.streetPendingTargetCount - discovery.streetErrorTargetCount)} / {discovery.streetTargetCount} · problémy {discovery.streetErrorTargetCount}</p>{discovery.streetErrorTargetCount > 0 || (discovery.status === 'delayed' && discovery.streetPendingTargetCount > 0) ? <RecoveryControl isAdmin={isAdmin} blocked={false} label="Spustit bezpečnou dávku Mapy.com" request={{ target: 'provider_discovery', provider: 'mapy' }} onRecovered={onReload} /> : null}</div><div className="rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)] p-3"><div className="flex items-start justify-between gap-2"><span><strong className="block text-[10px] text-[var(--text-primary)]">5 · Vyhodnocení</strong><small className="text-[7px] font-semibold uppercase tracking-[0.06em] text-[var(--text-secondary)]">Kandidáti ČEZ ze všech providerů</small></span><strong className="text-xs tabular-nums text-[var(--accent)]">{evaluationPercent.toLocaleString('cs-CZ')} %</strong></div><p className="mt-2 text-[8px] text-[var(--text-secondary)]">{discovery.evaluatedCandidateCount.toLocaleString('cs-CZ')} / {discovery.candidateCount.toLocaleString('cs-CZ')} · zbývá {discovery.pendingCandidateCount.toLocaleString('cs-CZ')}</p>{discovery.status === 'error' && discovery.exactErrorTargetCount === 0 && discovery.streetErrorTargetCount === 0 ? <RecoveryControl isAdmin={isAdmin} blocked={false} label="Obnovit celkové vyhodnocení" request={{ target: 'company_reconciliation' }} onRecovered={onReload} /> : null}</div></div> : null}
        <p className="mt-2 text-[8px] leading-4 text-[var(--text-secondary)]">Ruční spuštění ARES, Mapy.com nebo vyhodnocení používá společnou bezpečnou frontu všech distributorů; existující zámek zabrání duplicitnímu běhu.</p>
      </section>
