...
import { Activity, CircleAlert, CircleCheck, DatabaseZap, History, Info, LoaderCircle, MapPinned, RefreshCw, Route, SearchCheck, Sparkles, TimerReset } from 'lucide-react'
...
function cezNewStageLabel(stage: CompleteCezNewState['stage'], active = false) {
  if (active) return ({ ruian: 'Aktualizace katalogu RÚIAN', mapping: 'Aktualizace území ČEZ', scan: 'Celoplošný sběr odstávek', normalization: 'Příprava zdrojových adres', projection: 'Produkční publikace', ready: 'Provoz je aktuální' } as const)[stage]
  ...
  return <PowerOutagePopupShell titleId="complete-cez-new-status" eyebrow={`KOMPLETNÍ SBĚR · ${shadowActive ? 'CELOPLOŠNÝ ZDROJ' : 'NOVÝ CELOPLOŠNÝ ZDROJ'}`} title={shadowActive ? <CezAllV1Label /> : 'ČEZ – NEW'} icon={<DatabaseZap aria-hidden size={21} />} onClose={onClose}>
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
      <section className="rounded-2xl border border-sky-400/25 bg-sky-500/8 p-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-3"><PowerOutageDetailRow label="Poslední dokončený cyklus" value={formatDate(state.scanFinishedAt)} /><PowerOutageDetailRow label="Poslední publikace" value={formatDate(state.lastProjectionAt)} /><PowerOutageDetailRow label="Aktivní zdroj" value={shadowActive ? 'ČEZ ALL v1' : 'Původní ČEZ'} /></div>
      </section>
      ...
      <section className="mt-4"><h3 className="text-[9px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Produkční zpracování dat</h3><div className="mt-2 grid gap-2 sm:grid-cols-2">
        <div className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-muted)] p-3"><div className="flex items-start justify-between gap-2"><span><strong className="block text-[10px] text-[var(--text-primary)]">1 · Publikace katalogu</strong><small className="mt-0.5 block text-[7px] text-[var(--text-secondary)]">Pouze úplný a bezpečný cyklus</small></span><strong className={`text-[8px] ${state.latestProjectionMatches && state.projectionStatus === 'ready' ? 'text-emerald-600 [html[data-theme=dark]_&]:text-emerald-300' : 'text-sky-600 [html[data-theme=dark]_&]:text-sky-300'}`}>{state.latestProjectionMatches && state.projectionStatus === 'ready' ? 'AKTUÁLNÍ' : 'ZPRACOVÁNÍ'}</strong></div><p className="mt-2 text-[8px] text-[var(--text-secondary)]">{state.projectedCurrentOutageCount.toLocaleString('cs-CZ')} aktuálních odstávek · {state.projectedAddressCount.toLocaleString('cs-CZ')} adres</p></div>
...
function CommercialSelectionPanel({ workspace, value, sort, counts, onChange, onSortChange, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; value: CompleteCommercialSelectionFilter; sort: CompleteCommercialSort; counts: CompleteCommercialSelectionCounts | null; onChange: (value: CompleteCommercialSelectionFilter) => void; onSortChange: (value: CompleteCommercialSort) => void; onRetry?: () => void }) {
  if (workspace.loadErrors.commercialSelection) return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10 text-red-500"><CircleAlert aria-hidden size={18} /></span><span><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Obchodní výběr</small><h3 className="text-base font-semibold text-[var(--text-primary)]">AI SELECT</h3></span></div><PanelLoadError message={workspace.loadErrors.commercialSelection} onRetry={onRetry} /></PanelShell>

  return <PanelShell>
    <div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-violet-500/10 text-violet-600 [html[data-theme=dark]_&]:text-violet-300"><Sparkles aria-hidden size={18} /></span><span className="min-w-0"><small className="block whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Obchodní výběr</small><h3 className="whitespace-nowrap text-base font-semibold text-[var(--text-primary)]">AI SELECT</h3></span></div><CompletePanelStatusBadge label="AKTIVNÍ" badgeClassName="border-emerald-400/35 bg-emerald-400/10 text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300" dotClassName="bg-emerald-500" /></div>
    <p className="mt-3 text-[8px] leading-4 text-[var(--text-secondary)]">Výběr pracuje lokálně nad vypočteným skóre. Ostatní filtry tabulky zůstávají dostupné.</p>
