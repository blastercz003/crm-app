...
function CoveragePanel({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  ...
  return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-600 [html[data-theme=dark]_&]:text-emerald-300"><MapPinned aria-hidden size={18} /></span><span><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Aktuální odstávky</small><h3 className="text-base font-semibold text-[var(--text-primary)]">Pokrytí adres</h3></span></div><button type="button" onClick={openDetail} className="mt-4 rounded-2xl border border-sky-400/25 bg-sky-500/8 p-3.5 text-left transition hover:-translate-y-px hover:border-sky-400/45 hover:bg-sky-500/10" aria-label="Otevřít detail pokrytí adres"><div className="flex items-end justify-between gap-3"><span><small className="flex items-center gap-1.5 text-[7px] font-bold uppercase text-[var(--text-secondary)]"><i className={`h-1.5 w-1.5 rounded-full ${presentation.dot}`} />Připraveno pro hledání</small><strong className="mt-1 block text-[10px] text-[var(--text-primary)]">{coverage.searchableCount} z {coverage.progressTotalCount}</strong></span><span className="flex items-end gap-2"><CompletePanelStatusBadge label={addressCoverageStatusLabel(coverage.status)} badgeClassName={presentation.badge} dotClassName={presentation.dot} attention={presentation.attention} /><strong className="text-2xl tabular-nums text-[var(--accent)]">{percent.toLocaleString('cs-CZ')} %</strong></span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[var(--surface-border)]"><div className="h-full rounded-full bg-sky-500" style={{ width: `${Math.min(100, percent)}%` }} /></div></button><div className="mt-3 grid grid-cols-2 gap-1.5 text-center"><span className="rounded-xl border border-emerald-400/25 bg-emerald-400/8 p-2 text-[6px] font-bold uppercase text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300">Přesné<strong className="mt-1 block text-sm text-[var(--text-primary)]">{coverage.exactCount}</strong></span><span className="rounded-xl border border-amber-400/25 bg-amber-400/8 p-2 text-[6px] font-bold uppercase text-amber-700 [html[data-theme=dark]_&]:text-amber-300">Pouze ulice<strong className="mt-1 block text-sm text-[var(--text-primary)]">{coverage.streetCount}</strong></span><span className="rounded-xl border border-orange-400/25 bg-orange-400/8 p-2 text-[6px] font-bold uppercase text-orange-700 [html[data-theme=dark]_&]:text-orange-300">K pozornosti<strong className="mt-1 block text-sm text-[var(--text-primary)]">{attentionCount}</strong></span><span className="rounded-xl border border-slate-400/25 bg-slate-400/8 p-2 text-[6px] font-bold uppercase text-slate-600 [html[data-theme=dark]_&]:text-slate-300">Čeká<strong className="mt-1 block text-sm text-[var(--text-primary)]">{coverage.pendingCount}</strong></span></div><p className="mt-2 text-center text-[7px] leading-3 text-[var(--text-secondary)]">Pouze obec mimo vyhledávání: {coverage.municipalityCount}</p>{showDetail && typeof document !== 'undefined' ? createPortal(<AddressCoverageDiagnosticPopup diagnostic={diagnostic} loading={loading} error={error} isAdmin={workspace.currentUser.isAdmin} onReload={() => loadDiagnostic(false)} onClose={() => { setShowDetail(false); setDiagnostic(null); setError(null) }} />, document.body) : null}</PanelShell>
}
