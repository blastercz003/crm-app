  ...
  return <PowerOutagePopupShell titleId="complete-cez-new-status" eyebrow="KOMPLETNÍ SBĚR · NOVÝ CELOPLOŠNÝ ZDROJ" title={state.activeSource === 'shadow' ? 'ČEZ' : 'ČEZ – NEW'} icon={<DatabaseZap aria-hidden size={21} />} onClose={onClose}>
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] sm:p-5">
      <section className="rounded-2xl border border-sky-400/25 bg-sky-500/8 p-4">
        <div className="flex items-start justify-between gap-3"><span><small className="block text-[8px] font-bold uppercase tracking-[0.1em] text-[var(--text-secondary)]">Právě probíhá</small><strong className="mt-1 block text-base text-[var(--text-primary)]">{cezNewStageLabel(state.stage)}</strong><p className="mt-1 max-w-lg text-[9px] leading-4 text-[var(--text-secondary)]">{state.statusMessage}</p></span><span className={`relative inline-flex h-8 w-[108px] shrink-0 items-center justify-center gap-1.5 rounded-xl border px-2 text-[8px] font-bold uppercase leading-none tracking-[0.06em] ${presentation.badge}`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${presentation.dot}`} />{cezNewStatusLabel(state.status)}{presentation.attention || hasWarning ? <i className="absolute -right-1.5 -top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-amber-400 text-[9px] font-black not-italic text-amber-950">!</i> : null}</span></div>
      ...
      {sourceCards.map((card) => {
        if (card.kind === 'cez-new') {
          ...
          return <div key="cez-new" className={`weather-alerts__record-surface relative h-full rounded-xl border px-2.5 py-1 lg:px-3 lg:py-2 ${compact ? 'lg:h-[90px]' : 'lg:h-[94px]'}`}>
            <div className="flex items-center justify-between gap-2"><strong className="text-[13px] text-[var(--text-primary)]">{shadowActive ? 'ČEZ' : 'ČEZ – NEW'}</strong><button type="button" onClick={() => setShowCezNew(true)} className={`relative inline-flex h-8 w-[104px] items-center justify-center gap-1.5 rounded-xl border px-2 text-center text-[8px] font-bold uppercase leading-none tracking-[0.065em] transition hover:-translate-y-px ${presentation.badge}`} title={state.statusMessage} aria-label={`${shadowActive ? 'ČEZ' : 'ČEZ – NEW'}: ${cezNewStatusLabel(state.status)}. ${state.statusMessage} Zobrazit detail.`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${presentation.dot}`} /><span>{cezNewStatusLabel(state.status)}</span>{presentation.attention || hasWarning ? <span className="absolute -right-1.5 -top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-amber-400 text-[9px] font-black leading-none text-amber-950 shadow-sm">!</span> : null}</button></div>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={() => openSource(source.source)} className={`relative inline-flex h-8 w-[104px] items-center justify-center gap-1.5 rounded-xl border px-2 text-center text-[8px] font-bold uppercase leading-none tracking-[0.065em] transition hover:-translate-y-px ${presentation.badge}`} title={discovery.statusMessage} aria-label={`${sourceLabel(source.source)}: ${discoveryStatusLabel(discovery.status)}. ${discovery.statusMessage} Zobrazit provozní detail.`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${presentation.dot}`} /><span>{discoveryStatusLabel(discovery.status)}</span>{presentation.attention || discovery.errorTargetCount > 0 ? <span className="absolute -right-1.5 -top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-amber-400 text-[9px] font-black leading-none text-amber-950 shadow-sm">!</span> : null}</button>
        </div>
  ...
  return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-500/10 text-violet-600 [html[data-theme=dark]_&]:text-violet-300"><SearchCheck aria-hidden size={18} /></span><span><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">ARES · Mapy.com · Google</small><h3 className="text-base font-semibold text-[var(--text-primary)]">Vyhledávání firem</h3></span></div><div className="mt-3 grid min-h-0 flex-1 grid-rows-3 gap-2 lg:mt-4 lg:block lg:space-y-2.5">{workspace.providers.map((provider) => {
    ...
    const capacityText = provider.configured
      ? provider.provider === 'mapy'
        ? `Volných ${provider.monthlyCreditRemaining.toLocaleString('cs-CZ')} z ${provider.monthlyCreditSafetyCap.toLocaleString('cs-CZ')} kreditů`
...
function AddressCoverageDiagnosticPopup({ diagnostic, loading, error, isAdmin, onReload, onClose }: {
  ...
  const normalizationStalled = diagnostic?.task?.status === 'running'
    ...
    && new Date(diagnostic.task.lockExpiresAt!).getTime() <= Date.now()
...
function CoveragePanel({ workspace, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; onRetry?: () => void }) {
  ...
  return <PanelShell><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-600 [html[data-theme=dark]_&]:text-emerald-300"><MapPinned aria-hidden size={18} /></span><span><small className="block text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Aktuální odstávky</small><h3 className="text-base font-semibold text-[var(--text-primary)]">Pokrytí adres</h3></span></div><button type="button" onClick={openDetail} className="mt-4 rounded-2xl border border-sky-400/25 bg-sky-500/8 p-3.5 text-left transition hover:-translate-y-px hover:border-sky-400/45 hover:bg-sky-500/10" aria-label="Otevřít detail pokrytí adres"><div className="flex items-end justify-between gap-3"><span><small className="flex items-center gap-1.5 text-[7px] font-bold uppercase text-[var(--text-secondary)]"><i className={`h-1.5 w-1.5 rounded-full ${presentation.dot}`} />Zpracováno</small><strong className="mt-1 block text-[10px] text-[var(--text-primary)]">{coverage.normalizedCount} z {coverage.totalCount}</strong></span><span className="flex items-end gap-2"><small className={`relative mb-0.5 inline-flex h-7 w-[92px] items-center justify-center gap-1 rounded-xl border px-2 text-[7px] font-bold uppercase leading-none tracking-[0.06em] ${presentation.badge}`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${presentation.dot}`} /><span>{addressCoverageStatusLabel(coverage.status)}</span>{presentation.attention ? <i className="absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-amber-400 text-[8px] font-black not-italic text-amber-950">!</i> : null}</small><strong className="text-2xl tabular-nums text-[var(--accent)]">{percent.toLocaleString('cs-CZ')} %</strong></span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[var(--surface-border)]"><div className="h-full rounded-full bg-sky-500" style={{ width: `${Math.min(100, percent)}%` }} /></div></button><div className="mt-3 grid grid-cols-2 gap-1.5 text-center"><span className="rounded-xl border border-emerald-400/25 bg-emerald-400/8 p-2 text-[6px] font-bold uppercase text-emerald-700 [html[data-theme=dark]_&]:text-emerald-300">Přesné<strong className="mt-1 block text-sm text-[var(--text-primary)]">{coverage.exactCount}</strong></span><span className="rounded-xl border border-amber-400/25 bg-amber-400/8 p-2 text-[6px] font-bold uppercase text-amber-700 [html[data-theme=dark]_&]:text-amber-300">Širší rozsah<strong className="mt-1 block text-sm text-[var(--text-primary)]">{coverage.broadCount}</strong></span><span className="rounded-xl border border-orange-400/25 bg-orange-400/8 p-2 text-[6px] font-bold uppercase text-orange-700 [html[data-theme=dark]_&]:text-orange-300">K pozornosti<strong className="mt-1 block text-sm text-[var(--text-primary)]">{attentionCount}</strong></span><span className="rounded-xl border border-slate-400/25 bg-slate-400/8 p-2 text-[6px] font-bold uppercase text-slate-600 [html[data-theme=dark]_&]:text-slate-300">Čeká<strong className="mt-1 block text-sm text-[var(--text-primary)]">{coverage.pendingCount}</strong></span></div>{showDetail && typeof document !== 'undefined' ? createPortal(<AddressCoverageDiagnosticPopup diagnostic={diagnostic} loading={loading} error={error} isAdmin={workspace.currentUser.isAdmin} onReload={() => loadDiagnostic(false)} onClose={() => { setShowDetail(false); setDiagnostic(null); setError(null) }} />, document.body) : null}</PanelShell>
}
