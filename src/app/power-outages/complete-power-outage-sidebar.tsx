...
import { CircleAlert, CircleCheck, DatabaseZap, History, Info, LoaderCircle, MapPinned, Route, SearchCheck, TimerReset } from 'lucide-react'
...
function CompleteSourceDiagnosticPopup({ source, diagnostic, loading, error, onClose }: {
...
function SourcesPanel({ workspace }: { workspace: CompletePowerOutageWorkspace }) {
  ...
  useEffect(() => {
    if (!selectedSource || !discoveryPollingStatus || !['waiting', 'processing', 'delayed'].includes(discoveryPollingStatus)) return
  ...
  return <PanelShell>
    ...
    <div className="mt-3 space-y-1">
      {workspace.sources.map((source) => {
        ...
        return <div key={source.source} className="weather-alerts__record-surface h-[70px] rounded-xl border px-2.5 py-1">
        <div className="flex items-center justify-between gap-2">
          <strong className="text-[13px] text-[var(--text-primary)]">{sourceLabel(source.source)}</strong>
          <button type="button" onClick={() => openSource(source.source)} className={`relative inline-flex h-6 w-[98px] items-center justify-center gap-1 rounded-lg border px-1.5 text-center text-[7px] font-bold uppercase leading-none tracking-[0.055em] transition hover:-translate-y-px ${presentation.badge}`} title={discovery.statusMessage} aria-label={`${sourceLabel(source.source)}: ${discoveryStatusLabel(discovery.status)}. ${discovery.statusMessage} Zobrazit provozní detail.`}><i className={`h-1.5 w-1.5 shrink-0 rounded-full ${presentation.dot}`} /><span className="translate-y-px">{discoveryStatusLabel(discovery.status)}</span>{presentation.attention || discovery.errorTargetCount > 0 ? <span className="absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full border border-white bg-amber-400 text-[8px] font-black leading-none text-amber-950 shadow-sm [html[data-theme=dark]_&]:border-slate-900">!</span> : null}</button>
        </div>
        <div className="mt-0.5 grid grid-cols-2 gap-1.5">
          <span className="weather-alerts__record-surface flex h-5 items-center justify-between rounded-lg border px-2"><small className="text-[6px] font-bold uppercase leading-[8px] tracking-[0.035em] text-[var(--text-secondary)]">Prověřeno</small><strong className="text-[10px] tabular-nums text-[var(--text-primary)]">{discovery.completedTargetCount}/{discovery.totalTargetCount}</strong></span>
          <span className="weather-alerts__record-surface flex h-5 items-center justify-between rounded-lg border px-2"><small className="text-[6px] font-bold uppercase leading-[8px] tracking-[0.035em] text-[var(--text-secondary)]">Zbývá</small><strong className="text-[10px] tabular-nums text-[var(--text-primary)]">{discovery.remainingTargetCount}</strong></span>
        </div>
        <div className="mt-1 flex items-center gap-1.5"><span className="h-1.5 min-w-0 flex-1 overflow-hidden rounded-full bg-[var(--surface-border)]"><i className="block h-full rounded-full bg-sky-500 transition-[width] duration-700" style={{ width: `${Math.min(100, discovery.progressPercent)}%` }} /></span><strong className="w-7 text-right text-[7px] tabular-nums text-[var(--accent)]">{discovery.progressPercent.toLocaleString('cs-CZ')}%</strong><small className="w-[72px] truncate text-right text-[6px] text-[var(--text-secondary)]">{formatRelativeDate(discovery.lastProgressAt)}</small></div>
      </div>})}
    </div>
    {selectedSource && typeof document !== 'undefined' ? createPortal(<CompleteSourceDiagnosticPopup source={selectedSource} diagnostic={diagnostic} loading={loading} error={error} onClose={() => { setSelectedSource(null); setDiagnostic(null); setError(null) }} />, document.body) : null}
  </PanelShell>
}

function CompleteProviderDiagnosticPopup({ provider, diagnostic, loading, error, onClose }: {
...
function DiscoveryPanel({ workspace }: { workspace: CompletePowerOutageWorkspace }) {
  ...
  })}</div><div className={`mt-auto flex items-center justify-center gap-1.5 pt-3 text-center text-[9px] ${runtime.status === 'attention' ? 'text-red-700 [html[data-theme=dark]_&]:text-red-300' : 'text-[var(--text-secondary)]'}`} title={runtime.issues.join('\n')}><RuntimeIcon aria-hidden size={12} className={runtime.status === 'processing' ? 'animate-spin' : ''} /><span className="truncate">{runtimeText}</span></div><p className="mt-0.5 flex items-center justify-center gap-1.5 text-center text-[8px] text-[var(--text-secondary)]"><TimerReset aria-hidden size={11} /><span>Poslední aktivita: {formatDate(runtime.lastActivityAt)}</span></p>{selectedProvider && typeof document !== 'undefined' ? createPortal(<CompleteProviderDiagnosticPopup provider={selectedProvider} diagnostic={diagnostic} loading={loading} error={error} onClose={() => { setSelectedProvider(null); setDiagnostic(null); setError(null) }} />, document.body) : null}</PanelShell>
}

function AddressCoverageDiagnosticPopup({ diagnostic, loading, error, onClose }: {
