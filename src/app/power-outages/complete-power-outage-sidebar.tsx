    ...
    <div className="mt-3 flex min-h-0 flex-1 flex-col justify-between lg:mt-4 lg:block lg:space-y-2">
      {sourceCards.map((card) => {
        if (card.kind === 'cez-new') {
          ...
          return <div key="cez-new" className={`weather-alerts__record-surface relative rounded-xl border px-2.5 py-1 lg:px-3 lg:py-2 ${compact ? 'h-[62px] lg:h-[78px]' : 'h-[70px] lg:h-[94px]'}`}>
            ...
            <div className="absolute bottom-1 left-2.5 right-2.5 flex items-center gap-1.5 lg:left-3 lg:right-3 lg:gap-2"><span className="h-1.5 min-w-0 flex-1 overflow-hidden rounded-full bg-[var(--surface-border)]"><i className="block h-full rounded-full bg-sky-500 transition-[width] duration-700" style={{ width: `${Math.min(100, state.progressPercent)}%` }} /></span><strong className="w-8 text-right text-[7px] tabular-nums text-[var(--accent)]">{state.progressPercent.toLocaleString('cs-CZ')}%</strong><small className="w-[72px] truncate text-right text-[6px] text-[var(--text-secondary)]">{cezNewStageLabel(state.stage)}</small></div>
          </div>
        }
        const source = card.source
        const discovery = source.discovery
        const presentation = discoveryStatusPresentation(discovery.status)
        return <div key={source.source} className={`weather-alerts__record-surface relative rounded-xl border px-2.5 py-1 lg:px-3 lg:py-2 ${compact ? 'h-[62px] lg:h-[78px]' : 'h-[70px] lg:h-[94px]'}`}>
