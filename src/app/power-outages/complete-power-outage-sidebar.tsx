...
function CommercialSelectionPanel({ workspace, value, onChange, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; value: CompleteCommercialSelectionFilter; onChange: (value: CompleteCommercialSelectionFilter) => void; onRetry?: () => void }) {
  ...
  return <PanelShell>
    ...
    <div className="mt-3 grid grid-cols-2 gap-2" role="group" aria-label="Obchodní výběr firem">{COMMERCIAL_SELECTION_OPTIONS.map((option) => <button key={option.value} type="button" onClick={() => onChange(option.value)} aria-pressed={value === option.value} title={option.description} className={`flex h-12 flex-col items-center justify-center rounded-xl border px-2 text-center transition hover:-translate-y-px ${value === option.value ? 'border-violet-400/45 bg-violet-500/12 text-violet-700 shadow-sm [html[data-theme=dark]_&]:text-violet-200' : 'border-[var(--surface-border)] bg-[var(--surface-muted)] text-[var(--text-secondary)] hover:border-violet-400/30 hover:text-[var(--text-primary)]'}`}><strong className="text-[8px] font-extrabold uppercase tracking-[0.055em]">{option.label}</strong><small className="mt-1 line-clamp-1 text-[6px] font-medium normal-case tracking-normal opacity-80">{option.description}</small></button>)}</div>
  </PanelShell>
}

export function CompletePowerOutageSidebar({ workspace, commercialSelection, onCommercialSelectionChange, onRetry }: { workspace: CompletePowerOutageSidebarWorkspace; commercialSelection: CompleteCommercialSelectionFilter; onCommercialSelectionChange: (value: CompleteCommercialSelectionFilter) => void; onRetry?: () => void }) {
  return <div className="-mt-2 min-w-0 lg:mt-0 xl:h-full">
    ...
    <aside className="power-outages-mobile-aside-carousel grid min-w-0 auto-cols-[100%] grid-flow-col items-stretch gap-3 snap-x snap-mandatory overflow-x-auto overflow-y-hidden overscroll-x-contain rounded-[24px] lg:block lg:space-y-4 lg:overflow-visible lg:rounded-none" aria-label="Stav kompletního sběru a vyhledávání firem">
      {workspace.commercialSelection.enabled ? <div className="min-w-0 snap-start snap-always lg:snap-none"><CommercialSelectionPanel workspace={workspace} value={commercialSelection} onChange={onCommercialSelectionChange} onRetry={onRetry} /></div> : null}
