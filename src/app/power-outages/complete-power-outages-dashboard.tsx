...
function SidebarSkeleton({ error, onRetry }: { error?: string | null; onRetry?: () => void }) {
  const panelClass = 'activities-page__panel flex h-[380px] min-w-0 flex-col rounded-[24px] border border-white/70 bg-[linear-gradient(155deg,rgba(255,255,255,0.96)_0%,rgba(248,250,252,0.92)_100%)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_16px_36px_rgba(15,23,42,0.1)] backdrop-blur-[10px] sm:p-5 lg:h-auto lg:min-h-[210px] lg:p-4'
  ...
  const loadingBadge = <span className="inline-flex h-6 w-[98px] items-center justify-center gap-1 rounded-lg border border-sky-400/30 bg-sky-500/10 text-[7px] font-bold uppercase tracking-[0.055em] text-[var(--accent)] lg:h-8 lg:w-[104px] lg:gap-1.5 lg:rounded-xl lg:text-[8px] lg:tracking-[0.06em]"><LoaderCircle aria-hidden size={11} className="animate-spin" /> Načítám</span>
  const sourceCard = (name: string, newCez = false) => <div key={name} className="weather-alerts__record-surface relative h-[66px] rounded-xl border px-2.5 py-1 lg:h-[94px] lg:px-3 lg:py-2">
    ...
    <div className="mt-0.5 grid grid-cols-2 gap-1.5 lg:mt-1 lg:gap-2"><span className="weather-alerts__record-surface flex h-5 items-center justify-between rounded-lg border px-2 lg:h-6 lg:px-2.5"><small className="text-[6px] font-bold uppercase text-[var(--text-secondary)]">{newCez ? 'Hotovo' : 'Prověřeno'}</small><LoaderCircle aria-label={`Načítám stav ${name}`} size={10} className="animate-spin text-[var(--accent)]" /></span><span className="weather-alerts__record-surface flex h-5 items-center justify-between rounded-lg border px-2 lg:h-6 lg:px-2.5"><small className="text-[6px] font-bold uppercase text-[var(--text-secondary)]">Zbývá</small><span className="text-[10px] font-bold text-[var(--text-secondary)]">—</span></span></div>
  ...
  const providerCard = (name: string) => <div key={name} className="weather-alerts__record-surface h-[80px] rounded-xl border px-3 py-2 lg:h-[108px]">
    <div className="flex items-center justify-between gap-2"><strong className="text-[13px] text-[var(--text-primary)]">{name}</strong>{loadingBadge}</div>
    <div className="mt-1.5 grid grid-cols-4 gap-1"><span className="rounded-lg border border-[var(--surface-border)] py-1 text-center"><small className="block text-[5px] font-bold uppercase text-emerald-700">Nalezeno</small><span className="text-[10px] font-bold text-[var(--text-secondary)]">—</span></span><span className="rounded-lg border border-[var(--surface-border)] py-1 text-center"><small className="block text-[5px] font-bold uppercase text-[var(--text-secondary)]">Čeká</small><LoaderCircle aria-label={`Načítám data ${name}`} size={9} className="mx-auto mt-0.5 animate-spin text-[var(--accent)]" /></span><span className="rounded-lg border border-[var(--surface-border)] py-1 text-center"><small className="block text-[5px] font-bold uppercase text-amber-700">Bez nálezu</small><span className="text-[10px] font-bold text-[var(--text-secondary)]">—</span></span><span className="rounded-lg border border-[var(--surface-border)] py-1 text-center"><small className="block text-[5px] font-bold uppercase text-red-600">Chyby</small><span className="text-[10px] font-bold text-[var(--text-secondary)]">—</span></span></div>
    <p className="mt-1.5 text-center text-[7px] text-[var(--text-secondary)]">Načítám dostupnou kapacitu…</p>
  </div>
  return <div className="-mt-2 min-w-0 lg:mt-0 xl:h-full">
    ...
    <aside className="power-outages-mobile-aside-carousel grid min-w-0 auto-cols-[100%] grid-flow-col items-stretch gap-3 snap-x snap-mandatory overflow-x-auto overflow-y-hidden rounded-[24px] lg:block lg:space-y-4 lg:overflow-visible lg:rounded-none">
      <div className="min-w-0 snap-start snap-always lg:snap-none"><section className={panelClass}>
        ...
        <div className="mt-3 flex min-h-0 flex-1 flex-col justify-between gap-1 lg:mt-4 lg:block lg:space-y-2">{sourceCard('ČEZ')}{sourceCard('ČEZ – NEW', true)}{sourceCard('EG.D')}{sourceCard('PRE')}</div>
      ...
      <div className="min-w-0 snap-start snap-always lg:snap-none"><section className={panelClass}>
        ...
        <div className="mt-3 flex min-h-0 flex-1 flex-col justify-between gap-1 lg:mt-4 lg:block lg:space-y-2.5">{providerCard('ARES')}{providerCard('GOOGLE')}{providerCard('MAPY.COM')}</div>
      ...
      <div className="min-w-0 snap-start snap-always lg:snap-none"><section className={panelClass}>
        ...
        <div className="mt-3 grid grid-cols-3 gap-1.5">{['Přesné', 'Širší rozsah', 'Čeká'].map((label) => <span key={label} className="flex h-12 flex-col items-center justify-center rounded-xl border border-[var(--surface-border)] text-center"><small className="text-[6px] font-bold uppercase text-[var(--text-secondary)]">{label}</small><LoaderCircle aria-label={`Načítám ${label.toLowerCase()}`} size={11} className="mt-1 animate-spin text-[var(--accent)]" /></span>)}</div>
      </section></div>
    </aside>
  </div>
}
