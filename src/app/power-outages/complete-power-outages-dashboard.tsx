  ...
  return <div className="-mt-2 min-w-0 lg:mt-0 xl:h-full">
    ...
    <aside className="power-outages-mobile-aside-carousel grid min-w-0 auto-cols-[100%] grid-flow-col items-stretch gap-3 snap-x snap-mandatory overflow-x-auto overflow-y-hidden rounded-[24px] lg:block lg:space-y-4 lg:overflow-visible lg:rounded-none">
      <div className="min-w-0 snap-start snap-always lg:snap-none"><section className={panelClass}>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">{['TOP VÝBĚR', 'VELKÉ FIRMY', 'POUZE A', 'POUZE B', 'VŠECHNY', 'PROVOZNĚ CITLIVÉ'].map((label) => <span key={label} className="flex h-[58px] animate-pulse flex-col items-center justify-center rounded-xl border border-[var(--surface-border)] bg-[var(--surface-muted)] text-[8px] font-bold uppercase text-[var(--text-secondary)]"><span>{label}</span><i className="mt-1 block h-1.5 w-12 rounded-full bg-[var(--surface-border)]" /></span>)}</div>
...
export function CompletePowerOutagesDashboard({ currentUser }: { currentUser: CompletePowerOutageCurrentUser }) {
  ...
  return <>
    ...
    <div className="grid items-start gap-5 xl:grid-cols-4 xl:items-stretch xl:gap-3">
      <CompletePowerOutageRecords currentUser={currentUser} owners={owners} commercialSelection={commercialSelection} commercialSort={commercialSort} clientsOnly={clientsOnly} commercialCountsEnabled={sidebar?.commercialSelection.countsAndSortingEnabled === true} onCommercialSelectionChange={setCommercialSelection} onClientsOnlyChange={(nextValue) => { setClientsOnly(nextValue); if (nextValue) setCommercialSelection('all') }} onCommercialSelectionCountsChange={setCommercialSelectionCounts} />
      {sidebar ? <CompletePowerOutageSidebar workspace={sidebar} sourceRefreshWarning={sidebarError} commercialSelection={commercialSelection} commercialSort={commercialSort} clientsOnly={clientsOnly} commercialSelectionCounts={commercialSelectionCounts} onCommercialSelectionChange={setCommercialSelection} onCommercialSortChange={setCommercialSort} onClientsOnlyChange={(nextValue) => { setClientsOnly(nextValue); if (nextValue) setCommercialSelection('all') }} onRetry={() => void loadSidebar()} /> : <SidebarSkeleton error={sidebarError} isAdmin={currentUser.isAdmin} onRetry={() => void loadSidebar()} />}
    </div>
  </>
}
