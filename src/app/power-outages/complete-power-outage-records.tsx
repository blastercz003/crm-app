...
function RecordsLoadingState() {
  return <div className="relative min-h-[300px] overflow-hidden"><div className="hidden lg:block"><table className="w-full table-fixed border-collapse text-left"><thead className="power-outages-table__header"><tr className="text-[8px] font-bold uppercase tracking-[0.09em] text-[var(--text-secondary)]"><th className="w-[17%] px-3 py-2.5">Firma / subjekt</th><th className="w-[16%] px-3 py-2.5">Adresa</th><th className="w-[8%] px-3 py-2.5">Distributor</th><th className="w-[14%] px-3 py-2.5">Termín od</th><th className="w-[14%] px-3 py-2.5">Termín do</th><th className="w-[22%] px-3 py-2.5">Stav</th><th className="w-[9%] px-3 py-2.5 text-right">Akce</th></tr></thead><tbody className="divide-y divide-[var(--surface-border)]">{Array.from({ length: 5 }, (_, row) => <tr key={row}>{[17, 16, 8, 14, 14, 22, 9].map((width, cell) => <td key={cell} style={{ width: `${width}%` }} className="px-3 py-3"><span className={`block h-8 animate-pulse rounded-lg bg-[var(--surface-muted)] ${cell === 2 || cell === 6 ? 'max-w-12' : ''}`} /></td>)}</tr>)}</tbody></table></div><div className="grid gap-2.5 p-2 lg:hidden">{Array.from({ length: 3 }, (_, index) => <div key={index} className="h-[126px] animate-pulse rounded-[18px] border border-[var(--surface-border)] bg-[var(--surface-muted)]" />)}</div><span className="pointer-events-none absolute left-1/2 top-1/2 z-10 inline-flex -translate-x-1/2 -translate-y-1/2 items-center gap-2 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-strong)]/95 px-4 py-3 text-xs font-semibold text-[var(--text-secondary)] shadow-lg backdrop-blur"><LoaderCircle aria-hidden size={18} className="animate-spin text-[var(--accent)]" /> Načítám výsledky…</span></div>
}
