...
function RecordsLoadingState() {
  ...
  return <div className="relative min-h-[300px] overflow-hidden" aria-busy="true" aria-label="Načítám výsledky">
    <div className="hidden lg:block"><table className="w-full table-fixed border-collapse text-left"><thead className="power-outages-table__header"><tr className="text-[8px] font-bold uppercase tracking-[0.09em] text-[var(--text-secondary)]"><th className="w-[17%] px-3 py-2.5">Firma / subjekt</th><th className="w-[16%] px-3 py-2.5">Adresa</th><th className="w-[8%] px-3 py-2.5">Distributor</th><th className="w-[14%] px-3 py-2.5">Termín od</th><th className="w-[14%] px-3 py-2.5">Termín do</th><th className="w-[22%] px-3 py-2.5">Stav</th><th className="w-[9%] px-3 py-2.5 text-right">Akce</th></tr></thead><tbody className="divide-y divide-[var(--surface-border)]">{Array.from({ length: 5 }, (_, row) => <tr key={row} className="h-14"><td className="px-3 py-2.5"><div className="flex items-center gap-2"><span className="h-8 w-8 shrink-0 animate-pulse rounded-lg bg-sky-500/10" /><span className="min-w-0 flex-1"><i className={`${line} h-2.5 w-4/5`} /><i className={`${line} mt-1.5 h-2 w-1/2`} /></span></div></td><td className="px-3 py-2.5"><i className={`${line} h-2.5 w-2/3`} /><i className={`${line} mt-1.5 h-2 w-5/6`} /></td><td className="px-3 py-2.5"><i className={`${line} h-6 w-11 rounded-lg`} /></td><td className="px-3 py-2.5"><i className={`${line} h-2.5 w-4/5`} /><i className={`${line} mt-1.5 h-2 w-1/2`} /></td><td className="px-3 py-2.5"><i className={`${line} h-2.5 w-4/5`} /><i className={`${line} mt-1.5 h-2 w-1/2`} /></td><td className="px-3 py-2.5"><i className={`${line} h-7 w-4/5 rounded-lg`} /></td><td className="px-3 py-2.5"><div className="flex justify-end gap-1.5"><i className={`${line} h-8 w-8 rounded-xl`} /><i className={`${line} h-8 w-8 rounded-xl`} /></div></td></tr>)}</tbody></table></div>
