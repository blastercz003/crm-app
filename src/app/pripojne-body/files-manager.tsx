...
import {
  deleteConnectionPointAttachmentsAction,
  getConnectionPointAttachmentDownloadLinksAction,
  getConnectionPointAttachmentPreviewUrlAction,
  uploadConnectionPointFilesAction,
  type ConnectionPointAttachmentCategory,
} from './actions'
...
type FileItem = {
  ...
  category: ConnectionPointAttachmentCategory
  note: string | null
  createdAt: string
}
...
function matchesQuery(file: FileItem, query: string) {
  ...
  return (
    ...
    file.siteAddress.toLowerCase().includes(q) ||
    (file.note ?? '').toLowerCase().includes(q)
  )
}

function matchesFolderQuery(folder: JobFolder, query: string) {
  if (!query) return true
  const q = query.toLowerCase()
  return (
    folder.jobNumber.toLowerCase().includes(q) ||
    folder.companyName.toLowerCase().includes(q) ||
    folder.siteAddress.toLowerCase().includes(q)
  )
}
...
export function FilesManager({ files, jobs, initialQuery, isAdmin }: FilesManagerProps) {
  ...
  const visibleFolders = useMemo(() => {
    const topLevelQuery = query.trim()
    const topFilteredFolders = topLevelQuery
      ? sortedFolders.filter((folder) => matchesFolderQuery(folder, topLevelQuery))
      : sortedFolders

    const normalized = foldersQuery.trim().toLowerCase()
    if (normalized.length < 3) return topFilteredFolders

    return topFilteredFolders.filter((folder) => {
      return (
        folder.jobNumber.toLowerCase().includes(normalized) ||
        folder.companyName.toLowerCase().includes(normalized)
      )
    })
  }, [sortedFolders, foldersQuery, query])
  ...
  const selectedVisibleFileId =
    selectedFileId && visibleFiles.some((file) => file.id === selectedFileId)
      ? selectedFileId
      : visibleFiles[0]?.id ?? null

  const selectedFile = selectedVisibleFileId ? filesById.get(selectedVisibleFileId) ?? null : null
  const previewForSelectedFile =
    selectedFile && previewTargetId === selectedFile.id ? previewUrl : null
  ...
  async function ensurePreview(fileId: string, mimeType: string | null) {
    ...
    const result = await getConnectionPointAttachmentPreviewUrlAction(fileId)
  ...
  function runDownload(ids: string[], zipName: string) {
    startTransition(async () => {
      const result = await getConnectionPointAttachmentDownloadLinksAction(ids)
      if (!result.success) {
  ...
  function runDirectDownload(id: string) {
    startTransition(async () => {
      const result = await getConnectionPointAttachmentDownloadLinksAction([id])
  ...
  function handleDelete(ids: string[]) {
    ...
    startTransition(async () => {
      const result = await deleteConnectionPointAttachmentsAction(ids)
      if (!result.success) {
      ...
      <section className="soubory-page__layout grid min-w-0 w-full items-start gap-3 overflow-x-hidden lg:items-stretch lg:grid-cols-[280px_minmax(0,1fr)_360px]">
        <aside className="soubory-page__sidebar min-w-0 w-full rounded-3xl border border-zinc-200 bg-[#f8fafc] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)] lg:h-full">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">{visibleFolders.length}</span>
          </div>
            ...
            {visibleFolders.map((folder) => {
              ...
              return (
                ...
                >
                  ...
                  {folder.siteAddress.trim() ? (
                    <div className="mt-0.5 truncate text-[11px] leading-tight opacity-65">
                      {folder.siteAddress}
                    </div>
                  ) : null}
        ...
        <section className="soubory-page__files-panel min-w-0 w-full rounded-3xl border border-zinc-200 bg-[#f8fafc] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)] lg:h-full">
          ...
          <div className="mb-3">
            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <button
                ...
                onClick={() => runDownload(checkedVisibleIds, 'pripojne-body-vyber.zip')}
              ...
              <button
                ...
                onClick={() => runDownload(visibleFiles.map((file) => file.id), 'pripojne-body-vse-filtrovane.zip')}
                className="soubory-page__secondary-button inline-flex h-9 w-full items-center justify-center rounded-xl border border-white/75 bg-[linear-gradient(155deg,rgba(255,255,255,0.88)_0%,rgba(238,242,247,0.8)_100%)] px-2 text-[11px] font-bold uppercase text-gray-800 shadow-[inset_0_1px_0_rgba(255,255,255,0.92),0_7px_18px_rgba(15,23,42,0.10)] disabled:opacity-40"
              >
                Stáhnout vše (filtr)
              </button>
              <button
                ...
                onClick={() => {
                  ...
                  runDownload(ids, `pripojne-body-${selectedJobId}.zip`)
                }}
                className="soubory-page__secondary-button inline-flex h-9 w-full items-center justify-center rounded-xl border border-white/75 bg-[linear-gradient(155deg,rgba(255,255,255,0.88)_0%,rgba(238,242,247,0.8)_100%)] px-2 text-[11px] font-bold uppercase text-gray-800 shadow-[inset_0_1px_0_rgba(255,255,255,0.92),0_7px_18px_rgba(15,23,42,0.10)] disabled:opacity-40"
              >
                Stáhnout vše (složka)
          ...
          {errorMessage ? (
            <div className="soubory-page__error mb-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
              {errorMessage}
            </div>
          ) : null}

          <div className="mb-2 flex items-center justify-between">
            <div className="soubory-page__file-count text-sm text-gray-500">
              {visibleFiles.length} souborů
            </div>
            ...
            {visibleFiles.length === 0 ? (
              ...
              <ul className="soubory-page__file-list divide-y divide-gray-100">
                {visibleFiles.map((file) => (
                  <li
                    key={file.id}
                    data-selected={selectedVisibleFileId === file.id}
                    className={`soubory-page__file-row p-3 transition ${selectedVisibleFileId === file.id ? 'bg-[#2980B9]/5' : 'bg-white/92'}`}
                  >
        ...
        <aside className="soubory-page__preview-panel hidden min-w-0 w-full rounded-3xl border border-zinc-200 bg-[#f8fafc] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)] lg:block lg:h-full">
          ...
          {!selectedFile ? (
            ...
            <>
              ...
              {previewForSelectedFile ? (
                selectedFile.mimeType?.startsWith('image/') ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={previewForSelectedFile}
...
function UploadModal({
  ...
  const [category, setCategory] = useState<ConnectionPointAttachmentCategory>('foto')
  ...
  function submitUpload() {
    startTransition(async () => {
      ...
      const result = await uploadConnectionPointFilesAction(formData)
      if (!result.success) {
      ...
      <div className="soubory-page__upload-shell mx-auto mt-10 w-full max-w-xl rounded-3xl border border-white/75 bg-[linear-gradient(168deg,rgba(255,255,255,0.94)_0%,rgba(249,250,251,0.88)_42%,rgba(244,244,245,0.84)_100%)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.92),0_34px_84px_rgba(24,24,27,0.30)]">
        ...
        <div className="mt-3 space-y-3">
          ...
          <div>
            ...
            <select
              value={category}
              onChange={(event) => setCategory(event.target.value as ConnectionPointAttachmentCategory)}
              className="soubory-page__upload-select h-10 w-full appearance-none rounded-xl border border-gray-200 bg-white/96 px-3 text-sm text-gray-900 shadow-[inset_0_1px_0_rgba(255,255,255,0.95),0_8px_18px_rgba(39,39,42,0.08)] outline-none transition focus:border-[#9dc7e5] focus:ring-2 focus:ring-[#b9d8ef]"
            >
