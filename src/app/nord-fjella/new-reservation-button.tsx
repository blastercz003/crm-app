...
import {
  ...
  type CreateNordFjellaReservationActionState,
} from './actions'
import { createNordFjellaReservationInitialState } from './action-states'
import type { NordFjellaGuestRow, NordFjellaSettingsRow } from '@/lib/nord-fjella/types'
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center justify-center rounded-2xl border border-[#2b6f9f]/95 bg-[linear-gradient(160deg,rgba(60,132,186,0.95)_0%,rgba(41,117,174,0.96)_45%,rgba(26,92,146,0.98)_100%)] px-4 py-2.5 text-sm font-medium uppercase text-white shadow-[inset_0_1px_0_rgba(170,217,247,0.42),0_12px_26px_rgba(9,48,82,0.32)] transition duration-200 ease-out hover:-translate-y-[1px] hover:border-[#1f5f8e] hover:bg-[linear-gradient(160deg,rgba(56,125,177,0.95)_0%,rgba(37,109,163,0.96)_45%,rgba(22,86,138,0.98)_100%)]"
      >
        NOVÝ PRONÁJEM
      </button>

      {isOpen ? (
        <CreateReservationModal guests={guests} settings={settings} onClose={() => setIsOpen(false)} />
      ) : null}
    </>
  )
}
...
function CreateReservationModal({
  ...
  const [recordType, setRecordType] = useState<RecordType>('reservation')
  const [guestType, setGuestType] = useState<GuestType>('person')
  ...
  const [guestCountry, setGuestCountry] = useState('Česká republika')
  const [guestBirthDate, setGuestBirthDate] = useState('')
  const [guestIdentityDocumentNumber, setGuestIdentityDocumentNumber] = useState('')
  const [guestIco, setGuestIco] = useState('')
  const [guestDic, setGuestDic] = useState('')
  const [guestNote, setGuestNote] = useState('')
  const [adultCount, setAdultCount] = useState('2')
  const [childCount, setChildCount] = useState('0')
  const [stayStartDate, setStayStartDate] = useState('')
  const [stayEndDate, setStayEndDate] = useState('')
  const [accommodationNightRate, setAccommodationNightRate] = useState('0')
  const [accommodationVatRate, setAccommodationVatRate] = useState(
    String(settings?.default_accommodation_vat_rate ?? 12)
  )
  const [cleaningFee, setCleaningFee] = useState(String(settings?.default_cleaning_fee ?? 0))
  const [cleaningVatRate, setCleaningVatRate] = useState('12')
  const [cityTaxPersonCount, setCityTaxPersonCount] = useState('2')
          ...
          <div className="clients-modal__header flex shrink-0 items-start justify-between gap-4 border-b border-gray-100/90 bg-[linear-gradient(180deg,rgba(255,255,255,0.70)_0%,rgba(255,255,255,0.24)_100%)] px-4 py-3 sm:px-5 sm:py-4">
            <div className="min-w-0">
              ...
              <p className="clients-modal__subtitle mt-1 text-sm text-gray-500">
                Založení rezervace nebo blokace termínu pro Nord Fjella.
              </p>
            </div>
              ...
              <div className="grid gap-5">
                ...
                {recordType === 'reservation' ? (
                  <>
                    <div className="nord-fjella-modal-card rounded-2xl border border-white/80 bg-[linear-gradient(155deg,rgba(255,255,255,0.92)_0%,rgba(241,245,249,0.84)_100%)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)]">
                      ...
                      <div className="grid gap-4 md:grid-cols-4">
                        <div className="space-y-2 md:col-span-2">
                          ...
                          <div ref={guestMenuRef} className="relative">
                            ...
                            {isGuestMenuOpen && filteredGuests.length > 0 ? (
                              <div className="nord-fjella-guest-menu absolute left-0 right-0 top-[calc(100%+0.5rem)] z-20 overflow-hidden rounded-2xl border border-white/80 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.14)]">
                                <div className="max-h-64 overflow-y-auto p-2">
                                  {filteredGuests.map((guest) => (
                                    <button
                                      ...
                                      className="nord-fjella-guest-menu-option flex w-full flex-col items-start rounded-xl px-3 py-2 text-left transition hover:bg-slate-50"
                                    >
                        ...
                        <div className="space-y-2">
                          ...
                          <select name="reservation_status" defaultValue="reserved" className={selectClassName}>
                      ...
                      {guestType === 'person' ? (
                        <div className="space-y-2">
                          ...
                          <input name="guest_full_name" value={guestFullName} onChange={(event) => setGuestFullName(event.target.value)} required className={inputClassName} />
                        </div>
                      ) : (
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">Název firmy</label>
                          <input name="guest_company_name" value={guestCompanyName} onChange={(event) => setGuestCompanyName(event.target.value)} required className={inputClassName} />
                        </div>
                      )}
                      ...
                      <div className="space-y-2">
                        ...
                        <input name="guest_email" type="email" value={guestEmail} onChange={(event) => setGuestEmail(event.target.value)} required className={inputClassName} />
                      </div>

                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Telefon</label>
                        <input name="guest_phone" value={guestPhone} onChange={(event) => setGuestPhone(event.target.value)} required className={inputClassName} />
                      </div>

                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Ulice a číslo</label>
                        <input name="guest_street" value={guestStreet} onChange={(event) => setGuestStreet(event.target.value)} required className={inputClassName} />
                      </div>

                      <div className="grid gap-4 md:grid-cols-3 md:col-span-2">
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">Město</label>
                          <input name="guest_city" value={guestCity} onChange={(event) => setGuestCity(event.target.value)} required className={inputClassName} />
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">PSČ</label>
                          <input name="guest_postal_code" value={guestPostalCode} onChange={(event) => setGuestPostalCode(event.target.value)} required className={inputClassName} />
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">Země</label>
                          <input name="guest_country" value={guestCountry} onChange={(event) => setGuestCountry(event.target.value)} required className={inputClassName} />
                        </div>
                      </div>

                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Datum narození</label>
                        <input name="guest_birth_date" type="date" value={guestBirthDate} onChange={(event) => setGuestBirthDate(event.target.value)} required className={inputClassName} />
                      </div>

                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Číslo OP / pasu</label>
                        <input name="guest_identity_document_number" value={guestIdentityDocumentNumber} onChange={(event) => setGuestIdentityDocumentNumber(event.target.value)} required className={inputClassName} />
                      </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-4">
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Místní poplatek / osoba / noc</label>
                        <input name="city_tax_rate" defaultValue={settings?.default_city_tax_rate ?? 30} required className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Počet osob pro poplatek</label>
                        <input
                    ...
                    <div className="grid gap-4 md:grid-cols-4">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          defaultValue={settings?.default_security_deposit ?? 0}
                          required
                          className={inputClassName}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <input name="requested_deposit_amount" defaultValue="0" className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Splatnost zálohy</label>
                        <input name="deposit_due_date" type="date" className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Platební stav</label>
                        <select name="payment_status" defaultValue="unpaid" className={selectClassName}>
                    ...
                    <div className="grid gap-4 md:grid-cols-4">
                      <div className="space-y-2">
                        ...
                        <input name="deposit_paid_amount" defaultValue="0" className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Datum úhrady zálohy</label>
                        <input name="deposit_paid_at" type="date" className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Forma úhrady zálohy</label>
                        <select name="deposit_payment_method" defaultValue="" className={selectClassName}>
                      ...
                      <div className="space-y-2">
                        ...
                        <select name="settlement_status" defaultValue="draft" className={selectClassName}>
                    ...
                    <div className="grid gap-4 md:grid-cols-4">
                      <div className="space-y-2">
                        ...
                        <input name="balance_paid_amount" defaultValue="0" className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Datum úhrady doplatku</label>
                        <input name="balance_paid_at" type="date" className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Forma úhrady doplatku</label>
                        <select name="balance_payment_method" defaultValue="" className={selectClassName}>
                          <option value="">Nevybráno</option>
                          <option value="bank_transfer">Bankovní převod</option>
                          <option value="cash">Hotově</option>
                        </select>
                      ...
                      <div className="space-y-2">
                        ...
                        <input name="balance_due_date" type="date" className={inputClassName} />
                      </div>
                    </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <input name="cancellation_fee_vat_rate" defaultValue="12" className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">DUZP</label>
                        <input name="taxable_supply_date" type="date" className={inputClassName} />
                        <p className="text-[11px] text-gray-500">Prázdné = datum odjezdu</p>
                      </div>
                      <div className="space-y-2">
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                      <div className="space-y-2">
                        ...
                        <input name="payment_refund_amount" defaultValue="0" className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Datum vrácení platby</label>
                        <input name="payment_refund_at" type="date" className={inputClassName} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Forma vrácení</label>
                        <select name="payment_refund_method" defaultValue="" className={selectClassName}>
                          <option value="">Nevybráno</option>
                          <option value="bank_transfer">Bankovní převod</option>
                          <option value="cash">Hotově</option>
                        </select>
                    ...
                    <div className="nord-fjella-modal-card rounded-2xl border border-white/80 bg-[linear-gradient(155deg,rgba(255,255,255,0.92)_0%,rgba(241,245,249,0.84)_100%)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)]">
                      ...
                      <div className="grid gap-4 md:grid-cols-4">
                        <label className="nord-fjella-modal-subtle-card nord-fjella-modal-subtle-card-text flex items-center gap-3 rounded-2xl border border-white/75 bg-white/80 px-4 py-3 text-sm text-gray-900">
                          <input type="checkbox" name="security_deposit_received" className="h-4 w-4" />
                          Kauce přijata
                        </label>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">Datum přijetí</label>
                          <input name="security_deposit_received_at" type="date" className={inputClassName} />
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">Vrácená kauce</label>
                          <input name="security_deposit_refund_amount" defaultValue="0" className={inputClassName} />
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">Datum vrácení</label>
                          <input name="security_deposit_refunded_at" type="date" className={inputClassName} />
                        </div>
                      </div>
                      <div className="mt-4 grid gap-4 md:grid-cols-3">
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">Zadržená část kauce</label>
                          <input name="security_deposit_withheld_amount" defaultValue="0" className={inputClassName} />
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">Datum zadržení</label>
                          <input name="security_deposit_withheld_at" type="date" className={inputClassName} />
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-900">Důvod zadržení</label>
                          <input name="security_deposit_withheld_reason" className={inputClassName} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <textarea name="internal_note" rows={4} className={`clients-modal__textarea ${inputClassName} min-h-[120px] py-3`} />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-900">Veřejná poznámka do PDF</label>
                        <textarea
                          name="public_note"
                          rows={4}
                          defaultValue={settings?.public_note_template ?? ''}
                          className={`clients-modal__textarea ${inputClassName} min-h-[120px] py-3`}
                        />
                      </div>
                    </div>
                    ...
                    <div className="nord-fjella-modal-subtle-card p-4">
                      <div className="text-sm font-semibold text-gray-900">Doklady hosta</div>
                      <p className="mt-2 text-sm text-gray-600">
                        OP, ŘP nebo další doklady bude možné nahrát až po vytvoření rezervace
                        v detailu záznamu.
                      </p>
                    </div>
                  </>
                  ...
                  <div className="nord-fjella-modal-card rounded-2xl border border-white/80 bg-[linear-gradient(155deg,rgba(255,255,255,0.92)_0%,rgba(241,245,249,0.84)_100%)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)]">
                    <div className="grid gap-4 md:grid-cols-3">
                      ...
                      <div className="space-y-2 md:col-span-2">
                        <label className="text-sm font-medium text-gray-900">Interní poznámka</label>
                        <input name="internal_note" className={inputClassName} placeholder="Např. servis, rodinný pobyt, odstávka objektu" />
                      </div>
                    </div>
                  </div>
                )}
