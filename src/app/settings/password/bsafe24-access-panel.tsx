...
type BSafe24AccessPanelProps = {
  isAdmin: boolean
}

export function BSafe24AccessPanel({ isAdmin }: BSafe24AccessPanelProps) {
  const [profiles, setProfiles] = useState<BSafe24AccessProfile[]>([])
  ...
  useEffect(() => {
    ...
    async function loadProfiles() {
      ...
      try {
        const response = await fetch('/settings/password/bsafe24-access', {
          cache: 'no-store',
        })

        const payload = (await response.json().catch(() => null)) as
          | { error?: string; profiles?: BSafe24AccessProfile[] }
          | null
  ...
  const enabledCount = useMemo(
    () => profiles.filter((profile) => profile.can_view_bsafe24).length,
    [profiles]
  )

  async function toggleAccess(profile: BSafe24AccessProfile) {
    const nextValue = !profile.can_view_bsafe24
    ...
    setProfiles((currentProfiles) =>
      currentProfiles.map((item) =>
        item.id === profile.id
          ? { ...item, can_view_bsafe24: nextValue }
          : item
      )
    )
    ...
    try {
      ...
      setProfiles((currentProfiles) =>
        currentProfiles.map((item) =>
          item.id === profile.id
            ? { ...item, can_view_bsafe24: profile.can_view_bsafe24 }
            : item
        )
      )
      setError(
    ...
    <section className="password-page__panel flex h-full max-h-[34rem] flex-col overflow-hidden rounded-[24px] border border-white/70 bg-[linear-gradient(155deg,rgba(255,255,255,0.96)_0%,rgba(248,250,252,0.92)_48%,rgba(241,245,249,0.88)_100%)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_18px_36px_rgba(15,23,42,0.1)] backdrop-blur-[10px] md:p-5 xl:h-[34rem]">
      ...
      <div className="mt-3 flex-1 space-y-4 overflow-y-auto pr-1">
        <div className="rounded-2xl border border-white/75 bg-[linear-gradient(155deg,rgba(255,255,255,0.92)_0%,rgba(240,245,250,0.86)_100%)] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_8px_18px_rgba(15,23,42,0.08)] [html[data-theme='dark']_&]:border-[rgba(148,163,184,0.16)] [html[data-theme='dark']_&]:bg-[linear-gradient(155deg,rgba(17,27,46,0.96)_0%,rgba(12,20,34,0.94)_100%)] [html[data-theme='dark']_&]:shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_10px_22px_rgba(0,0,0,0.22)]">
          <div className="text-sm text-zinc-600 [html[data-theme='dark']_&]:text-slate-400">
            Aktivní přístup:{' '}
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-semibold text-zinc-900 [html[data-theme='dark']_&]:text-[#f8fbff]">
                        {profile.name}
                      </div>
                      <div className="mt-1 text-xs uppercase tracking-[0.12em] text-zinc-400 [html[data-theme='dark']_&]:text-slate-400">
                        {profile.role || 'Bez role'}
                      </div>
                    </div>

                    <div className="flex items-center gap-3">
                      <span className="text-xs font-semibold text-zinc-600 [html[data-theme='dark']_&]:text-slate-400">
                        {profile.can_view_bsafe24 ? 'Aktivní' : 'Neaktivní'}
                      </span>
                      <button
                        type="button"
                        role="switch"
                        aria-checked={profile.can_view_bsafe24}
                        disabled={isSaving}
