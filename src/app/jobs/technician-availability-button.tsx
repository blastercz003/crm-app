    ...
    <div className="fixed inset-0 z-[170] flex items-center justify-center bg-slate-950/45 p-3 backdrop-blur-[2px] sm:p-6" role="dialog" aria-modal="true" aria-labelledby="technician-availability-title">
      <div className="jobs-page__modal-shell flex h-[calc(100dvh-1.5rem)] w-full max-w-5xl flex-col overflow-hidden rounded-[30px] border border-white/80 bg-[linear-gradient(155deg,rgba(255,255,255,0.98)_0%,rgba(244,248,252,0.95)_52%,rgba(233,242,249,0.92)_100%)] shadow-[0_28px_70px_rgba(15,23,42,0.28)] sm:h-[min(760px,calc(100dvh-3rem))]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
          {isLoading || !data ? (
            <div className="rounded-2xl border border-white/80 bg-white/70 px-5 py-10 text-center text-sm text-slate-500 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)]">Načítám pohotovosti techniků…</div>
          ) : (
            <div className={`grid gap-5 transition-opacity duration-150 lg:h-full lg:min-h-0 lg:grid-cols-2 ${isRefreshing ? 'pointer-events-none opacity-60' : ''}`}>
              <section className="rounded-[24px] border border-white/85 bg-white/72 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.94),0_12px_28px_rgba(15,23,42,0.07)] sm:p-5 lg:grid lg:min-h-0 lg:grid-rows-[2fr_1fr]">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    ...
                    <div className="inline-flex items-center gap-1 rounded-xl border border-[#c8ddec] bg-[#edf6fc]/85 p-1 text-xs font-semibold text-[#316f9e]">
                ...
                <div className="mt-4 space-y-2.5 lg:min-h-0 lg:flex-1 lg:overflow-y-auto lg:pr-1">
                  {data.selectedWeekendDates.map((date, index) => {
                    ...
                    return (
                      <div key={date} className="rounded-2xl border border-[#d2e3f0] bg-[linear-gradient(155deg,rgba(249,252,254,0.95)_0%,rgba(239,247,252,0.86)_100%)] px-3.5 py-3">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          {canEdit ? <button type="button" onClick={() => setEditingDate(date)} className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-[#bcd7ea] bg-white/85 text-[#3d7eab] transition hover:-translate-y-[1px] hover:bg-[#eff8fd]" aria-label={`Upravit pohotovost pro ${DAY_LABELS[dayKey]}`}><PencilIcon /></button> : null}
                        </div>
                ...
                <div className="mt-5 border-t border-[#d5e4ef] pt-4 lg:mt-0 lg:flex lg:min-h-0 lg:flex-col">
                  ...
                  <div className="mt-2 max-h-52 space-y-2 overflow-y-auto pr-1 lg:max-h-none lg:min-h-0 lg:flex-1">
                    {history.length === 0 ? <p className="rounded-xl border border-dashed border-slate-200 px-3 py-3 text-sm text-slate-400">Zatím žádná historie.</p> : history.map((entry) => <div key={entry.date} className="rounded-xl border border-slate-200/80 bg-slate-50/70 px-3 py-2.5 text-sm text-slate-400"><div className="flex items-start justify-between gap-3"><span>{formatDate(entry.date)}</span><div className="flex items-start gap-1.5"><span className="text-right">{entry.technicianIds.map((id) => technicianNameById.get(id) ?? '—').join(', ')}</span>{data.isAdmin ? <><button type="button" onClick={() => setEditingDate(entry.date)} className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white/75 text-slate-400 transition hover:text-[#3d7eab]" aria-label={`Upravit historii pohotovosti: ${formatDate(entry.date)}`}><PencilIcon /></button><button type="button" onClick={() => setPendingDelete({ kind: 'on-call', weekendDate: entry.date, technicianName: entry.technicianIds.map((id) => technicianNameById.get(id) ?? 'Technik').join(', '), timeRange: formatDate(entry.date) })} className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white/75 text-slate-400 transition hover:text-red-500" aria-label={`Smazat historii pohotovosti: ${formatDate(entry.date)}`}>×</button></> : null}</div></div>{editingDate === entry.date ? <DayEditor technicians={data.technicians} selectedIds={entry.technicianIds} onCancel={() => setEditingDate(null)} onSave={(ids) => saveDay(entry.date, ids)} /> : null}</div>)}
                  </div>
                </div>
              </section>
              ...
              <section className="rounded-[24px] border border-white/85 bg-white/72 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.94),0_12px_28px_rgba(15,23,42,0.07)] sm:p-5 lg:grid lg:min-h-0 lg:grid-rows-[2fr_1fr]">
                ...
                <div className="mt-4 space-y-2.5 lg:min-h-0 lg:flex-1 lg:overflow-y-auto lg:pr-1">
                  {selectedWeekendUnavailability.length === 0 ? <div className="rounded-2xl border border-dashed border-[#c8ddec] bg-[#f4f9fc]/75 px-4 py-6 text-center text-sm text-slate-500">Není evidována žádná aktuální nedostupnost.</div> : selectedWeekendUnavailability.map((entry) => {
                    const canEdit = canEditUnavailability(entry)
                    const technicianName = technicianNameById.get(entry.technician_id) ?? 'Technik'
                    return <div key={entry.id} className="rounded-2xl border border-[#d2e3f0] bg-[linear-gradient(155deg,rgba(249,252,254,0.95)_0%,rgba(239,247,252,0.86)_100%)] px-3.5 py-3">
                ...
                <div className="mt-5 border-t border-[#d5e4ef] pt-4 lg:mt-0 lg:flex lg:min-h-0 lg:flex-col">
                  ...
                  <div className="mt-2 max-h-52 space-y-2 overflow-y-auto pr-1 lg:max-h-none lg:min-h-0 lg:flex-1">
                    {unavailabilityHistory.length === 0 ? <p className="rounded-xl border border-dashed border-slate-200 px-3 py-3 text-sm text-slate-400">Zatím žádná historie.</p> : unavailabilityHistory.map((entry) => {
                      const technicianName = technicianNameById.get(entry.technician_id) ?? 'Technik'
                      return <div key={entry.id} className="rounded-xl border border-slate-200/80 bg-slate-50/70 px-3 py-2.5 text-sm text-slate-400">{editingUnavailabilityId === entry.id ? <UnavailabilityEditor entry={entry} technicians={data.technicians} currentUserId={data.currentUserId} canManage={data.canManage} onCancel={() => setEditingUnavailabilityId(null)} onSave={saveUnavailability} /> : <div className="flex items-start justify-between gap-3"><div><p>{technicianName}</p><p className="mt-0.5 text-xs">{formatDateTimeRange(entry.starts_at, entry.ends_at)}</p></div>{data.isAdmin ? <div className="flex gap-1"><button type="button" onClick={() => setEditingUnavailabilityId(entry.id)} className="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 bg-white/75 text-slate-400 transition hover:text-[#3d7eab]" aria-label={`Upravit historii nedostupnosti: ${technicianName}`}><PencilIcon /></button><button type="button" onClick={() => setPendingDelete({ kind: 'unavailability', id: entry.id, technicianName, timeRange: formatDateTimeRange(entry.starts_at, entry.ends_at) })} className="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 bg-white/75 text-slate-400 transition hover:text-red-500" aria-label={`Smazat historii nedostupnosti: ${technicianName}`}>×</button></div> : null}</div>}</div>
                    })}
                  </div>
                </div>
              </section>
...
function DayEditor({ technicians, selectedIds, onCancel, onSave }: { technicians: TechnicianOption[]; selectedIds: string[]; onCancel: () => void; onSave: (ids: string[]) => Promise<boolean> }) {
  ...
  return (
    <div className="mt-2 border-t border-[#d8e7f1] pt-2">
...
function UnavailabilityEditor({
  ...
  return (
    <div className="mt-3 rounded-2xl border border-[#bcd8eb] bg-[#eef7fc]/85 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.92)]">
      ...
      <div className="mt-2 flex justify-end gap-1.5"><button type="button" onClick={onCancel} disabled={isSaving} className="rounded-xl border border-[#cbdce9] bg-white/85 px-2.5 py-1.5 text-[11px] font-semibold uppercase text-slate-600 transition hover:-translate-y-[1px]">Zrušit</button><button type="button" onClick={() => void save()} disabled={isSaving || !technicianId || !startsAt || !endsAt} className="rounded-xl border border-[#2f78aa] bg-[#377fb4] px-2.5 py-1.5 text-[11px] font-semibold uppercase text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.3)] transition hover:-translate-y-[1px] hover:bg-[#2e73a6] disabled:cursor-not-allowed disabled:opacity-65">{isSaving ? 'Ukládám…' : 'Uložit'}</button></div>
    </div>
  )
}
...
function DeleteUnavailabilityConfirmation({
  ...
  return (
    <div className="absolute inset-0 z-10 flex items-center justify-center bg-slate-950/30 p-4 backdrop-blur-[1px]">
      <div className="w-full max-w-sm rounded-[24px] border border-white/85 bg-[linear-gradient(155deg,rgba(255,255,255,0.98)_0%,rgba(244,248,252,0.96)_100%)] p-5 text-center shadow-[0_22px_50px_rgba(15,23,42,0.25)]">
