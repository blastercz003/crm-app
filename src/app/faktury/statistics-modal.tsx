            ...
            {!payload ? (
              ...
              >
                <section className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-3">
                  <StatisticCard label="Počet zakázek" value={formatMetric(payload.summary.jobCount)} />
                  <StatisticCard label="Marné výjezdy" value={formatMetric(payload.summary.wastedTripCount)} />
                  <StatisticCard label="Ujeté kilometry" value={`${formatMetric(payload.summary.kilometers)} km`} />
                  <StatisticCard label="Odpracované hodiny" value={`${formatMetric(payload.summary.hours)} h`} />
                  <StatisticCard label="Průměr km / zakázku" value={`${formatMetric(payload.summary.averageKilometersPerJob)} km`} />
                  <StatisticCard label="Průměr hodin / zakázku" value={`${formatMetric(payload.summary.averageHoursPerJob)} h`} />
                </section>
                ...
                <section className="mt-4 overflow-hidden rounded-[24px] border border-white/80 bg-white/72 shadow-[inset_0_1px_0_rgba(255,255,255,0.92),0_12px_28px_rgba(15,23,42,0.08)] [html[data-theme='dark']_&]:border-[rgba(148,163,184,0.15)] [html[data-theme='dark']_&]:bg-[rgba(13,23,39,0.88)] [html[data-theme='dark']_&]:shadow-[inset_0_1px_0_rgba(255,255,255,0.03),0_14px_30px_rgba(0,0,0,0.24)]">
                  <div className="flex items-center justify-between gap-3 border-b border-zinc-200/70 px-4 py-3 [html[data-theme='dark']_&]:border-[rgba(148,163,184,0.12)]">
                    <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-500 [html[data-theme='dark']_&]:text-slate-400">
                      Přehled techniků
                    </div>
                    <div className="text-xs text-zinc-500 [html[data-theme='dark']_&]:text-slate-500">
                      {payload.technicians.length} techniků
                    </div>
                  </div>

                  {payload.technicians.length === 0 ? (
                    <div className="px-4 py-12 text-center text-sm text-zinc-500 [html[data-theme='dark']_&]:text-slate-400">
                      V tomto období nejsou výkony přiřazené žádnému technikovi.
                    </div>
                  ) : (
                    <div className="divide-y divide-zinc-200/70 [html[data-theme='dark']_&]:divide-[rgba(148,163,184,0.12)]">
                      {payload.technicians.map((technician) => (
                        <details key={technician.id} className="group">
                          <summary className="grid cursor-pointer list-none grid-cols-[minmax(0,1fr)_repeat(3,auto)_18px] items-center gap-3 px-4 py-3 text-sm transition hover:bg-slate-50/80 sm:grid-cols-[minmax(180px,1fr)_100px_130px_110px_18px] [html[data-theme='dark']_&:hover]:bg-white/[0.025]">
                          ...
                          <div className="border-t border-zinc-200/60 bg-slate-50/70 px-3 py-3 [html[data-theme='dark']_&]:border-[rgba(148,163,184,0.1)] [html[data-theme='dark']_&]:bg-[rgba(5,12,23,0.42)]">
                            <div className="grid gap-2">
                              {technician.jobs.map((job) => (
                                <div
                                  key={job.id}
                                  className="grid grid-cols-[82px_minmax(0,1fr)_auto_auto] items-center gap-3 rounded-xl border border-zinc-200/70 bg-white/80 px-3 py-2 text-xs [html[data-theme='dark']_&]:border-[rgba(148,163,184,0.12)] [html[data-theme='dark']_&]:bg-[rgba(15,25,42,0.76)]"
                                >
                                  <div>
                                    <div className="font-semibold text-zinc-900 [html[data-theme='dark']_&]:text-slate-100">
                                      {job.jobNumber || '—'}
                                    </div>
                                    <div className="mt-0.5 text-[10px] text-zinc-500 [html[data-theme='dark']_&]:text-slate-500">
                                  ...
                                  <div className="whitespace-nowrap text-right text-zinc-700 [html[data-theme='dark']_&]:text-slate-200">
                                    ...
                                    {job.isWastedTrip ? (
                                      <span className="ml-2 text-[9px] font-semibold uppercase text-rose-600 [html[data-theme='dark']_&]:text-rose-300">
                                        Marný
                                      </span>
                                    ) : null}
                                  </div>
